...
void CDummy::UpdateLoopingSound()
{
	...
	{
		m_looping_sound_id = sfx_manager->PlaySfx( m_looping_sound_checksum, 0, 0 );
	}
...
//					Pcm::PlayStreamFromObject(p_dummy,stream_name,drop_off,volume,pitch,channel,use_pos_info);
			...
			case PLAY_SKATER_SOUND_EFFECT:				
			{
				...
				Env::CTerrainManager::sPlaySfx((Env::ETerrainActionType) which_array, (ETerrainType) surface_flag,pos,vol_percent,100.0f,0.0f,false);
				break;
			}
...
//				sfx_manager->PlaySfx(checksum,vol_l,vol_r,pitch);
				sfx_manager->PlaySfx(checksum,&vol,pitch);
				break;
			}
			...
			case PLAY_LOOPING_SOUND:
			{
				...
				if (whichList==PLAYBACK_DUMMY_LIST)
				{
					...
					p_dummy->m_looping_sound_id = sfx_manager->PlaySfx( checksum, 0, 0 );
				}
				else
				{
...
void WritePlaySfx(uint32 checksum, float volL, float volR, float pitch)
{
	if (sMode!=RECORD)
	{
		return;
...
bool CDummy::LoadSkeleton( uint32 skeletonName )
{
    ...
    pTempStructure->AddChecksum( Crc::ConstCRC("skeleton"), skeletonName );
