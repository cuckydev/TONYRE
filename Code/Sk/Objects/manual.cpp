		...
		mpSkaterBalanceTrickComponent->mBalanceTrickType==0x8d10119d) // Slide
	{
		...
		else
		{
			...
			if (p_rail_node->ProbablyOnSameRailAs(mOldRailNode))
			{
				...
				mManualTime+=mpSkaterBalanceTrickComponent->GetBalanceStat(Crc::ConstCRC("Same_Grind_Add_Time"));
				#ifdef		__DEBUG_CHEESE__
			...
			else
			{
				...
				mManualTime-=mpSkaterBalanceTrickComponent->GetBalanceStat(Crc::ConstCRC("New_Grind_Sub_Time"));
				if (mManualTime<0.0f)
				{
					mManualTime=0.0f;
				}	
...
//			printf ("SHOULD ADJUST BALANCE HERE, AS ON A ROBOT LINE\n");
			...
			mManualTime += Script::GetFloat(Crc::ConstCRC("robot_rail_add_time") )* 0.5f / robot;
			...
			mManualLean += Script::GetFloat(Crc::ConstCRC("robot_rail_nudge")) * Mth::Sgn(mManualLeanDir) * 0.5f / robot;
//			printf ("Time = %.2f,  Lean = %.2f, dir = %.2f\n",mManualTime, mManualLean, mManualLeanDir);
	...
	float repeat_min = mpSkaterBalanceTrickComponent->GetBalanceStat(Crc::ConstCRC("Repeat_Min"));
	if ( Mth::Abs(mManualLeanDir) != 0.0f)
	{
		// first multiplied by the reat multipler
		mManualLeanDir = mManualLeanDir * mpSkaterBalanceTrickComponent->GetBalanceStat(Crc::ConstCRC("Repeat_Multiplier"));		
...
//	mManualLean = Mth::Abs(mManualLean) * Mth::Sgn(mManualLeanDir);		// possibly alter lean to face the bail direction
	...
	mManualLean *= mpSkaterBalanceTrickComponent->GetBalanceStat(Crc::ConstCRC("Lean_Repeat_Multiplier"));
	...
	float BailAngle=mpSkaterBalanceTrickComponent->GetBalanceStat(Crc::ConstCRC("Lean_Bail_Angle"));
	...
	// Reset the cheese.
	mManualCheese=mpSkaterBalanceTrickComponent->GetBalanceStat(Crc::ConstCRC("Cheese"));
...
void CManual::DoManualPhysics()
{
	...
	{
		mManualCheese -= scale_with_frame_length(frame_length, mpSkaterBalanceTrickComponent->GetBalanceStat(Crc::ConstCRC("Cheese"))/mpSkaterBalanceTrickComponent->GetBalanceStat(Crc::ConstCRC("CheeseFrames")));
...
//	mManualLean += scale_with_frame_length(mManualLeanDir * (0.5f + mManualTime/2)/ManualInstableRate);	 // speed of turn
	...
	uint32   safe_period = (uint32)Script::GetInt(Crc::ConstCRC("BalanceSafeButtonPeriod"));
	...
	{
		if (Mth::Abs(mManualLeanDir) < mpSkaterBalanceTrickComponent->GetBalanceStat(Crc::ConstCRC("Lean_Min_Speed")))
		{
			int r=(int)mpSkaterBalanceTrickComponent->GetBalanceStat(Crc::ConstCRC("Lean_Rnd_Speed"));
			r|=1;
//			mManualLeanDir = Mth::Rnd(r)-r/2;
	...
	*/
	...
	if (Mth::Abs(mManualLean)>mpSkaterBalanceTrickComponent->GetBalanceStat(Crc::ConstCRC("Lean_Bail_Angle")))
	{
		if (mManualLean>0)
		{
			mpSkater->SelfEvent(Crc::ConstCRC("OffMeterTop"));
		}
		else	
		{
			mpSkater->SelfEvent(Crc::ConstCRC("OffMeterBottom"));
		}
