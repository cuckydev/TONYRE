...
void CMovieDetails::SetParams( Script::CStruct* pParams )
{
	...
	int skippable = 0;
	pParams->GetInteger( Crc::ConstCRC("skippable"), &skippable, Script::NO_ASSERT );
	m_skippable = skippable;

	// if we want the movie to continue to hold on the last frame (i.e. don't go on to the next anim)
	m_holdOnLastFrame = pParams->ContainsFlag( Crc::ConstCRC("play_hold") );

	int allow_pause = 0;
	pParams->GetInteger( Crc::ConstCRC("allow_pause"), &allow_pause, Script::NO_ASSERT );
...
Script::CStruct* CMovieDetails::GetParams()
{
	...
	pReturnParams->AddInteger( Crc::ConstCRC("skippable"), m_skippable );
	pReturnParams->AddInteger( Crc::ConstCRC("allow_pause"), m_allowPause );
...
bool CObjectAnimDetails::InitFromStructure( Script::CStruct* pParams )
{
	...
	if ( !pParams->ContainsFlag( Crc::ConstCRC("virtual_cam") ) )
	{
		...
		this->set_frame_data( pData, 
							pParams->ContainsFlag( Crc::ConstCRC("loop") ) ? Gfx::LOOPING_CYCLE : Gfx::LOOPING_HOLD,
							pParams->ContainsFlag( Crc::ConstCRC("backwards") ) );
	}
	else
	{
		int numFrames = 0;
		pParams->GetInteger( Crc::ConstCRC("frames"), &numFrames, Script::ASSERT );
...
void CObjectAnimDetails::update_moving_objects()
{
	...
	for ( int i = 0; i < mp_frameData->GetNumBones(); i++ )
	{
		...
		if ( pObject )
		{
			...
			if ( Script::GetInt( Crc::ConstCRC("moviecam_debug") ) )
			{
...
void CSkaterCamDetails::set_target_params( Script::CStruct* pParams )
{
	...
	m_hasTarget = pParams->GetChecksum( Crc::ConstCRC("targetID"), &m_targetID, Script::NO_ASSERT );
	
	if ( m_hasTarget )
	{
		// target offset, if any
		m_targetOffset.Set(0.0f,0.0f,0.0f);
		pParams->GetVector( Crc::ConstCRC("targetOffset"), &m_targetOffset, Script::NO_ASSERT );
		...
		m_hasPositionOffset = pParams->GetVector( Crc::ConstCRC("positionOffset"), &m_positionOffset, Script::NO_ASSERT );
	}
}
...
void CSkaterCamDetails::update_camera()
{
	...
	if ( m_hasTarget )
	{
		if ( m_targetID != Crc::ConstCRC("world") )
		{
