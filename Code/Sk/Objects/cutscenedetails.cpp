...
//		mp_object[i]->MarkAsDead();
	...
	Script::RunScript( Crc::ConstCRC("CutsceneKillObjects") );

	Script::UnloadQB( Crc::ConstCRC("cutscenefrommemory.qb") );
}
...
Mth::Vector get_scale( Script::CStruct* pAppearance, uint32 bone_group_name )
{
	...
	if ( pAppearance->GetStructure( bone_group_name, &pVirtualDescStruct, Script::NO_ASSERT ) )
	{
		...
		pVirtualDescStruct->GetInteger( Crc::ConstCRC("use_default_scale"), &use_default_scale, Script::NO_ASSERT );
		
		if ( !use_default_scale )
		{
			int x, y, z;
			pVirtualDescStruct->GetInteger( Crc::ConstCRC("x"), &x, Script::ASSERT );
			pVirtualDescStruct->GetInteger( Crc::ConstCRC("y"), &y, Script::ASSERT );
			pVirtualDescStruct->GetInteger( Crc::ConstCRC("z"), &z, Script::ASSERT );
	...
	for ( int i = 0; i < numFiles; i++ )
	{
		...
		switch ( pFileInfo->fileExtensionChecksum )
		{
			...
			case 0x2e4bf21b:	// OBA
			{
				...
				add_anim( Crc::ConstCRC("cutscene_oba"), pData, pFileInfo->fileSize );
				mp_objectQuickAnim = new Nx::CQuickAnim;
				mp_objectQuickAnim->SetAnimAssetName( Crc::ConstCRC("cutscene_oba") );
...
//				m_objectAnimController.PlaySequence( 0, 0.0f, mp_objectAnimData->GetDuration(), Gfx::LOOPING_HOLD, 0.0f, 1.0f );
			...
			case 0x05ca1497:	// CAM
			{
				...
				add_anim( Crc::ConstCRC("cutscene_cam"), pData, pFileInfo->fileSize );
				Dbg_MsgAssert( !mp_cameraQuickAnim, ( "Cutscene file should have exactly 1 Camera\n" ) );
				mp_cameraQuickAnim = new Nx::CQuickAnim;
				mp_cameraQuickAnim->SetAnimAssetName( Crc::ConstCRC("cutscene_cam") );
...
//				m_cameraController.PlaySequence( 0, 0.0f, mp_cameraData->GetDuration(), Gfx::LOOPING_HOLD, 0.0f, 1.0f );
			...
			case 0x0524fd4e:	// MDL
			{
				...
				uint32 model_asset_name = pFileInfo->fileNameChecksum + Crc::ConstCRC("SKIN");// pFileInfo->fileExtensionChecksum;
//				Dbg_Message( "Adding model asset %08x", model_asset_name );
				...
				theInfo.pTextureData = mp_fileLibrary->GetFileData( pFileInfo->fileNameChecksum, Crc::ConstCRC("TEX"), true );
				const File::SFileInfo* pTextureFileInfo;
				pTextureFileInfo = mp_fileLibrary->GetFileInfo( pFileInfo->fileNameChecksum, Crc::ConstCRC("TEX"), true );
				...
				theInfo.pCASData = (uint8*)mp_fileLibrary->GetFileData( pFileInfo->fileNameChecksum, Crc::ConstCRC("CAS"), false );;

				theInfo.isSkin = pFileInfo->fileExtensionChecksum == Crc::ConstCRC("SKIN");		
				...
				if ( pFileInfo->fileExtensionChecksum == Crc::ConstCRC("GEOM") )
				{
				...
				uint8* pWeightMapData = (uint8*)mp_fileLibrary->GetFileData( pFileInfo->fileNameChecksum, Crc::ConstCRC("WGT"), false );
				if ( pWeightMapData )
				{
					...
					if ( Script::GetInteger( Crc::ConstCRC("CutsceneScaleTest"), Script::NO_ASSERT ) )
					{
						...
						pVec = Script::GetVector( Crc::ConstCRC("CutsceneScaleTestJaw") );
						testBoneScales[2] = Mth::Vector( pVec->mX, pVec->mY, pVec->mZ, 1.0f );
						pVec = Script::GetVector( Crc::ConstCRC("CutsceneScaleTestHat") );
						testBoneScales[3] = Mth::Vector( pVec->mX, pVec->mY, pVec->mZ, 1.0f );
						pVec = Script::GetVector( Crc::ConstCRC("CutsceneScaleTestBrow") );
...
#endif
					{
						...
						testBoneScales[0] = get_scale( pCASModelStruct, Crc::ConstCRC("head_bone_group") );
						testBoneScales[1] = get_scale( pCASModelStruct, Crc::ConstCRC("head_bone_group") );
						testBoneScales[2] = get_scale( pCASModelStruct, Crc::ConstCRC("jaw_bone_group") );
						testBoneScales[3] = get_scale( pCASModelStruct, Crc::ConstCRC("headtop_bone_group") );
						testBoneScales[4] = get_scale( pCASModelStruct, Crc::ConstCRC("nose_bone_group") );
					}
					...
					pInfo->GetInteger( Crc::ConstCRC("is_male"), &is_male_skater, Script::ASSERT );
				...
				mp_fileLibrary->ClearFile( pFileInfo->fileNameChecksum, pFileInfo->fileExtensionChecksum );
				mp_fileLibrary->ClearFile( pFileInfo->fileNameChecksum, Crc::ConstCRC("TEX") );
				mp_fileLibrary->ClearFile( pFileInfo->fileNameChecksum, Crc::ConstCRC("CAS") );
				mp_fileLibrary->ClearFile( pFileInfo->fileNameChecksum, Crc::ConstCRC("WGT") );
		  }
			break;
/*
			case 0xedd8d75f:	// SKE
...
void CCutsceneData::init_objects()
{
	...
	uint32* pCIFData = mp_fileLibrary->GetFileData( 0, Crc::ConstCRC("CIF"), true );
	Dbg_Assert( pCIFData );
	create_objects( pCIFData );
	mp_fileLibrary->ClearFile( 0, Crc::ConstCRC("CIF") );
...
{  
	...
	Script::RunScript( Crc::ConstCRC("cutsceneobj_add_components"), pNodeData, pDummyObject );
...
//		Dbg_Message( "munged modelAssetName = %08x", modelAssetName );
		...
		bool do_player_substitution = ( modelName == Crc::ConstCRC("skater") || modelName == Crc::ConstCRC("player") );
		if ( do_player_substitution )
		{
			uint32 refObjectName = 0;
			pNodeData->AddChecksum( Crc::ConstCRC("refObjectName"), refObjectName );
		}
		else
		{
			// if it's not a skater model, then check for correct skeleton/anims
			if ( !mp_fileLibrary->FileExists( modelName, Crc::ConstCRC("SKIN") ) )
			{
...
//				Dbg_MsgAssert( animName == 0, ( "Non-skinned model %08x has anim name", objectName ) );
			...
			if ( is_head_model && is_skater_model )
			{
				...
				{
					Dbg_MsgAssert( mp_fileLibrary->FileExists( modelName + 1, Crc::ConstCRC("SKIN") ), ( "Couldn't find eyeball-less head model" ) );
				...
				Script::CStruct* pInfo = pSkaterProfile->GetInfo();
				int is_male = 0;
				pInfo->GetInteger( Crc::ConstCRC("is_male"), &is_male, Script::ASSERT );
				if ( !is_male )
				{
				...
				pNodeData->AddInteger( Crc::ConstCRC("multicolor"), is_male );
			}
			
			pNodeData->AddChecksum( Crc::ConstCRC("assetName"), modelAssetName );
		}
		...
		// then we don't want to subsitute the hi-res skeleton
		{
			...
			if ( is_head_model && is_skater_model )
			{
				...
				if ( !pHeadStruct )
				{
					pHeadStruct = pCASModelAppearance->GetActualDescStructure( Crc::ConstCRC("skater_f_head") );
				}
				if ( pHeadStruct && pHeadStruct->ContainsFlag( Crc::ConstCRC("NoCutsceneHead") ) )
				{
					Script::RunScript( Crc::ConstCRC("UnhideLoResHeads") );
					delete pNodeData;
					continue;
				}
...
//		Dbg_Message( "animName = %08x", animName );
		...
		if ( skeletonName )
		{
			pNodeData->AddChecksum( Crc::ConstCRC("skeletonName"), skeletonName );

			// this disables the "bone skip list" optimization,
			// which would otherwise make the cutscene object's
			// hands disappear
			pNodeData->AddInteger( Crc::ConstCRC("max_bone_skip_lod"), 0 );
		}
		...
		// details' changes
		//pNodeData->AddChecksum( Crc::ConstCRC("animName"), anim_name );
	...
	for ( int i = 0; i < m_numObjects; i++ )
	{
		...
		if ( pParentObjectInfo )
		{
			...
			if ( pParentObjectInfo->m_doPlayerSubstitution )
			{
				...
				pCASModelComponent->InitModelFromProfile(pCASModelAppearance,false,0,Crc::ConstCRC("replace_cutscene_skater_from_appearance"));
				...
				pHeadStruct = pCASModelAppearance->GetVirtualDescStructure( Crc::ConstCRC("skater_m_head") );
				
				if ( !pHeadStruct )
				{
					pHeadStruct = pCASModelAppearance->GetVirtualDescStructure( Crc::ConstCRC("skater_f_head") );
				}
				...
				if ( pHeadStruct )
				{
					...
					pHeadStruct->GetInteger( Crc::ConstCRC("use_default_hsv"), &use_default_hsv );
					if ( !use_default_hsv )
					{
						int h, s, v;
						if ( pHeadStruct->GetInteger( Crc::ConstCRC("h"), &h, false )
							 && pHeadStruct->GetInteger( Crc::ConstCRC("s"), &s, false )
							 && pHeadStruct->GetInteger( Crc::ConstCRC("v"), &v, false ) )
						{
							Script::CStruct* pTempStruct = new Script::CStruct;
							pTempStruct->AddChecksum( Crc::ConstCRC("part"), 0 );
							pTempStruct->AddChecksum( Crc::ConstCRC("material"), Crc::ConstCRC("cashead_head") );
							pTempStruct->AddInteger( Crc::ConstCRC("pass"), 0 );
...
//		Script::SpawnScript( Crc::ConstCRC("FadeInCutscene"), pStruct );
		m_videoStarted = true;
		
		Script::RunScript( Crc::ConstCRC("CutsceneStartVideo") );
...
//		camMatrix.Ident();
		...
		float fov_in_degrees = Script::GetFloat( Crc::ConstCRC("camera_fov") );
...
*/
	if ( Script::GetInteger( Crc::ConstCRC("debug_cutscenes") ) )
	{
...
//			pTempStruct->AddString( "line1", (const char*)&line1 );
		...
		Script::RunScript( Crc::ConstCRC("draw_cutscene_panel"), pTempStruct );
...
void CCutsceneData::update_extra_cutscene_objects()
{
	...
	Script::CArray* pArray = Script::GetArray( Crc::ConstCRC("CutsceneObjectNames"), Script::NO_ASSERT );
	if ( pArray )
	{
	...
	for ( int i = 0; i < m_numObjects; i++ )
	{
		if ( m_objects[i].mp_animInfo )
		{
			...
			if ( pSkeletonComponent )
			{
				...
				{					
					int bone_index = pSkeleton->GetBoneIndexById( Crc::ConstCRC("bone_jaw") );
...
void CCutsceneData::update_moving_objects()
{
	...
	for ( int i = m_numObjects - 1; i >= 0; i-- )
	{
		...
		if ( pInfo->mp_parentObject )
		{
			...
			Dbg_Assert( pSkeleton );
			Mth::Vector neckPos;
			bool success = pSkeleton->GetBonePosition( Crc::ConstCRC("bone_neck"), &neckPos );
			if ( !success )
			{
			...
			success = pSkeleton->GetBoneMatrix( Crc::ConstCRC("bone_neck"), &bone_matrix );
			if ( !success )
			{
				Dbg_MsgAssert( 0, ( "Couldn't find the neck bone" ) );
			}
...
//		}
		...
		if ( Script::GetInteger( Crc::ConstCRC("debug_cutscenes"), Script::NO_ASSERT ) )
		{
			// set up bounding box
			SBBox theBox;
...
//	bool dont_assert = false;
	...
	pTempParams->GetText( Crc::ConstCRC("name"), &pCutsceneFileName, Script::ASSERT );
	...
	else
	{
		...
		int should_unload_anims = !mp_cutsceneStruct->ContainsFlag( Crc::ConstCRC("dont_unload_anims") );
		int should_reload_anims = !mp_cutsceneStruct->ContainsFlag( Crc::ConstCRC("dont_unload_anims") );

		if ( pParams->ContainsFlag( Crc::ConstCRC("from_cutscene_menu") ) )
		{
		...
		mp_cutsceneStruct->AddInteger( Crc::ConstCRC("unload_anims"), should_unload_anims );
		mp_cutsceneStruct->AddInteger( Crc::ConstCRC("reload_anims"), should_reload_anims );

		// by default, unload the goals
		mp_cutsceneStruct->AddInteger( Crc::ConstCRC("unload_goals"), 1 );
		mp_cutsceneStruct->AddInteger( Crc::ConstCRC("reload_goals"), 1 );
		...
		Script::RunScript( Crc::ConstCRC("PreCutscene"), mp_cutsceneStruct );
...
void CCutsceneDetails::Cleanup()
{
	...
	Script::RunScript( Crc::ConstCRC("kill_cutscene_panel") );
	...
	Script::RunScript( Crc::ConstCRC("kill_cutscene_camera_hud") );
...
{
	// hides the goal peds
	Script::RunScript( Crc::ConstCRC("cutscene_hide_objects") );

	// first count the number of objects, so we can allocate the proper number of hide flags
	int numObjects = 0;
	Obj::CBaseComponent *p_component = Obj::CCompositeObjectManager::Instance()->GetFirstComponentByType( Crc::ConstCRC("model") );
	while( p_component )
	{
	...
	p_component = Obj::CCompositeObjectManager::Instance()->GetFirstComponentByType( Crc::ConstCRC("model") );
	while( p_component )
	{
		Obj::CModelComponent* pModelComponent = ((Obj::CModelComponent*)p_component);
		Nx::CModel* pModel = pModelComponent->GetModel();
...
void CCutsceneDetails::unhide_moving_objects()
{
	...
	Script::RunScript( Crc::ConstCRC("cutscene_unhide_objects") );
}
