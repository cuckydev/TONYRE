...
void CCar::update_wheels()
{
	...
	if ( Script::GetInt( Crc::ConstCRC("car_debug"), Script::NO_ASSERT ) )
	{
		GetCarPhysicsComponent()->m_minCarRot = Script::GetFloat( Crc::ConstCRC("accelCarRot"), Script::ASSERT );
		GetCarPhysicsComponent()->m_maxCarRot = Script::GetFloat( Crc::ConstCRC("decelCarRot"), Script::ASSERT );
		GetCarPhysicsComponent()->m_stepCarRot = Script::GetFloat( Crc::ConstCRC("speedCarRot"), Script::ASSERT );
		GetCarPhysicsComponent()->m_accelToCarRotFactor = Script::GetFloat( Crc::ConstCRC("accelCarRotFactor"), Script::ASSERT );
...
void CCar::InitCar( CGeneralManager* p_obj_man, Script::CStruct* pNodeData )
{
	...
	uint32 skeletonName = Crc::ConstCRC("car");
	bool use_skeletal_cars = false;

	if ( pNodeData->GetChecksum( Crc::ConstCRC("skeletonName"), &skeletonName, false ) )
	{
...
//		m_model_restoration_info.mSkeletonName=0;
	...
	{		
		...
		pNodeData->GetText( Crc::ConstCRC("model"), &p_model_name, true );
	...
	else
	{
		...
		{
			uint32 name = 0;
			pNodeData->GetChecksum( Crc::ConstCRC("name"), &name, Script::ASSERT );
	...
	if ( !pNodeData->ContainsFlag( Crc::ConstCRC("NoCollision") ) )
	{
	...
	if ( !pNodeData->ContainsFlag( Crc::ConstCRC("NoRail") ) )
	{
	...
	if ( !pNodeData->ContainsFlag( Crc::ConstCRC("NoSkitch") ) )
	{
