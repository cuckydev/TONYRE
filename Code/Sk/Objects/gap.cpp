...
void CGapChecklist::got_gap(Script::CStruct *pScriptStruct, const uint8 *pPC)
{
	...
	const char *p_name="";
	pScriptStruct->GetText(Crc::ConstCRC("text"),&p_name);
...
{	
	...
	SkateScript::ScanNodeScripts(Crc::ConstCRC("TriggerScript"), Crc::ConstCRC("EndGap"), Gap_GotGap);	
	...
	while (pOther)						 		
	{
		if(pOther->GotGapPending())
		{
			...
			if ( pOther->Got() == 0 )//&& !ran_got_gap_for_first_time )
			{
				...
				pScriptParams->AddString( Crc::ConstCRC("gap_text"), pOther->GetText() );
				Script::RunScript( Crc::ConstCRC("got_gap_for_first_time"), pScriptParams );
...
bool CGapChecklist::ScriptAddGapsToMenu( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	while (pGap)						 		
	{
		...
		p_gap_params->AddVector(Crc::ConstCRC("skater_start_pos"),v[X],v[Y],v[Z]);
		v=pGap->GetSkaterStartDir();
		p_gap_params->AddVector(Crc::ConstCRC("skater_start_dir"),v[X],v[Y],v[Z]);
		
		p_gap_params->AddChecksum(Crc::ConstCRC("choose_script"),choose_script);
...
bool CGapChecklist::ScriptCreateGapList( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	pScript->GetParams()->AddInteger(Crc::ConstCRC("array_size"),num_gaps);
	
	if (!num_gaps)
	{
		pScript->GetParams()->AddInteger(Crc::ConstCRC("num_gaps_got"),0);
		return false;
	}
	...
	pScript->GetParams()->AddArrayPointer(Crc::ConstCRC("GapList"),p_gap_list);
	pScript->GetParams()->AddInteger(Crc::ConstCRC("num_gaps_got"),num_gaps_got);
	return true;
}
