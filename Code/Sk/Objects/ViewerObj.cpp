...
void CViewerObject::LoadModel( Script::CStruct* pNodeData )
{
	...
	Script::RunScript( Crc::ConstCRC("viewerobj_add_components"), pNodeData, this );

	Finalize();

	Script::RunScript( Crc::ConstCRC("viewerobj_init_model"), pNodeData, this );
...
{       
	...
	int vAUTO_REPEAT_TIME = Script::GetInteger( Crc::ConstCRC("ViewerAutoRepeatTime"), Script::NO_ASSERT );

	// this is how fast it auto repeats
	int vAUTO_REPEAT_SPEED = Script::GetInteger( Crc::ConstCRC("ViewerAutoRepeatSpeed"), Script::NO_ASSERT );
