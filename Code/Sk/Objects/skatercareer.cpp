...
void CSkaterCareer::WriteIntoStructure(Script::CScriptStructure *pIn)
{
	...
	for (unsigned int i = 0; i<vMAX_CAREER_LEVELS; ++i)
	{
		...
		p_gap_struct->AddInteger(Crc::ConstCRC("level"),i);
		if (!p_checklist->IsValid())
		{
			p_gap_struct->AddInteger(Crc::ConstCRC("valid"),0);
		}
		else
		{
			p_gap_struct->AddInteger(Crc::ConstCRC("valid"),1);
			...
			for (int gap=0; gap<num_gaps; ++gap)
			{
				...
				p_struct->AddString(Crc::ConstCRC("GapName"),p_checklist->GetGapText(gap));
				p_struct->AddInteger(Crc::ConstCRC("GapCount"),p_checklist->GetGapCount(gap));
				p_struct->AddInteger(Crc::ConstCRC("GapScore"),p_checklist->GetGapScore(gap));
				...
				p_struct->AddVector(Crc::ConstCRC("SkaterStartPos"),skater_start_pos[X],skater_start_pos[Y],skater_start_pos[Z]);
				p_struct->AddVector(Crc::ConstCRC("SkaterStartDir"),skater_start_dir[X],skater_start_dir[Y],skater_start_dir[Z]);
				
				p_gaps_array->SetStructure(gap,p_struct);
			}
						
			p_gap_struct->AddArrayPointer(Crc::ConstCRC("Gaps"),p_gaps_array);
		}
	...
	pTemp->AddChecksum(Crc::ConstCRC("LastLevelLoadScript"),p_skate_mod->m_cur_level);
	pTemp->AddChecksum(Crc::ConstCRC("LastGameMode"),p_skate_mod->GetGameMode()->GetNameChecksum());
		...
		for (unsigned int i = 0; i<vMAX_CAREER_LEVELS; ++i)
		{
			...
			if (valid)
			{
				...
				for (int gap = 0; gap<num_gaps; gap++)
				{
					...
					p_struct->GetInteger(Crc::ConstCRC("GapCount"),&count);
					p_struct->GetInteger(Crc::ConstCRC("GapScore"),&score);
					...
					if (count)
					{
						Mth::Vector skater_start_pos;
						Mth::Vector skater_start_dir;
						p_struct->GetVector(Crc::ConstCRC("SkaterStartPos"),&skater_start_pos);
						p_struct->GetVector(Crc::ConstCRC("SkaterStartDir"),&skater_start_dir);
