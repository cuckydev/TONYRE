...
//	mp_first_node = pRailNode;
	...
	bool climbing = class_checksum == Crc::ConstCRC("ClimbingNode");
	...
	if (climbing)
	{
		...
		pRailNode->m_flags.Set(LADDER, type_checksum == Crc::ConstCRC("Ladder"));
	}
	
	pRailNode->m_flags.Set(NO_HANG_WITH_RIGHT_ALONG_RAIL, p_node_struct->ContainsFlag(Crc::ConstCRC("HangLeft")));
	pRailNode->m_flags.Set(NO_HANG_WITH_LEFT_ALONG_RAIL, p_node_struct->ContainsFlag(Crc::ConstCRC("HangRight")));
	pRailNode->m_flags.Set(NO_CLIMBING, p_node_struct->ContainsFlag(Crc::ConstCRC("NoClimbing")));
...
bool CRailManager::CheckForAirGrabLadderRail ( const Mth::Vector& start_pos, const Mth::Vector& end_pos, CWalkComponent* p_walk_component, SLadderRailData& rail_data, const CRailNode** pp_rail_node )
{
	*pp_rail_node = nullptr;
	...
	float snap_dist_sqr = Script::GetFloat(Crc::ConstCRC("Climb_Max_Snap"));
...
bool CRailManager::StickToRail(const Mth::Vector &pos1, const Mth::Vector &pos2, Mth::Vector *p_point, CRailNode **pp_rail_node, const CRailNode *p_ignore_node, float min_dot, int side)
{
	...
	Mth::Vector bb_min, bb_max;
	Rail_ComputeBB(movement.m_start, movement.m_end, bb_min, bb_max);
	float snap_dist = Script::GetFloat(Crc::ConstCRC("Rail_Max_Snap"));
...
void	CRailManager::DebugRender(Mth::Matrix *p_transform)
{
	...
	bool	draw_arrows = Script::GetInt( Crc::ConstCRC("rail_arrows"), false );
...
void	CRailManager::AddRailsFromNodeArray(Script::CArray * p_nodearray)
{
	...
	for (i = 0; i < p_nodearray->GetSize(); ++i)
	{
		...
		if ( !skate_mod->ShouldBeAbsentNode( p_node_struct ) )
		{
			...
			if (class_checksum == Crc::ConstCRC("railnode") || class_checksum == Crc::ConstCRC("ClimbingNode"))
			{
				m_num_nodes++;										  
			}
			
			// if (class_checksum == Crc::ConstCRC("climbnode"))
			// {
