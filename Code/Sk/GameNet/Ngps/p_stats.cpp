...
bool	StatsMan::ScriptReportStats(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	final = pParams->ContainsFlag( Crc::ConstCRC("final"));
	stats_man = gamenet_man->mpStatsMan;
...
bool	StatsMan::ScriptGetRank(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	pScript->GetParams()->AddInteger( Crc::ConstCRC("rank"), rank ); 
...
bool	StatsMan::ScriptFillStatsArrays(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	pParams->GetChecksum( Crc::ConstCRC("array"), &array, true );
...
bool	StatsMan::ScriptRetrieveTopStats(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	file_index = 0;
	if( pParams->GetInteger( Crc::ConstCRC("file"), &file_index ))
	{
	...
	context.m_file_not_found_script = Crc::ConstCRC("StatsRetrievalFailed");
	context.m_transfer_failed_script = Crc::ConstCRC("StatsRetrievalFailed");
	context.m_update_progress_script = Crc::ConstCRC("do_nothing");
	context.m_max_size = vMAX_STATS_FILE_SIZE;

	if( stats_man->m_cur_file_index == ( vNUM_RATINGS_FILES - 1 ))
	{
		context.m_transfer_complete_script = Crc::ConstCRC("stats_retrieval_complete");
	}
	else
	{
		context.m_transfer_complete_script = Crc::ConstCRC("download_more_stats");
	}
...
void	Stats::FillMenu( void )
{
	...
	for( p_stats = sh.FirstItem( stat_list ); p_stats; p_stats = sh.NextItem())
	{
        ...
        Script::RunScript( Crc::ConstCRC("add_stat_personal_menu_item"), p_entry );
...
void	StatsKeeper::FillSectionedMenu( void )
{
	...
	for( level = sh.FirstItem( m_Levels ); level; level = sh.NextItem())
	{
		...
		if( level->m_Players.CountItems() > 0)
		{
			...
			Script::RunScript( Crc::ConstCRC("add_stat_header_menu_item"), p_level_entry );
		...
		for( player = player_sh.FirstItem( level->m_Players ); player; player = player_sh.NextItem())
		{
			...
			Script::RunScript( Crc::ConstCRC("add_stat_score_menu_item"), p_entry );
	...
	p_header->AddString( "text", Script::GetLocalString( Crc::ConstCRC("category_all_levels")));
	Script::RunScript( Crc::ConstCRC("add_stat_header_menu_item"), p_header );
	delete p_header;
}
