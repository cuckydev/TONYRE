	...
	if( result == GHTTPSuccess )
	{
		...
		switch( content_man->m_vault_type ) 
		{
			case 0x5be5569f:	// parks
				type = Crc::ConstCRC("park");
				break;
			case 0x7364ea1:		// skaters
				type = Crc::ConstCRC("optionsandpros");
				break;
			case 0x38dbe1d0:	// goals
				type = Crc::ConstCRC("createdgoals");
				break;
			case 0x1e26fd3e:	// tricks
				type = Crc::ConstCRC("cat");
				break;
			default:
				Dbg_Assert( 0 );
				break;
													...
													)
{
	...
	if( result == GHTTPSuccess )
	{
		...
		Script::RunScript( Crc::ConstCRC("refresh_skater_model") );
	}
	else
	{
		content_man->m_result = vCONTENT_RESULT_FAILURE;
				...
				if( line )
				{
					do
					{
						...
						*colon = '\0';
						...
						if( content_man.m_vault_type == Crc::ConstCRC("parks") )
						{
							token = colon + 1;
							colon = strstr( token, ":" );
							Dbg_Assert( colon );
						...
						if( content_man.m_vault_type == Crc::ConstCRC("tricks") )
						{
							token = colon + 1;
							colon = strstr( token, ":" );
							Dbg_Assert( colon );
						...
						// Read the content sex, if we're looking at skaters
						if( content_man.m_vault_type == Crc::ConstCRC("goals") )
						{
							token = colon + 1;
							colon = strstr( token, ":" );
							Dbg_Assert( colon );
...
void		ContentMan::GetDirListing( char* path )
{
	...
	m_transfer_failed_script = Crc::ConstCRC("LaunchTransferFailedDialog");
	m_file_not_found_script = Crc::ConstCRC("LaunchGeneralFileNotFoundDialog");
	m_update_progress_script = Crc::ConstCRC("update_transfer_progress");
...
void	ContentMan::DownloadFace( char* filename )
{
	...
	m_transfer_complete_script = Crc::ConstCRC("LaunchFaceDownloadCompleteDialog");
	m_transfer_failed_script = Crc::ConstCRC("LaunchTransferFailedDialog");
	m_file_not_found_script = Crc::ConstCRC("LaunchFileNotFoundDialog");
	m_update_progress_script = Crc::ConstCRC("update_transfer_progress");
...
bool	ContentMan::ScriptDownloadFile(Script::CScriptStructure *p_item_params, Script::CScript *pScript)
{
	...
	context.m_transfer_failed_script = Crc::ConstCRC("LaunchTransferFailedDialog");
	context.m_file_not_found_script = Crc::ConstCRC("LaunchGeneralFileNotFoundDialog");
	context.m_update_progress_script = Crc::ConstCRC("update_transfer_progress");
...
bool	ContentMan::ScriptUploadFile(Script::CScriptStructure *p_item_params, Script::CScript *pScript)
{
	...
	content_man->m_percent_complete = 0;
	content_man->m_last_comm_time = Tmr::GetTime();
	content_man->m_transfer_failed_script = Crc::ConstCRC("LaunchTransferFailedDialog");
