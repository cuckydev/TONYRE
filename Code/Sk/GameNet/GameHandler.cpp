		...
		if( !( player->m_flags.TestMask( PlayerInfo::mHAS_PLAYER_INFO )))
		{
			...
			else
			{
				...
				if(( king = gamenet_man->GetKingOfTheHill()) != nullptr)
				{
					...
					Dbg_MsgAssert( skate_mod->GetGameMode()->GetNameChecksum() == Crc::ConstCRC("netking")  ||
...
{   
	...
	else if(( skate_mod->GetGameMode()->GetNameChecksum() != Crc::ConstCRC("netlobby") ) &&
...
int Manager::s_handle_change_level( Net::MsgHandlerContext* context )
{
	...
	if( pMsg->m_ShowWarning )
	{
		...
		{                                    
			...
			if( gamenet_man->m_level_id == Crc::ConstCRC("Load_Sk5Ed_gameplay"))
			{
...
{   
...

	{
		...
		pTempStructure->AddComponent( Crc::ConstCRC("fireball_level"), ESYMBOLTYPE_INTEGER, msg->m_FireballLevel );
		pTempStructure->AddComponent( Crc::ConstCRC("stop_at_zero"), ESYMBOLTYPE_INTEGER, msg->m_StopAtZero );
...
int	Manager::s_handle_team_change( Net::MsgHandlerContext* context )
{
	...
	if( player )
	{
		...
		if( all_same_team && ( num_other_players > 0 ))
		{
			...
			{
				Script::RunScript( Crc::ConstCRC("warn_all_same_team"));
			}
		}
	}
			...
			else
			{
					...
					( local_player->m_Team == player->m_Team ))
				{
					...
					if( num_goals > num_beaten )
					{
						...
						pParams = new Script::CStruct;
						pParams->AddInteger( Crc::ConstCRC("NumGoalsLeft"), num_goals - num_beaten );
						pParams->AddString( Crc::ConstCRC("PlayerName"), player->m_Name );
						pParams->AddString( Crc::ConstCRC("GoalText"), p_view_goals_text );
						Script::RunScript( Crc::ConstCRC("goal_attack_completed_goal_other_same_team"), pParams );
						delete pParams;
					}
...
int Manager::s_handle_started_goal_relay( Net::MsgHandlerContext* context )
{
	...
	if( pGoal )
	{
		...
		if (gamenet_man->GetLocalPlayer()->m_Team == player->m_Team)
		{
			...
			pParams->AddString( Crc::ConstCRC("PlayerName"), player->m_Name );
			pParams->AddString( Crc::ConstCRC("GoalText"), p_view_goals_text );
			Script::RunScript( Crc::ConstCRC("goal_attack_started_goal_other_same_team"), pParams );
			delete pParams;
		}
	}
...
int	Manager::s_handle_cheat_checksum_response( Net::MsgHandlerContext* context )
{
	...
	if(( msg->m_ServerChecksum ^ 0xDEADFACE ) != msg->m_ClientChecksum )
	{
		...
		if( gamenet_man->HasCheatingOccurred() == false )
		{
			...
			p_params = new Script::CStruct;
			p_params->AddString( Crc::ConstCRC("String0"), player->m_Name );
			Script::RunScript( Crc::ConstCRC("notify_client_cheating"), p_params );
			delete p_params;
