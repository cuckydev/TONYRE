	...
	if( num_chars > 0 )
	{
		...
		else if( makes[0] == SIO::vKB_F2 )
		{
			if(( Tmr::GetTime() - s_last_taunt_time ) > vTAUNT_INTERVAL )
			{
				...
				pParams = new Script::CStruct;
				pParams->AddChecksum( Crc::ConstCRC("string_id"), Crc::ConstCRC("your_daddy_string"));
				Script::RunScript( "SendTauntMessage", pParams );
				delete pParams;
		...
		else if( makes[0] == SIO::vKB_F3 )
		{
			if(( Tmr::GetTime() - s_last_taunt_time ) > vTAUNT_INTERVAL )
			{
				...
				pParams = new Script::CStruct;
				pParams->AddChecksum( Crc::ConstCRC("string_id"), Crc::ConstCRC("get_some_string"));
				Script::RunScript( "SendTauntMessage", pParams );
				delete pParams;
		...
		else if( makes[0] == SIO::vKB_F4 )
		{
			if(( Tmr::GetTime() - s_last_taunt_time ) > vTAUNT_INTERVAL )
			{
				...
				pParams = new Script::CStruct;
				pParams->AddChecksum( Crc::ConstCRC("string_id"), Crc::ConstCRC("no_way_string"));
				Script::RunScript( "SendTauntMessage", pParams );
				delete pParams;
...
void	Manager::s_ctf_logic_code( const Tsk::Task< Manager >& task )
{
	...
	for( player = gamenet_man->FirstPlayerInfo( sh ); player; player = gamenet_man->NextPlayerInfo( sh ))
	{
		if( player->HasCTFFlag())
		{
			...
			pSkeleton->GetBoneMatrix( Crc::ConstCRC("Bone_Head"), &flag_matrix );
	...
	if( man.NumPartiallyLoadedPlayers() > 0 )
	{
		if( man.OnServer())
		{
			if( showing_wait_screen == false )
			{
				...
				Front::CScreenElementPtr p_elem = pManager->GetElement( Crc::ConstCRC("current_menu_anchor"), Front::CScreenElementManager::DONT_ASSERT );
				if( !p_elem )
				{
			...
			case Net::vMODEM_STATE_ERROR:
			{
				...
				switch( net_man->GetModemError())
				{
					...
					case SNTC_ERR_CONNECT:
					{
						...
						config_type = prefs->GetPreferenceChecksum( Crc::ConstCRC("config_type"), 
																	Crc::ConstCRC("checksum") );
						if( config_type == Crc::ConstCRC("config_manual"))
						{
...
void	Manager::s_timeout_connections_code( const Tsk::Task< Manager >& task )
{
		...
		(( Tmr::GetTime() - s_last_link_check ) > vLINK_CHECK_FREQUENCY ))
	{
		...
		else
		{
			...
			{
				Front::CScreenElementManager* pManager = Front::CScreenElementManager::Instance();
				Front::CScreenElementPtr p_elem = pManager->GetElement( Crc::ConstCRC("dialog_box_anchor"), Front::CScreenElementManager::DONT_ASSERT );
				if( !p_elem )
				{
...
#ifdef __PLAT_XBOX__
						Front::CScreenElementManager* pManager = Front::CScreenElementManager::Instance();
						Front::CScreenElementPtr p_elem = pManager->GetElement( Crc::ConstCRC("quit_dialog_anchor"), Front::CScreenElementManager::DONT_ASSERT );
						if( !p_elem )
#endif
						{
...
char*				Manager::GetLevelName( bool get_created_park_name )
{
	...
	{   
		...
		if( GetNetworkLevelId() == checksum )
		{
			...
			if( get_created_park_name && ( checksum == Crc::ConstCRC("Load_Sk5Ed_gameplay")))
			{
...
bool Manager::ShouldStopAtZero( void )
{
	...
	if( m_gamemode_id == Script::GenerateCRC( "netctf" ))
	{
		...
		if( mode != Crc::ConstCRC("timed_ctf"))
		{
			return true;
		}
	}
	...
	if( should_stop == Crc::ConstCRC("boolean_true"))
	{
		return true;
	}
...
void Manager::StartNetworkGame( void )
{
	...
	if( ( m_gamemode_id == Crc::ConstCRC("netgoalattack")) ||
		( m_gamemode_id == Crc::ConstCRC("king")) ||
		( m_gamemode_id == Crc::ConstCRC("netking")) ||
		( m_gamemode_id == Crc::ConstCRC("netscorechallenge")) ||
		( m_gamemode_id == Crc::ConstCRC("scorechallenge")) ||
		( m_gamemode_id == Crc::ConstCRC("firefight")) ||
		( m_gamemode_id == Crc::ConstCRC("netfirefight")))
	{
...
int		Manager::GetFireballLevelFromPreferences( void )
{
	...
	pStructure = m_network_preferences.GetPreference( Crc::ConstCRC("fireball_difficulty") );
	pStructure->GetChecksum( "Checksum", &checksum, true );
	if( checksum == Script::GenerateCRC( "num_1" ))
	{
		return 1;
...
bool	Manager::ShouldSendScoreUpdates( void )
{
	...
	if( ( pGameMode->ShouldAccumulateScore() == false ) &&
		...
		( pGameMode->GetNameChecksum() != Crc::ConstCRC("netfirefight")) &&
		( pGameMode->GetNameChecksum() != Crc::ConstCRC("netgoalattack")) &&
		( pGameMode->GetNameChecksum() != Crc::ConstCRC("netctf")))
	{
		return false;
	}
	
	if( pGameMode->EndConditionsMet() && ( pGameMode->GetNameChecksum() != Crc::ConstCRC("netfirefight")))
	{
		return false;
	}
...
bool Manager::ScriptFillPlayerListMenu( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	for( player = gamenet_man->FirstPlayerInfo( sh, true ); player; player = gamenet_man->NextPlayerInfo( sh, true ))
	{
		...
		*/
		...
		p_item_params->AddChecksum( "centered", Crc::ConstCRC("centered") );
