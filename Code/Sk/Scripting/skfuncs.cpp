...
static void s_preload_ped_part( uint32 partChecksum )
{
	...
	for ( uint32 i = 0; i < pArray->GetSize(); i++ )
	{
		...
		int allow_random = 0;
		pStruct->GetInteger( Crc::ConstCRC("allow_random"), &allow_random, Script::NO_ASSERT );
			
		if ( allow_random )
		{
			const char* pMeshName;
			if ( pStruct->GetText( Crc::ConstCRC("mesh"), &pMeshName, false ) )
			{
...
bool ScriptGetGoalsCompleted(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	pScript->GetParams()->AddComponent(Crc::ConstCRC("GoalsCompleted"),ESYMBOLTYPE_STRING,pBuf);
	pScript->GetParams()->AddComponent(Crc::ConstCRC("BestMedal"),ESYMBOLTYPE_STRING,"---");
...
bool ScriptGetNextLevelRequirements(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	Script::CArray *pUnlockRequirementsArray=Script::GetArray(Crc::ConstCRC("UnlockRequirements"));
	for (uint32 i=0; i<pUnlockRequirementsArray->GetSize(); ++i)
	{
		...
		if (pUnlockRequirements->ContainsFlag(Crc::ConstCRC("Goals")) || pUnlockRequirements->ContainsFlag(Crc::ConstCRC("Goal")))
		{
			// Need a certain number of goals.
			pUnlockRequirements->GetInteger(NONAME,&NumGoalsRequired);
		}	
				
		if (pUnlockRequirements->ContainsFlag(Crc::ConstCRC("Medals")) || pUnlockRequirements->ContainsFlag(Crc::ConstCRC("Medal")))
		{
		...
		if (TotalGoals<NumGoalsRequired)
		{
			sprintf(pBuf,Script::GetLocalString(Crc::ConstCRC("cfuncs_str_goalsfornextlevel")),NumGoalsRequired-TotalGoals);
			break;
		}
		if (TotalMedals<NumMedalsRequired)
		{
			sprintf(pBuf,Script::GetLocalString(Crc::ConstCRC("cfuncs_str_medalsfornextlevel")),NumMedalsRequired-TotalMedals);
			break;
		}
		#endif		
	}		
	
	pScript->GetParams()->AddComponent(Crc::ConstCRC("NextLevelRequirements"),ESYMBOLTYPE_STRING,pBuf);
	return true;
}
...
bool ScriptRememberTemporaryAppearance(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	if ( pParams->GetStructure( Crc::ConstCRC("appearance_structure"), &pAppearanceStruct, Script::NO_ASSERT ) )
	{
	...
	if ( pParams->ContainsFlag( Crc::ConstCRC("NoFaceTexture") ) )
	{
...
bool ScriptPreloadModels( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	for ( uint32 i = 0; i < pNodeArray->GetSize(); i++ )
	{
		...
		else 
		{
			...
			pNode->GetChecksum( Crc::ConstCRC("Class"), &ClassChecksum );

			uint32 TypeChecksum = 0;
			pNode->GetChecksum(Crc::ConstCRC("Type"), &TypeChecksum );
					...
					case 0xef59c100: // GameObject
						{
							...
							if ( pNode->GetText( 0x286a8d26, &pModelName ) ) // checksum 'model'
							{
								...
								{		   
									...
									bool forceTexDictLookup = pNode->ContainsFlag( Crc::ConstCRC("AllowReplaceTex") );
...
//				Dbg_Message("Preloading single-skinned ped model %s", fullModelName.getString() );
			...
			uint32 profileName;
			if ( pNode->GetChecksum( Crc::ConstCRC("profile"), &profileName, false ) )
			{
...
bool ScriptPreselectRandomPedestrians( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	size_t fullPartArraySize = pFullPartArray->GetSize();
	for (size_t i = 0; i < fullPartArraySize; i++ )
	{
		...
		pStruct->RemoveComponent( Crc::ConstCRC("allowed_to_pick") );

		// they all default to not be able to be randomized
		pStruct->AddInteger( Crc::ConstCRC("allow_random"), 0 );

		pStruct->AddInteger( Crc::ConstCRC("already_selected"), 0 );
		
		pStruct->AddInteger( Crc::ConstCRC("random_index"), -1 );
	}
	...
	for (size_t i = 0; i < fullPartArraySize; i++ )
	{
		...
		if ( pStruct->ContainsComponentNamed( Crc::ConstCRC("no_random") ) )
		{
			allowed_to_pick = false;
		
			if ( pStruct->GetStructure( Crc::ConstCRC("level_specific"), &pLevelSpecificStruct, false ) )
			{
		...
		else if ( pStruct->GetStructure( Crc::ConstCRC("level_specific"), &pLevelSpecificStruct, false ) )
		{
			Dbg_MsgAssert( !pStruct->ContainsComponentNamed(Crc::ConstCRC("exclude_from_levels")), ( "level_specific and exclude_from_levels are mutually exclusive" ) )

			// check to see if we're in this level...
			allowed_to_pick = pLevelSpecificStruct->ContainsComponentNamed( levelName );
		}
		else if ( pStruct->GetStructure( Crc::ConstCRC("exclude_from_levels"), &pLevelSpecificStruct, false ) )
		{
			Dbg_MsgAssert( !pStruct->ContainsComponentNamed(Crc::ConstCRC("level_specific")), ( "level_specific and exclude_from_levels are mutually exclusive" ) )
		...
		{
			allowableCount++;
			pStruct->AddInteger( Crc::ConstCRC("allowed_to_pick"), 1 );
		}
		else
		{
			// clear the already picked flag
			pStruct->AddInteger( Crc::ConstCRC("allowed_to_pick"), 0 );
...
//				Script::PrintContents(pStruct);
	...
	for (size_t i = 0; i < pNodeArray->GetSize(); i++ )
	{
		Script::CStruct* pNode = pNodeArray->GetStructure( i );
		Dbg_MsgAssert( pNode, ( "nullptr pNode" ) );
		...
		if ( ClassChecksum == Script::GenerateCRC("pedestrian") )
		{
			...
			if ( pNode->GetChecksum( Crc::ConstCRC("profile"), &profileName, false ) )
			{
				...
				if ( pProfileStruct )
				{
					...
					{
						uint32 descID;
						if ( pVirtualStruct->GetChecksum( Crc::ConstCRC("desc_id"), &descID, false ) )
						{
							Script::CStruct* pActualStruct = Cas::GetOptionStructure( fullPartChecksum, descID, false );
							if ( pActualStruct && !pActualStruct->ContainsComponentNamed( Crc::ConstCRC("no_random") ) )
							{
								if ( Script::GetInt( "cas_artist", false ) )
								{
//									Dbg_Message( "Allowing to pick %s", Script::FindChecksumName( descID ) );
								}
								pActualStruct->AddInteger( Crc::ConstCRC("allow_random"), 1 );
								pActualStruct->AddInteger( Crc::ConstCRC("allowed_to_pick"), 1 );

								// these get lowest-priority when randomizing...
								pActualStruct->AddInteger( Crc::ConstCRC("already_selected"), 1 );
							}
						}
					}
				}
	...
	pParams->GetInteger( Crc::ConstCRC("num"), &numToSelect, Script::NO_ASSERT );
	...
	if ( skinToneCount )
	{
		...
		pStruct->AddInteger( Crc::ConstCRC("allow_random"), 1 );
		pStruct->AddInteger( Crc::ConstCRC("allowed_to_pick"), 0 );
		pStruct->AddInteger( Crc::ConstCRC("random_index"), numToSelect-1 );
		numToSelect--;
	}
	else
	{
		...
		Cas::BuildRandomSetList( fullPartChecksum, Crc::ConstCRC("light"), pSkinTonePicks, &skinToneCount );
		...
		Cas::BuildRandomSetList( fullPartChecksum, Crc::ConstCRC("tan"), pSkinTonePicks, &skinToneCount );
		...
		Cas::BuildRandomSetList( fullPartChecksum, Crc::ConstCRC("dark"), pSkinTonePicks, &skinToneCount );
	...
	for ( int i = 0; i < numToSelect; i++ )
	{
		...
		while ( !allowed_to_pick )
		{
			...
			if ( allowed_to_pick )
			{
				pStruct->AddInteger( Crc::ConstCRC("allow_random"), 1 );
				pStruct->AddInteger( Crc::ConstCRC("allowed_to_pick"), 0 );
				pStruct->AddInteger( Crc::ConstCRC("random_index"), i );
			}
		}
	}
	//-------------------------------------------------------------
	...
	for ( uint32 i = 0; i < fullPartArraySize; i++ )
	{
		...
		int allow_random = 0;
		pStruct->GetInteger( Crc::ConstCRC("allow_random"), &allow_random, Script::ASSERT );

		uint32 descID;
		pStruct->GetChecksum( Crc::ConstCRC("desc_id"), &descID, Script::ASSERT );
...
//	Tmr::Time baseTime = Tmr::ElapsedTime(0);
	...
	while( p_component )
	{
		...
		if ( p_component->GetObj()->GetType() == SKATE_TYPE_CAR )
		{
			...
			{                
				num_to_rebuild++;
				p_component->CallMemberFunction( Crc::ConstCRC("Obj_ReplaceTexture"), pParams, pScript );
			}
		}
...
//	Tmr::Time baseTime = Tmr::ElapsedTime(0);
	...
	for ( uint32 i = 0; i < pMasterSkaterArray->GetSize(); i++ )
	{
		...
		uint32 skaterName;
		pOriginalInfo->GetChecksum( Crc::ConstCRC("name"), &skaterName, Script::ASSERT );
		...
		for ( uint32 j = 0; j < pStatNameArray->GetSize(); j++ )
		{
			...
			uint32 statName;
			pStatInfo->GetChecksum( Crc::ConstCRC("name"), &statName, Script::ASSERT );
		...
		for ( uint32 j = 0; j < numProfiles; j++ )
		{
			...
			uint32 test;
			pInfo->GetChecksum( Crc::ConstCRC("name"), &test, Script::ASSERT );
			if ( test == skaterName )
			{
...
//	Tmr::Time baseTime = Tmr::ElapsedTime(0);
	...
	pParams->GetChecksum( Crc::ConstCRC("name"), &resetThisSkaterName, Script::ASSERT );
	...
	for ( uint32 i = 0; i < pMasterSkaterArray->GetSize(); i++ )
	{
		...
		if ( resetThisSkaterName == skaterName )
		{
			...
			uint32 partial;
			if ( pParams->GetChecksum( Crc::ConstCRC("partial"), &partial, Script::NO_ASSERT ) )
			{
...
//	Tmr::Time baseTime = Tmr::ElapsedTime(0);
	...
	for ( int i = 0; i < numProfiles; i++ )
	{
		...
		uint32 skaterName;
		pInfo->GetChecksum( Crc::ConstCRC("name"), &skaterName, Script::ASSERT );

		Script::CStruct* pTempStructure = new Script::CStruct;
		pTempStructure->AppendStructure( pSubParams );
		pTempStructure->AddChecksum( Crc::ConstCRC("name"), skaterName );
...
bool ScriptResetSkaters(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	Mdl::Skate::Instance()->ResetSkaters(node, pParams->ContainsFlag(Crc::ConstCRC("RestartWalking")));
	return true;
}
...
bool ScriptCurrentScorePotGreaterThan( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	scale = 1;
	score = s_get_score_from_params( pParams, pScript );
	pParams->GetInteger( Crc::ConstCRC("point_scale"), &scale );
	score *= scale;
...
bool ScriptSkaterGetScoreInfo( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	pScript->GetParams()->AddInteger(Crc::ConstCRC("TotalScore"),p_score->GetTotalScore());
	pScript->GetParams()->AddInteger(Crc::ConstCRC("LastScoreLanded"),p_score->GetLastScoreLanded());
	return true;
}
...
bool ScriptPlaySkaterCamAnim( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	{
		pParams->GetInteger( Crc::ConstCRC("skater"), &skaterIndex );
	...
	if ( pSkater )
	{
		if ( pSkater->IsLocalClient() )
		{
			...
			else
			{
				...
				Script::RunScript( Crc::ConstCRC("disable_skater_scaling") );

				// start the movie with the default params...
				Mdl::Skate * skate_mod = Mdl::Skate::Instance();
				skate_mod->GetMovieManager()->AddMovieToQueue( movieName, pParams, Crc::ConstCRC("skatercamanim") );
			}
		}
		else
		{
...
bool ScriptGetCurrentSkaterCamAnimName( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	if ( movieName != 0 )
	{
		pScript->GetParams()->AddChecksum( Crc::ConstCRC("name"), movieName );
		return true;
	}

	pScript->GetParams()->AddString( Crc::ConstCRC("name"), "none" );
	return false;
}
...
bool ScriptKillSkaterCamAnim( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	else if( pParams->GetInteger( Crc::ConstCRC("skater"), &skaterIndex ))
	{
		pSkater = Mdl::Skate::Instance()->GetSkater( skaterIndex );
	}
	else
	...
	if( pSkater && pSkater->IsLocalClient() )
	{
		...
		{
			uint32 movieName = 0;
			pParams->GetChecksum( Crc::ConstCRC("name"), &movieName, true );
			Mdl::Skate * skate_mod = Mdl::Skate::Instance();
...
//	pParams->GetChecksum( Crc::ConstCRC("name"), &movieName );
	...
	// at any given time...
	uint32 movieName = Crc::ConstCRC("cutscene");
	...
	// start the movie with the default params...
	Mdl::Skate * skate_mod = Mdl::Skate::Instance();
	skate_mod->GetMovieManager()->AddMovieToQueue( movieName, pParams, Crc::ConstCRC("cutscene") );
...
bool ScriptPlayMovingObjectAnim( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	// start the movie with the default params...
	Mdl::Skate * skate_mod = Mdl::Skate::Instance();
	skate_mod->GetObjectAnimManager()->AddMovieToQueue( movieName, pParams, Crc::ConstCRC("objectanim") );
...
bool ScriptGetPlayerFacePoints(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	{
		pScript->GetParams()->AddStructure( Crc::ConstCRC("current_face_points"), pFacePointsStruct );
...
bool ScriptSetPlayerFaceTexture(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	pParams->GetText( Crc::ConstCRC("texture"), &pFaceName, Script::ASSERT );
	...
	// filename
	bool fullPathIncluded = pParams->ContainsFlag( Crc::ConstCRC("FullPathIncluded") );
...
bool ScriptSetPlayerFaceOverlayTexture(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	pParams->GetText( Crc::ConstCRC("texture"), &pOverlayTextureName, Script::ASSERT );
...
bool ScriptSetGoal(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	if (!Mdl::Skate::Instance()->GetGameMode()->IsTrue( Crc::ConstCRC("is_career") ) && !Mdl::Skate::Instance()->GetGameMode()->IsFrontEnd() )
	{
		return true;
	}
...
bool ScriptSetGlobalFlag(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	int flag; 		
	pParams->GetInteger(Crc::ConstCRC("flag"), &flag, true);  		// Note we assert if "flag" is missing
...
bool ScriptApplyToSkaterProfile(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	if ( pParams->GetInteger( Crc::ConstCRC("skater"), &skater ) )
	{
...
bool ScriptRefreshSkaterColors(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	pParams->GetInteger( Crc::ConstCRC("profile"), &profile, Script::ASSERT );
...
bool ScriptSkaterIsBraking( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	Dbg_Assert( pSkater );
	return pSkater->CallMemberFunction(Crc::ConstCRC("Braking"), pParams, pScript);
}
...
bool ScriptRefreshSkaterModel(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	profile = no_name;
	skater = no_name;
	pParams->GetInteger( Crc::ConstCRC("profile"), &profile );
	pParams->GetInteger( Crc::ConstCRC("skater"), &skater );
	Dbg_Printf( "Skater %d  Profile %d\n", skater, profile );
	Obj::CSkater* pSkater = Mdl::Skate::Instance()->GetSkater(skater);
	...
	{
		Obj::CCompositeObject*p_bailboard = (Obj::CCompositeObject*) Obj::CCompositeObjectManager::Instance()->GetObjectByID(Crc::ConstCRC("bailboard"));
		if (p_bailboard)
		{
	...
	if ( pParams->GetChecksum( Crc::ConstCRC("no_board"), &board ) )
	{
		...
		// Hide the board
		//Mdl::Skate * skate_mod =  Mdl::Skate::Instance();
		//if ( skate_mod->m_cur_level == Crc::ConstCRC("load_cas") )
		//{
			// turn off board
			Obj::CModelComponent* pModelComponent = GetModelComponentFromObject(pSkater);
			pModelComponent->HideGeom(Crc::ConstCRC("board"), true, true);
...
bool ScriptEditPlayerAppearance(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	pParams->GetInteger( NONAME, &no_name );
	profile_num = no_name;
	pParams->GetInteger( Crc::ConstCRC("profile"), &profile_num );
...
bool ScriptGetCurrentSkaterProfileIndex(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	pScript->GetParams()->AddInteger(Crc::ConstCRC("currentSkaterProfileIndex"), index);
...
bool ScriptPrintSkaterStats2( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	for (uint32 i=0;i<skate_mod->GetNumSkaters();i++)
	{
		...
		printf("Air    %i\n",pProfile->GetStatValue(Crc::ConstCRC("AIR")));		
		printf("Run    %i\n",pProfile->GetStatValue(Crc::ConstCRC("RUN")));		
		printf("Ollie  %i\n",pProfile->GetStatValue(Crc::ConstCRC("OLLIE")));		
		printf("Speed  %i\n",pProfile->GetStatValue(Crc::ConstCRC("SPEED")));		
		printf("Spin   %i\n",pProfile->GetStatValue(Crc::ConstCRC("SPIN")));		
		printf("Flip   %i\n",pProfile->GetStatValue(Crc::ConstCRC("FLIP_SPEED")));		
		printf("Switch %i\n",pProfile->GetStatValue(Crc::ConstCRC("SWITCH")));		
		printf("Rail   %i\n",pProfile->GetStatValue(Crc::ConstCRC("RAIL_BALANCE")));		
		printf("Lip    %i\n",pProfile->GetStatValue(Crc::ConstCRC("LIP_BALANCE")));		
		printf("Manual %i\n",pProfile->GetStatValue(Crc::ConstCRC("MANUAL_BALANCE")));		
	}
#endif		// __NOPT_ASSERT__
...
bool ScriptGetActualPlayerAppearancePart( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	uint32 partChecksum;
	if ( pParams->GetChecksum( Crc::ConstCRC("part"), &partChecksum, Script::NO_ASSERT ) )
	{
		Script::CStruct* pVirtualStruct;
...
bool ScriptSetPlayerAppearanceColor( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	if ( pVirtualStruct )
	{
		float h, s, v;
		int useDefaultHSV;
		pParams->GetFloat( Crc::ConstCRC("h"), &h, Script::ASSERT );
		pParams->GetFloat( Crc::ConstCRC("s"), &s, Script::ASSERT );
		pParams->GetFloat( Crc::ConstCRC("v"), &v, Script::ASSERT );
		pParams->GetInteger( Crc::ConstCRC("use_default_hsv"), &useDefaultHSV, Script::ASSERT );

		pVirtualStruct->AddInteger( Crc::ConstCRC("h"), (int)h );
		pVirtualStruct->AddInteger( Crc::ConstCRC("s"), (int)s );
		pVirtualStruct->AddInteger( Crc::ConstCRC("v"), (int)v );
		pVirtualStruct->AddInteger( Crc::ConstCRC("use_default_hsv"), useDefaultHSV );
	}
	else
	{
...
bool ScriptSetPlayerAppearanceScale( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	float x;
	if ( pParams->GetFloat( Crc::ConstCRC("x"), &x, Script::NO_ASSERT ) )
	{
		pVirtualStruct->AddInteger( Crc::ConstCRC("x"), (int)x );
	}
	...
	float z;
	if ( pParams->GetFloat( Crc::ConstCRC("z"), &z, Script::NO_ASSERT ) )
	{
		pVirtualStruct->AddInteger( Crc::ConstCRC("z"), (int)z );
	}
	
	int useDefaultScale;
	if ( pParams->GetInteger( Crc::ConstCRC("use_default_scale"), &useDefaultScale, Script::NO_ASSERT ) )
	{
		pVirtualStruct->AddInteger( Crc::ConstCRC("use_default_scale"), useDefaultScale );
	}	
...
bool ScriptSetPlayerAppearanceUV( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	if ( pVirtualStruct )
	{
		...
		int useDefaultUV;
		pParams->GetFloat( Crc::ConstCRC("uv_u"), &u, Script::ASSERT );
		pParams->GetFloat( Crc::ConstCRC("uv_v"), &v, Script::ASSERT );
		pParams->GetFloat( Crc::ConstCRC("uv_scale"), &uv_scale, Script::ASSERT );
		pParams->GetFloat( Crc::ConstCRC("uv_rot"), &uv_rot, Script::ASSERT );
		pParams->GetInteger( Crc::ConstCRC("use_default_uv"), &useDefaultUV, Script::ASSERT );
				   
		pVirtualStruct->AddFloat( Crc::ConstCRC("uv_u"), u );
		pVirtualStruct->AddFloat( Crc::ConstCRC("uv_v"), v );
		pVirtualStruct->AddFloat( Crc::ConstCRC("uv_scale"), uv_scale );
		pVirtualStruct->AddFloat( Crc::ConstCRC("uv_rot"), uv_rot );
		pVirtualStruct->AddInteger( Crc::ConstCRC("use_default_uv"), useDefaultUV );
	}
	else
	{
...
Script::CStruct* GetNextUndisqualified( Script::CArray* pPartArray, size_t startIndex )
{
	Dbg_Assert( pPartArray );

	startIndex++;
	if ( startIndex >= pPartArray->GetSize() )
	{
		startIndex -= pPartArray->GetSize();
	}

	for (size_t i = 0; i < pPartArray->GetSize(); i++)
	{
		size_t currIndex = (i + startIndex) % pPartArray->GetSize();
...
Script::CStruct* GetPrevUndisqualified( Script::CArray* pPartArray, size_t startIndex )
{
	Dbg_Assert( pPartArray );
	...
	{
		size_t currIndex = (i + 1 + startIndex) % pPartArray->GetSize();
...
bool ScriptGetKeyComboBoundToTrick( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	if (trick_checksum != 0)
	{
		...
		{
			if ( !key_combo && pParams->ContainsFlag( Crc::ConstCRC("special") ) )
			{
...
bool ScriptUpdateTrickMappings( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	{
		int skater_num;
		pParams->GetInteger( Crc::ConstCRC("skater"), &skater_num, Script::ASSERT );
...
bool ScriptGetConfigurableTricksFromType( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	size_t size = p_all_tricks->GetSize();
...
bool ScriptTrickIsLocked( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	if ( p_trick )
	{
		...
		uint32 skater;
		if ( p_trick_params && p_trick_params->GetChecksum( Crc::ConstCRC("skater"), &skater, Script::NO_ASSERT ) )
		{
			Obj::CPlayerProfileManager* pPlayerProfileManager = Mdl::Skate::Instance()->GetPlayerProfileManager();
...
bool ScriptGetIndexOfItemContaining( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	uint32 item;
	pParams->GetChecksum( Crc::ConstCRC("name"), &item, Script::ASSERT );
	...
	int array_size = (int)p_array->GetSize();
...
bool ScriptGetNumSoundtracks( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	size_t numSoundtracks = Pcm::GetNumSoundtracks();
	pScript->GetParams()->AddInteger( "numSoundtracks", (int)numSoundtracks );
	return true;
}
...
bool ScriptGetSoundtrackName( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	const char* pSoundtrackName = Pcm::GetSoundtrackName((size_t)soundtrack_number);
...
bool ScriptGetKeyComboArrayFromTrickArray( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	Mdl::Skate * pSkate = Mdl::Skate::Instance();
	Obj::CSkaterProfile* pSkaterProfile = pSkate->GetCurrentProfile();
	Script::CStruct* pTrickMapping = pSkaterProfile->GetTrickMapping( Crc::ConstCRC("trick_mapping") );

	size_t size = pTricks->GetSize();
	// make a return array
...
static bool s_is_angle(Script::CArray *p_angles_array, uint32 component_name)
{
	...
	size_t size=p_angles_array->GetSize();
	for (size_t i=0; i<size; ++i)
	{
...
bool ScriptInterpolateParameters( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	pScript->GetParams()->AddStructurePointer(Crc::ConstCRC("Interpolated"),p_new_structure);
	return true;
}
...
bool ScriptPlaySkaterStream ( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	const char* p_type;
	pParams->GetString( Crc::ConstCRC("type"), &p_type, Script::ASSERT );
	
	int max_num = 10;
	pParams->GetInteger( Crc::ConstCRC("num_possible"), &max_num, Script::NO_ASSERT );
	...
	if ( p_skater->m_isPro )
	{
		...
		Script::CStruct* pLastNames = Script::GetStructure( Crc::ConstCRC("goal_pro_last_names"), Script::ASSERT );
...
bool ScriptGetTextureFromPath( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	pParams->GetString( Crc::ConstCRC("path"), &p_path, Script::ASSERT );
	...
	pScript->GetParams()->AddString( Crc::ConstCRC("texture"), new_string );
	return true;
}
...
bool ScriptBroadcastEnterVehicle ( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	pParams->GetChecksum( Crc::ConstCRC("SkaterId"), &id, Script::ASSERT );
	pParams->GetChecksum( Crc::ConstCRC("control_type"), &control_type, Script::ASSERT );
...
bool ScriptGetCollidingPlayerAndTeam( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	int exclude_team;
	pParams->GetInteger(Crc::ConstCRC("exclude_team"), &exclude_team, Script::ASSERT);
	
	float radius_squared;
	pParams->GetFloat(Crc::ConstCRC("radius"), &radius_squared, Script::ASSERT);
	...
	p_pass_back_params->AddInteger(Crc::ConstCRC("team"), team);
	p_pass_back_params->AddInteger(Crc::ConstCRC("player"), player_id);
	return true;
}
...
bool ScriptMarkRestarts ( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	for (int i = pNodeArray->GetSize(); i--; )
	{
		...
		uint32 node_class;
		pNodeArray->GetStructure(i)->GetChecksum(Crc::ConstCRC("Class"), &node_class);
		if (node_class == Crc::ConstCRC("Restart"))
		{
			Script::CArray *pRestartTypes;
			pNodeArray->GetStructure(i)->GetArray(Crc::ConstCRC("restart_types"), &pRestartTypes);
			for (int j = pRestartTypes->GetSize(); j--; )
			{
				switch (pRestartTypes->GetChecksum(j))
				{
					case Crc::ConstCRC("Player1"):
						color = MAKE_RGB(0, 255, 0);
						offset.Set(0.0f, 0.0f, 0.0f);
						break;
					case Crc::ConstCRC("Multiplayer"):
						color = MAKE_RGB(255, 0, 255);
						offset.Set(1.0f, 0.0f, 0.0f);
						break;
					case Crc::ConstCRC("Horse"):
						color = MAKE_RGB(0, 255, 255);
						offset.Set(0.0f, 1.0f, 0.0f);
						break;
					case Crc::ConstCRC("CTF"):
						color = MAKE_RGB(255, 0, 0);
						offset.Set(0.0f, 0.0f, 1.0f);
						break;
					case Crc::ConstCRC("Team"):
		...
		else if (node_class == Crc::ConstCRC("GenericNode"))
		{
			uint32 type;
			pNodeArray->GetStructure(i)->GetChecksum(Crc::ConstCRC("Type"), &type);
			if (type != Crc::ConstCRC("Crown")) continue;
