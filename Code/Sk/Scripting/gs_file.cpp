...
void LoadAllStartupQBFiles()
{
	...
	CSymbolTableEntry *p_debugger_names=LookUpSymbol(Crc::ConstCRC("debugger_names"));
	if (p_debugger_names)
	{
	...
	while (p_sym)
	{
			...
			p_sym->mNameChecksum==0xc472ecc5/*NodeArray*/)
		{
			...
			for (int i=0; i<size; ++i)
			{
				...
				if (p_node->GetVector(Crc::ConstCRC("Position"),&p))
				{
					...
					p_node->RemoveComponent(Crc::ConstCRC("Position"));
					p_node->AddVector(Crc::ConstCRC("Pos"),p[X],p[Y],-p[Z]);
				}	
			}
