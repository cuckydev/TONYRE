...
int	GetNearestNodeByPrefix(uint32 prefix, const Mth::Vector &pos)
{
	...
	while (num_nodes)
	{
		...
		Mth::Vector node_pos;
		p_node->GetVector(Crc::ConstCRC("pos"),&node_pos);
...
uint32	GetNodeNameChecksum(int nodeIndex)
{
	...
	uint32 name = 0;
	p_struct->GetChecksum(Crc::ConstCRC("name"),&name);
	return name;
}
...
void GetPosition(CStruct *p_node, Mth::Vector *p_vector)
{
	...
	{
		uint32 name = 0;
		p_node->GetChecksum(Crc::ConstCRC("name"),&name);
		if (name)
		{
	...
	if ( p_node->GetVector(Crc::ConstCRC("pos"),p_vector,Script::NO_ASSERT) )
	{
		// no need to negate the new style vectors
	}
	else if ( p_node->GetVector(Crc::ConstCRC("Position"),p_vector,Script::NO_ASSERT) )
	{
...
void CreateNodeArray(int size, bool hackUseFrontEndHeap)
{
	...
	CleanUpAndRemoveSymbol(Crc::ConstCRC("NodeArray"));
	DeletePrefixInfo();
	ClearNodeNameHashTable();
