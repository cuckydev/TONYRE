...
// of using whatever camera directly	
	...
	while (p_prox_comp)
	{
		Obj::CCameraComponent* p_peer_cam_comp = GetCameraComponentFromObject(p_prox_comp->GetObj());
...
bool ScriptIntegerEquals(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	int b = 0;
	pParams->GetInteger( Crc::ConstCRC("b"), &b );
...
bool ScriptGetArraySize(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	if(pParams->GetInteger(Crc::ConstCRC("index1"),(int*)&index, Script::NO_ASSERT))
	{
		...
		if(pParams->GetInteger(Crc::ConstCRC("index2"),(int*)&index, Script::NO_ASSERT))
		{
			pArray = pArray->GetArray(index);

			if(pParams->GetInteger(Crc::ConstCRC("index3"),(int*)&index, Script::NO_ASSERT))
			{
				pArray = pArray->GetArray(index);
			}
		}
...
bool ScriptSetArrayElement(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	Script::CComponent *p_new_value=pParams->FindNamedComponentRecurse(Crc::ConstCRC("NewValue"));
...
bool ScriptGet2DArrayData(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	// Get the index of the element
	uint32 index2=0;
	pParams->GetInteger(Crc::ConstCRC("Index2"),(int*)&index2,Script::ASSERT);
	...
	{
	case ESYMBOLTYPE_INTEGER:
		pScript->GetParams()->AddInteger( Crc::ConstCRC("val"), p_array2->GetInteger(index2) );
		break;
    case ESYMBOLTYPE_FLOAT:
		pScript->GetParams()->AddFloat( Crc::ConstCRC("val"), p_array2->GetFloat(index2) );
		break;
    case ESYMBOLTYPE_NAME:
		pScript->GetParams()->AddChecksum( Crc::ConstCRC("val"), p_array2->GetChecksum(index2) );
		break;
    case ESYMBOLTYPE_STRING:
	{
		pScript->GetParams()->AddString( Crc::ConstCRC("val"), p_array2->GetString(index2) );
		break;
	}	
    case ESYMBOLTYPE_LOCALSTRING:
	{
	...
	{	
		...
		pScript->GetParams()->AddPair( Crc::ConstCRC("val"), p_pair->mX, p_pair->mY );
		break;
	}	
    case ESYMBOLTYPE_VECTOR:
	{
		...
		pScript->GetParams()->AddVector( Crc::ConstCRC("val"), p_vector->mX, p_vector->mY, p_vector->mZ );
		break;
	}	
    case ESYMBOLTYPE_STRUCTURE:
	{
		pScript->GetParams()->AddStructure( Crc::ConstCRC("val"), p_array2->GetStructure(index2) );
		break;
	}	
    case ESYMBOLTYPE_ARRAY:
	{
		pScript->GetParams()->AddArray( Crc::ConstCRC("val"), p_array2->GetArray(index2) );
		break;
	}	
	default:
...
bool ScriptGet3DArrayData(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	// Get the index of the element
	uint32 index3=0;
	pParams->GetInteger(Crc::ConstCRC("Index3"),(int*)&index3,Script::ASSERT);
	...
	{
	case ESYMBOLTYPE_INTEGER:
		pScript->GetParams()->AddInteger( Crc::ConstCRC("val"), p_array3->GetInteger(index3) );
		break;
    case ESYMBOLTYPE_FLOAT:
		pScript->GetParams()->AddFloat(Crc::ConstCRC("val"), p_array3->GetFloat(index3) );
		break;
    case ESYMBOLTYPE_NAME:
		pScript->GetParams()->AddChecksum( Crc::ConstCRC("val"), p_array3->GetChecksum(index3) );
		break;
    case ESYMBOLTYPE_STRING:
	{
		pScript->GetParams()->AddString( Crc::ConstCRC("val"), p_array3->GetString(index3) );
		break;
	}	
    case ESYMBOLTYPE_LOCALSTRING:
	{
    ...
    case ESYMBOLTYPE_STRUCTURE:
	{
		pScript->GetParams()->AddStructure( Crc::ConstCRC("val"), p_array3->GetStructure(index3) );
		break;
	}	
    case ESYMBOLTYPE_ARRAY:
	{
		pScript->GetParams()->AddArray( Crc::ConstCRC("val"), p_array3->GetArray(index3) );
		break;
	}	
	default:
...
bool ScriptGetNDArrayData(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	{
	case ESYMBOLTYPE_INTEGER:
		pScript->GetParams()->AddInteger( Crc::ConstCRC("val"), p_array->GetInteger(index) );
		break;
    case ESYMBOLTYPE_FLOAT:
		pScript->GetParams()->AddFloat( Crc::ConstCRC("val"), p_array->GetFloat(index) );
		break;
    case ESYMBOLTYPE_NAME:
		pScript->GetParams()->AddChecksum( Crc::ConstCRC("val"), p_array->GetChecksum(index) );
		break;
    case ESYMBOLTYPE_STRING:
	{
		pScript->GetParams()->AddString( Crc::ConstCRC("val"), p_array->GetString(index) );
		break;
	}	
    case ESYMBOLTYPE_LOCALSTRING:
	{
    ...
    case ESYMBOLTYPE_STRUCTURE:
	{
		pScript->GetParams()->AddStructure( Crc::ConstCRC("val"), p_array->GetStructure(index) );
		break;
	}	
    case ESYMBOLTYPE_ARRAY:
	{
		pScript->GetParams()->AddArray( Crc::ConstCRC("val"), p_array->GetArray(index) );
		break;
	}	
	default:
...
bool ScriptSetScriptString(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	const char *p_string="";
	pParams->GetString(Crc::ConstCRC("string"),&p_string);
...
bool ScriptGetScriptString(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	pScript->GetParams()->AddString(Crc::ConstCRC("string"),Script::GetScriptString(index));
	return true;
}
...
bool ScriptKenTest1(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	if (pParams->ContainsFlag(Crc::ConstCRC("Clear")))
	{
...
bool ScriptAppendSuffix(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	pParams->GetChecksum(Crc::ConstCRC("NewTextName"),&new_text_name);
...
bool ScriptFindNearestRailPoint(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	pParams->GetFloat(Crc::ConstCRC("MinDist"),&min_dist);
	float min_dist2=min_dist*min_dist;

	uint32 ignore_rail_id=0;
	pParams->GetChecksum(Crc::ConstCRC("IgnoreRailId"),&ignore_rail_id);

	int ignore_rail_point_index=-1;
	pParams->GetInteger(Crc::ConstCRC("IgnoreRailPointIndex"),&ignore_rail_point_index);
	...
	if (pParams->ContainsFlag(Crc::ConstCRC("CheckLevelGeometry")))
	{
		...
		level_nearest_dist2 < min_dist2)
	{
		...
		pScript->GetParams()->AddVector(Crc::ConstCRC("NearestPos"),nearest_point[X],nearest_point[Y],nearest_point[Z]);
		return true;
	}
...
bool ScriptGetRandomValue(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	if (pParams->ContainsFlag(Crc::ConstCRC("Integer")))
	{
		int a=0;
		int b=0;
		pParams->GetInteger(Crc::ConstCRC("a"),&a);
	...
	else
	{
		...
		float a=0.0f;
		float b=0.0f;
		pParams->GetFloat(Crc::ConstCRC("a"),&a);
		pParams->GetFloat(Crc::ConstCRC("b"),&b);
...
bool ScriptSetConfig(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	if (pParams->ContainsFlag(Crc::ConstCRC("cd")))
	{
		if (!Config::gCD)
		{
	...
	if (pParams->ContainsFlag(Crc::ConstCRC("notcd")))
	{
		if (Config::gCD)
		{
	...
	if (pParams->ContainsFlag(Crc::ConstCRC("GotExtraMemory")))
	{
		Config::gGotExtraMemory=true;
	}
	if (pParams->ContainsFlag(Crc::ConstCRC("NoExtraMemory")))		
	{
		Config::gGotExtraMemory=false;
	}
	
	if (pParams->ContainsFlag(Crc::ConstCRC("Pal")))		
	{
		Config::gDisplayType=Config::DISPLAY_TYPE_PAL;
	}
	if (pParams->ContainsFlag(Crc::ConstCRC("NTSC")))		
	{
		Config::gDisplayType=Config::DISPLAY_TYPE_NTSC;
	}
	
	int frame_rate=60;
	if (pParams->GetInteger(Crc::ConstCRC("FrameRate"),&frame_rate))
	{
...
bool ScriptGetRandomArrayElement(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	{
	case ESYMBOLTYPE_STRUCTURE:
		pScript->GetParams()->AddStructure(Crc::ConstCRC("Element"),p_array->GetStructure(index));
		break;
	case ESYMBOLTYPE_ARRAY:
		pScript->GetParams()->AddArray(Crc::ConstCRC("Element"),p_array->GetArray(index));
		break;
	case ESYMBOLTYPE_INTEGER:
		pScript->GetParams()->AddInteger(Crc::ConstCRC("Element"),p_array->GetInteger(index));
		break;
	case ESYMBOLTYPE_FLOAT:
		pScript->GetParams()->AddFloat(Crc::ConstCRC("Element"),p_array->GetFloat(index));
		break;
	case ESYMBOLTYPE_NAME:
		pScript->GetParams()->AddChecksum(Crc::ConstCRC("Element"),p_array->GetChecksum(index));
		break;
	default:
...
bool ScriptGetAnalogueInfo(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	int controller=0;
	pParams->GetInteger( Crc::ConstCRC("Controller"), &controller, Script::NO_ASSERT );
	...
	pScript->GetParams()->AddFloat(Crc::ConstCRC("LeftX"),pad.m_scaled_leftX);
	pScript->GetParams()->AddFloat(Crc::ConstCRC("LeftY"),pad.m_scaled_leftY);
	pScript->GetParams()->AddFloat(Crc::ConstCRC("RightX"),pad.m_scaled_rightX);
	pScript->GetParams()->AddFloat(Crc::ConstCRC("RightY"),pad.m_scaled_rightY);
	return true;
}
...
bool ScriptLoadAnim( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	const char* p_asset_filename;
	uint32 descChecksum = 0;
	pParams->GetText( Crc::ConstCRC("name"), &p_asset_filename );

	pParams->GetChecksum( Crc::ConstCRC("descChecksum"), &descChecksum, true );

	bool async = false;
	if( pParams->ContainsFlag( Crc::ConstCRC("async") ))
	{
		async = true;
	}

	bool use_pip = false;
	if( pParams->ContainsFlag( Crc::ConstCRC("use_pip") ))
	{
		use_pip = true;
	}
...
bool ScriptLoadSound( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	if ( pParams->GetText( NONAME, &pSfxName, true ) )
	{
		...
		else if ( pParams->ContainsFlag( Crc::ConstCRC("NoReverb") ) )
		{
...
bool ScriptPlayStream( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	if ( pParams->GetChecksum( NONAME, &streamNameChecksum ) )
	{
		...
		pParams->GetInteger( Crc::ConstCRC("lipsync") , &lipsync );
...
bool ScriptSetStreamParams( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	if (pParams->GetChecksum( NONAME, &id ))
	{
		...
		adjust_vol = pParams->GetFloat( Crc::ConstCRC("VolL"), &VolL );
		adjust_vol = pParams->GetFloat( Crc::ConstCRC("VolR"), &VolR ) || adjust_vol;
		if (pParams->GetFloat( Crc::ConstCRC("Vol"), &Volume ))
		{
			VolL = Volume;
			VolR = Volume;
			adjust_vol = true;
		...
		if (pParams->GetFloat( Crc::ConstCRC("Pitch"), &Pitch ))
		{
...
bool ScriptPlayTrack( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	pParams->GetInteger(Crc::ConstCRC("loop"), &loop);
...
bool ScriptSetSoundParams( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	if (pParams->GetChecksum( NONAME, &id ))
	{
		...
		if (pParams->GetFloat( Crc::ConstCRC("Pitch"), &Pitch ) && !adjust_vol)
		{
...
bool ScriptSetReverb( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	// if the parameter isn't found, it should default to zero!	
	pParams->GetInteger( Crc::ConstCRC("mode"), &reverbMode );
...
bool ScriptSetSceneColor( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	if (p_scene)
	{
		...
		if 	(color != 0x808080 && (light_group == Crc::ConstCRC("outdoor") || light_group == 0))
		{
		...
		if ((light_group != Crc::ConstCRC("outdoor") && light_group != 0 ) || color == 0x808080 
			/*|| p_scene->GetID() ==Crc::ConstCRC("cloned")*/)
		#endif
		{
		...
		if (light_group == Crc::ConstCRC("outdoor") || light_group == 0)
		{
...
bool ScriptFakeLights( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	int time_period=0;
	pParams->GetInteger(Crc::ConstCRC("time"),&time_period);
	
	// K: Added this bit to allow a set of nodes with a given prefix to be adjusted.
	uint32 prefix=0;	
    if ( pParams->GetChecksumOrStringChecksum( Crc::ConstCRC("Prefix"), &prefix ) )
	{
		uint16 num_nodes = 0;
...
// Mick:  Now we attempt to get the viewer camera object
	...
	Obj::CCompositeObject *p_obj = (Obj::CCompositeObject *)Obj::CCompositeObjectManager::Instance()->GetObjectByID(Crc::ConstCRC("viewer_cam"));
	if (!p_obj)
	{
		return;
	}
...
static int sFormatText(CStruct *p_dest_struct, CStruct *p_format)
{
	...
	p_format->GetChecksum(Crc::ConstCRC("ChecksumName"),&checksum_name_checksum);
		...
		else if (*p_scan=='%')
		{
			...
			*p_param_name_dest=0;
			...
			switch (sTempComponent.mType)
			{
				case ESYMBOLTYPE_INTEGER:
				{
					if (p_format->ContainsFlag(Crc::ConstCRC("UseCommas")))
					{
				...
				case ESYMBOLTYPE_FLOAT:
				{
					...
					p_format->GetInteger(Crc::ConstCRC("DecimalPlaces"),&decimal_places);
...
//				Nx::CSector *p_sector = Nx::CEngine::sGetSector(checksumName);
		...
		case 0x8e6b02ad:  // railnode
			...
			break;
		case Crc::ConstCRC("ClimbingNode"):
...
//				Nx::CSector *p_sector = Nx::CEngine::sGetSector(checksumName);
			...
			case 0x8e6b02ad:  // railnode
			{
				pNode->RemoveComponent(Crc::ConstCRC("angles"));
				pNode->RemoveComponent(Crc::ConstCRC("type")); 	// Type is no longer used, as "TerrainType" suffices
				break;
			}
			...
			case 0xbf4d3536:  // LevelGeometry
			{
				...
				pNode->RemoveComponent(Crc::ConstCRC("collisionmode"));
//				pNode->RemoveComponent(Crc::ConstCRC("position"));
//				pNode->RemoveComponent(Crc::ConstCRC("angles"));
				////////////////////////////////////////////////////////////////////////////////////
...
//						printf ("Occluding node %d, <%s>\n",i,Script::FindChecksumName(checksumName));
					...
					pNode->GetChecksum(Crc::ConstCRC("lightgroup"),&light_group);
...
bool ScriptSetViewMode(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	switch (s_view_mode )
	{
		case Obj::GAMEPLAY_SKATER_ACTIVE:
			SetActiveCamera(Crc::ConstCRC("skatercam0"),0, false);			
			break;
		default:
			SetActiveCamera(Crc::ConstCRC("viewer_cam"),0, true);						
			break;		
	}
...
// ********************************************************************************************
	...
	Script::CSymbolTableEntry *p_sym=Script::LookUpSymbol(Crc::ConstCRC("NodeArray"));
	if (p_sym)
	{
...
{	
	...
	pParams->GetInteger( Crc::ConstCRC("color"), (int*)&color, TRUE );
...
bool ScriptConnectToInternet( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	pParams->GetChecksum( Crc::ConstCRC("success"), &success );
	pParams->GetChecksum( Crc::ConstCRC("failure"), &failure );
...
bool ScriptDisconnectFromInternet( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	pParams->GetChecksum( Crc::ConstCRC("callback"), &callback_script );
...
bool ScriptDisplayLoadingScreen( Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	Nx::CLoadScreen::sDisplay( (char*) pScreen, pParams->ContainsFlag(Crc::ConstCRC("Freeze")),pParams->ContainsFlag(Crc::ConstCRC("blank")));
...
bool ScriptGetFireballLevel(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	params->AddInteger( Crc::ConstCRC("level"), level );
...
bool ScriptClearCheats(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_1")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_2")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_3")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_4")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_5")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_6")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_7")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_8")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_9")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_10")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_11")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_12")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_13")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_14")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_15")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_16")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_17")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_18")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_19")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_20")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_21")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_22")));
	career->UnSetGlobalFlag( Script::GetInteger(Crc::ConstCRC("CHEAT_ON_23")));
...
bool ScriptBroadcastCheat(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	if( pParams->ContainsComponentNamed(Crc::ConstCRC("on")))
	{
		on = 1;
	}
	else
...
bool ScriptChangeSymbolValue(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	{
		if (pParams->ContainsFlag(Crc::ConstCRC("Resolve")))
		{
...
bool ScriptLaunchViewer( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	p_cam_params->AddChecksum("name",Crc::ConstCRC("viewer_cam"));
...
bool ScriptGetViewerObjectID( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	if ( pViewer )
	{
		...
		pScript->GetParams()->AddChecksum( Crc::ConstCRC("objID"), pViewerObject->GetID() );
		pScript->GetParams()->AddChecksum( Crc::ConstCRC("viewerObjectId"), pViewerObject->GetID() );
	}
...
bool ScriptAddRestartsToMenu( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	do
	{
		...
		if (node != -1)
		{
			...
			p_restart_params->AddChecksum("no_bg",Crc::ConstCRC("no_bg"));
			p_restart_params->AddChecksum("centered",Crc::ConstCRC("centered"));
...
#	ifdef __PLAT_XBOX__
	...
	{
		pParams->GetInteger(Crc::ConstCRC("total_blocks_needed"),(int*)&total_blocks_needed);
...
bool ScriptGetBonePosition( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	uint32 obj_id;
	pParams->GetChecksum( Crc::ConstCRC("id"), &obj_id, Script::ASSERT );
...
bool ScriptMangleChecksums( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	{
		pParams->GetInteger( Crc::ConstCRC("a"), (int*)&a, Script::ASSERT );
	}
	
	uint32 b;
	if ( !pParams->GetChecksum( Crc::ConstCRC("b"), &b, Script::NO_ASSERT ) )
	{
		pParams->GetInteger( Crc::ConstCRC("b"), (int*)&b, Script::ASSERT );
	}
	
	uint32 mangled_id = a + b;
	Dbg_Assert( pScript && pScript->GetParams() );
	pScript->GetParams()->AddChecksum( Crc::ConstCRC("mangled_id"), mangled_id );
...
bool ScriptAppendSuffixToChecksum( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	pParams->GetString(Crc::ConstCRC("SuffixString"), &suffix_string, Script::ASSERT);
	
	pScript->GetParams()->AddChecksum(Crc::ConstCRC("appended_id"), Crc::ExtendCRCWithString(base_checksum, suffix_string));
...
bool ScriptTogglePass( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	pParams->GetInteger(Crc::ConstCRC("Pass"),&pass);
...
bool	ScriptMoveNode(Script::CStruct* pParams, Script::CScript* pScript)
{
	...
	Mth::Vector new_pos, old_pos;
  	...
  	if ( !pParams->GetVector(Crc::ConstCRC("position"),&new_pos)
		 && !pParams->GetVector(Crc::ConstCRC("pos"),&new_pos) )
	{
		printf("Missing position parameter in MoveNode\n");
		return false;
	}

	if ( !pNode->GetVector(Crc::ConstCRC("position"),&old_pos)
		 && !pNode->GetVector(Crc::ConstCRC("pos"),&old_pos) )
	{
...
//	pNode->AddVector(Crc::ConstCRC("position"),new_pos[X],new_pos[Y],new_pos[Z]);
	pNode->AddVector(Crc::ConstCRC("pos"),new_pos[X],new_pos[Y],new_pos[Z]);
...
bool	ScriptSetActiveCamera(Script::CStruct* pParams, Script::CScript* pScript)
{
	...
	pParams->GetChecksum(Crc::ConstCRC("id"),&id);
	int	viewport=0;
	pParams->GetInteger(Crc::ConstCRC("viewport"),&viewport);
	bool move = pParams->ContainsFlag(Crc::ConstCRC("move"));
...
bool ScriptShowTracking(Script::CStruct* pParams, Script::CScript* pScript)
{
	...
	bool do_verts = pParams->ContainsFlag(Crc::ConstCRC("verts"));														   
	bool do_lines = pParams->ContainsFlag(Crc::ConstCRC("lines"));					
...
{
	uint32 ex = 0;
	pParams->GetChecksum(Crc::ConstCRC("Ex"), &ex, true);
	uint32 scr = 0;
	pParams->GetChecksum(Crc::ConstCRC("Scr"), &scr, true);
...
bool ScriptStartPreLoadedStream( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	float vol = 100;
	pParams->GetFloat( Crc::ConstCRC("volume"), &vol, Script::NO_ASSERT );
