	...
	else
	{
		...
		if (m_state != vINACTIVE && !blockRebuild)
		{
			if (CParkManager::sInstance()->GetTheme() != old_theme)
			{
				...
				params.AddChecksum("level", Crc::ConstCRC("Load_Sk5Ed"));
...
void CParkEditor::SetAppropriateCamera()
{
	...
	{
		CFuncs::SetActiveCamera(Crc::ConstCRC("raileditor"), 0, false);
	}
	else
	{
		CFuncs::SetActiveCamera(Crc::ConstCRC("parked_cam"), 0, false);
	}	
}
	...
	if (m_state == vEDITING && mp_cursor)
	{
		if (!m_paused)
		{
			if (mp_cursor->InRailPlacementMode())
			{
				...
				{
					Script::RunScript(Crc::ConstCRC("SwitchOnRailEditor"));
				}
				...
				if (!mp_cursor->InRailPlacementMode())
				{
					Script::RunScript(Crc::ConstCRC("SwitchOffRailEditor"));
				}
			}
			else
			{
		...
		if (!m_paused)		
		{
			...
			if (Script::GetInteger(Crc::ConstCRC("show_parked_main_heap_free")))
			{
			...
			*/
			...
			if (do_defragment)
			{
				...
				Script::SpawnScript(Crc::ConstCRC("reload_park"));
				#else
					// XXX
			...
			Front::CScreenElement *p_pct_bar = p_elem_man->GetElement(Crc::ConstCRC("percent_bar_colored_part"), Front::CScreenElementManager::DONT_ASSERT).Convert();
			if (p_pct_bar)
				p_pct_bar->SetScale(shown_pct_resources_used, 1.0f, false, Front::CScreenElement::FORCE_INSTANT);

			if (Script::GetInteger(Crc::ConstCRC("parked_show_memory_stats")))
			{
				...
				Front::CTextElement *p_main_free_elem = (Front::CTextElement *) p_elem_man->GetElement(Crc::ConstCRC("parked_mem_stats_main_free"), Front::CScreenElementManager::ASSERT).Convert();
				...
				Front::CTextElement *p_last_op_elem = (Front::CTextElement *) p_elem_man->GetElement(Crc::ConstCRC("parked_mem_stats_last_op"), Front::CScreenElementManager::ASSERT).Convert();
				...
				Front::CTextElement *p_pct_elem = (Front::CTextElement *) p_elem_man->GetElement(Crc::ConstCRC("parked_mem_stats_pct"), Front::CScreenElementManager::ASSERT).Convert();
				char pct_text[128];
				sprintf(pct_text, "pct:\\c3 %.2f", m_pct_resources_used * 100.0f);
				p_pct_elem->SetText(pct_text);
	
				Front::CTextElement *p_pieces_remaining_elem = (Front::CTextElement *) p_elem_man->GetElement(Crc::ConstCRC("parked_mem_pieces_remaining"), Front::CScreenElementManager::ASSERT).Convert();
				...
				Front::CTextElement *p_main_used_elem = (Front::CTextElement *) p_elem_man->GetElement(Crc::ConstCRC("parked_mem_stats_main_used"), Front::CScreenElementManager::ASSERT).Convert();
			...
			{
				Script::RunScript(Crc::ConstCRC("DestroyMemStatsScreenElements"));
			}	

			// Update the clipboard usage bar.			
			/*
			Front::CScreenElement *p_clipboard_percent_bar = p_elem_man->GetElement(Crc::ConstCRC("clipboard_percent_bar_colored_part"), Front::CScreenElementManager::DONT_ASSERT).Convert();
...
void CParkEditor::pass_event_to_listener(Obj::CEvent *pEvent)
{
	...
	if (target != Crc::ConstCRC("root_window"))
		return;
...
void CCursor::Update(float shiftX, float shiftZ, int rotInc)
{
	...
	*/
	...
	p_params->AddVector(Crc::ConstCRC("pos"),m_pos[X],m_pos[Y],m_pos[Z]);
	Script::RunScript(Crc::ConstCRC("ParkEdCursorPos"),p_params);
	delete p_params;
	*/
...
void CCursor::RefreshHelperText()
{
	...
	if (m_mode == GAP_ADJUST)
		params.AddChecksum(Crc::ConstCRC("mode"), Crc::ConstCRC("gap_adjust"));
	else if (m_mode == GAP)
		params.AddChecksum(Crc::ConstCRC("mode"), Crc::ConstCRC("gap_regular"));
	else if (m_mode == RAIL_PLACEMENT)
		params.AddChecksum(Crc::ConstCRC("mode"), Crc::ConstCRC("rail_placement"));
	else
		params.AddChecksum(Crc::ConstCRC("mode"), Crc::ConstCRC("regular"));
	...
	if (pSet != mp_current_set)
	{
		...
		for (size_t i = 0; i < pSet->mTotalEntries; i++)
		{
			...
			if (is_clipboard_set)
			{
				...
				{
					params.AddChecksum(NONAME,Crc::ConstCRC("EmptyClipBoard"));
				}	
				Script::RunScript("parked_make_piece_menu_item", &params);
			}
			else
...
bool ScriptGetParkEditorTimeOfDayScript(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	if (tod_script)
	{
		pScript->GetParams()->AddChecksum(Crc::ConstCRC("tod_script"),tod_script);
		return true;
	}	
	return false;
}
...
bool ScriptSwitchToParkEditorCamera(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	if (!p_obj)
	{
		...
		p_cam_params->AddChecksum("name",Crc::ConstCRC("parked_cam"));
...
bool ScriptCanCleanlyResizePark(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	{
		int x,z,w,l;
		p_new_bounds->GetInteger(Crc::ConstCRC("x"),&x);
...
bool ScriptGetCurrentParkBounds(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	pScript->GetParams()->AddInteger(Crc::ConstCRC("w"),bounds.GetW());
	pScript->GetParams()->AddInteger(Crc::ConstCRC("l"),bounds.GetL());
	return true;
}	
