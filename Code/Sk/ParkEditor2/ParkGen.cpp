...
//	const uint32 crc_terrain_wood	= 0x39075ea5;
	...
	*/
	...
	pFound->GetVector(Crc::ConstCRC("Pos"), pVector, true);
...
//	pFound->GetVector(Crc::ConstCRC("Position"), pVector, false);
...
//	uint32 crc_position 			= Script::GenerateCRC("position");
	...
	while (p_piece)
	{
			...
			!(p_cloned->GetFlags() & CPiece::mRESTART))
		{
			...
			pNode->AddComponent(Crc::ConstCRC("pos"), pos.GetX(), pos.GetY(), pos.GetZ());
...
// The new-style 'pos' vector is already correct and needs no negation.
//			pNode->AddComponent(Crc::ConstCRC("position"), pos.GetX(), pos.GetY(), pos.GetZ());
...
//	uint32 crc_terrain_metal	= Script::GenerateCRC("terrain_metal");
	...
	*/
	...
	for (; *pNodeNum < stop_node_num; (*pNodeNum)++)
	{
		...
		pNode->AddComponent(Crc::ConstCRC("pos"), pPoint->m_pos.GetX(), pPoint->m_pos.GetY(), pPoint->m_pos.GetZ());
//		pNode->AddComponent(Crc::ConstCRC("Position"), pPoint->m_pos.GetX(), pPoint->m_pos.GetY(), pPoint->m_pos.GetZ());
		if (debug_rail_out)
			Ryan("[%.1f,%.1f,%.1f]", pPoint->m_pos.GetX(), pPoint->m_pos.GetY(), pPoint->m_pos.GetZ());
		pNode->AddComponent(crc_angles, 0.0f, 0.0f, 0.0f);
			...
			for (int n = 0; n < repeat_num; n++)
			{
				...
				*/
				...
				if (type_checksum != 0)
				{
					...
					if (m_restart_tab[i].type == vKING_OF_HILL)
					{
						pNode->AddComponent(Crc::ConstCRC("pos"), m_restart_tab[i].pos.GetX(), m_restart_tab[i].pos.GetY() + 50.0f, m_restart_tab[i].pos.GetZ());
//						pNode->AddComponent(Crc::ConstCRC("Position"), m_restart_tab[i].pos.GetX(), m_restart_tab[i].pos.GetY() + 50.0f, -m_restart_tab[i].pos.GetZ());
						pNode->AddComponent(crc_class, ESYMBOLTYPE_NAME, (int) crc_genericnode);
					}
					else
					{
						pNode->AddComponent(Crc::ConstCRC("pos"), m_restart_tab[i].pos.GetX(), m_restart_tab[i].pos.GetY() + 50.0f, m_restart_tab[i].pos.GetZ());
//						pNode->AddComponent(Crc::ConstCRC("Position"), m_restart_tab[i].pos.GetX(), m_restart_tab[i].pos.GetY(), -m_restart_tab[i].pos.GetZ());
				...
				*/	
					...
					pNode->AddComponent(Crc::ConstCRC("pos"), m_restart_tab[i].pos.GetX(), m_restart_tab[i].pos.GetY(), m_restart_tab[i].pos.GetZ());
//					pNode->AddComponent(Crc::ConstCRC("Position"), m_restart_tab[i].pos.GetX(), m_restart_tab[i].pos.GetY(), -m_restart_tab[i].pos.GetZ());
					pNode->AddComponent(crc_angles, 0.0f, Mth::PI * 0.5f * ((float) m_restart_tab[i].dir + 2), 0.0f);
