...
bool CMetaPiece::IsRestartOrFlag()
{
	...
	{
		case Crc::ConstCRC("Sk3Ed_RS_1p"):
		case Crc::ConstCRC("Sk3Ed_RS_Mp"):
		case Crc::ConstCRC("Sk3Ed_Rs_Ho"):
		case Crc::ConstCRC("Sk3Ed_Rs_KOTH"):
		case Crc::ConstCRC("Sk4Ed_Team_Yellow"):
		case Crc::ConstCRC("Sk4Ed_Team_Green"):
		case Crc::ConstCRC("Sk4Ed_Team_Red"):
		case Crc::ConstCRC("Sk4Ed_Team_Blue"):
			return true;
			break;
...
void CParkManager::Initialize()
{
	...
	for (int s = m_total_sets-1; s >=0 ; --s)
	{
		...
		if (p_set->ContainsFlag(Crc::ConstCRC("clipboard_set")))
		{
			m_palette_set[s].mIsClipboardSet=true;
			m_palette_set[s].mTotalEntries=MAX_CLIPBOARDS;
			
			const char *p_clipboard_title=Script::GetString(Crc::ConstCRC("ClipboardTitle"));
...
void CParkManager::WriteIntoStructure(Script::CStruct *p_struct)
{
	...
	p_struct->AddArrayPointer(Crc::ConstCRC("Park_editor_map"),p_map);
	...
	p_goal_editor->WriteIntoStructure(Crc::ConstCRC("Load_Sk5Ed"),p_goals_struct);
	
	
	p_struct->AddStructurePointer(Crc::ConstCRC("Park_editor_goals"),p_goals_struct);

	// Add in the created rails.			
	Obj::GetRailEditor()->WriteIntoStructure(p_struct);

	p_struct->AddInteger(Crc::ConstCRC("MaxPlayers"),GetGenerator()->GetMaxPlayers());
}
...
#endif
{
	...
	int max_players=2;
	p_struct->GetInteger(Crc::ConstCRC("MaxPlayers"),&max_players);
...
{
	Script::CStruct *p_dma_usage=Script::GetStructure(Crc::ConstCRC("DMA_Usage"),Script::ASSERT);
	int usage=1;
...
//			printf("failed singularity, half %d\n", half);
	...
	descriptor.mCancelFlags=Script::GetInteger(Crc::ConstCRC("Cancel_Ground"));
}
...
void CParkManager::GetSummaryInfoFromBuffer(uint8 *p_buffer, int *p_numGaps, int *p_numMetas, uint16 *p_theme, uint32 *p_todScript, int *p_width, int *p_length)
{
	...
	*p_theme=p_header->mTheme;
	...
	{
		*p_todScript=Crc::ConstCRC("default");
	}	
}
