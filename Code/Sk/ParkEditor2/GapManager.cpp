...
void CGapManager::LaunchGap(int tab_index, Script::CScript *pScript)
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		pGapParms->AddChecksum(Crc::ConstCRC("GapID"), mp_gapInfoTab[tab_index]->m_id[half_num]);
		pGapParms->AddInteger(Crc::ConstCRC("score"), mp_gapInfoTab[tab_index]->m_descriptor.score);
		pGapParms->AddString(Crc::ConstCRC("text"), mp_gapInfoTab[tab_index]->mp_text);
		...
		if (i == 0)
		{
			...
			if (!cancel_flags)
			{
				...
				cancel_flags=Script::GetInteger(Crc::ConstCRC("CANCEL_GROUND"));
			}
			pGapParms->AddChecksum(Crc::ConstCRC("combined_flags"),cancel_flags);
