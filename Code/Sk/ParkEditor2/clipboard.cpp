...
bool CClipboardEntry::CreateGapFillerPieces()
{
	...
	{
	case Crc::ConstCRC("floor_block1"):
	case Crc::ConstCRC("floor_block2"):
	case Crc::ConstCRC("floor_block3"):
	case Crc::ConstCRC("floor_block4"):
	case Crc::ConstCRC("floor_block5"):
	case Crc::ConstCRC("floor_block6"):
	case Crc::ConstCRC("floor_block7"):
	case Crc::ConstCRC("floor_block8"):
		break;
	default:
		return true;
		break;
	...
	for (int y=mY; y>0; --y)
	{
		...
		if (y==mY)
		{
			p_new_entry->mpMeta = CParkManager::sInstance()->GetAbstractMeta(Crc::ConstCRC("floor_wall_block3"));
		}
		else
		{
			p_new_entry->mpMeta = CParkManager::sInstance()->GetAbstractMeta(Crc::ConstCRC("wall_block1"));
		}
