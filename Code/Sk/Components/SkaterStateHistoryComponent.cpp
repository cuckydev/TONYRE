...
void CSkaterStateHistoryComponent::CollideWithOtherSkaters( int start_index )
{
	...
	for (int i = start_index; i < Mdl::Skate::vMAX_SKATERS; i++)
	{
		...
		if (Mdl::Skate::Instance()->GetGameMode()->GetNameChecksum() == Crc::ConstCRC("netking")
			|| Mdl::Skate::Instance()->GetGameMode()->GetNameChecksum() == Crc::ConstCRC("king"))
		{
...
int			CSkaterStateHistoryComponent::sHandleProjectileHit ( Net::MsgHandlerContext* context )
{
	...
	switch (context->m_MsgId)
	{
		case GameNet::MSG_ID_SKATER_PROJECTILE_HIT_TARGET:
		{
			...
			params.AddString(Crc::ConstCRC("String0"), name);
			params.AddChecksum( NONAME, Crc::ConstCRC("fireball") );
			p_skater->SelfEvent(Crc::ConstCRC("MadeOtherSkaterBail"),&params);
		...
		case GameNet::MSG_ID_SKATER_HIT_BY_PROJECTILE:
		{
			...
			params.AddString(Crc::ConstCRC("String0"), name);
			params.AddChecksum( NONAME, Crc::ConstCRC("fireball") );
			p_skater->SelfEvent(Crc::ConstCRC("SkaterCollideBail"),&params);
			break;
		}
...
int	CSkaterStateHistoryComponent::sHandleCollision( Net::MsgHandlerContext* context )
{
	...
	switch (context->m_MsgId)
	{
		case GameNet::MSG_ID_SKATER_COLLIDE_WON:
		{
			...
			if (loser_drivng)
			{
				params.AddChecksum(NO_NAME, Crc::ConstCRC("LoserIsDriving"));
			}
			p_skater->SelfEvent(Crc::ConstCRC("MadeOtherSkaterBail"),&params);
		...
		case GameNet::MSG_ID_SKATER_COLLIDE_LOST:
		{
			...
			Script::CStruct params;
			params.AddString(Crc::ConstCRC("String0"), name);
			params.AddVector(Crc::ConstCRC("Offset"), p_msg->m_Offset);
			if (winner_driving)
			{
				params.AddChecksum(NO_NAME, Crc::ConstCRC("WinnerIsDriving"));
			}
			p_skater->SelfEvent(Crc::ConstCRC("SkaterCollideBail"),&params);
			break;
		}
...
float CSkaterStateHistoryComponent::get_collision_cylinder_coeff( bool driving )
{
	...
	{
		radius = GetPhysicsFloat( Crc::ConstCRC("LanClientCollCoefficient"), Script::ASSERT);
	}
...
float CSkaterStateHistoryComponent::get_collision_cylinder_radius( bool first_driving, bool second_driving )
{
	...
	if (GetSkater()->IsLocalClient())
	{
		...
		{
			radius = GetPhysicsFloat(Crc::ConstCRC("InternetServerCollRadius"), Script::ASSERT);
		}
		else
		{
			radius = GetPhysicsFloat(Crc::ConstCRC("LanServerCollRadius"), Script::ASSERT);
		}
	}
	else
	{
		if (GameNet::Manager::Instance()->InInternetMode())
		{
			radius = GetPhysicsFloat(Crc::ConstCRC("InternetClientCollRadius"), Script::ASSERT);
		}
		else
		{
			radius = GetPhysicsFloat(Crc::ConstCRC("LanClientCollRadius"), Script::ASSERT);
		}
	}
	...
	return radius * (1.0f + driving_count * GetPhysicsFloat(Crc::ConstCRC("DrivingRadiusBoostFactor")));
}
