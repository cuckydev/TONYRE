...
void CSkaterLoopingSoundComponent::InitFromStructure( Script::CStruct* pParams )
{
	...
	pParams->GetFloat( Crc::ConstCRC("volume_mult"), &m_vol_mult, Script::NO_ASSERT );
}
...
CBaseComponent::EMemberFunctionResult CSkaterLoopingSoundComponent::CallMemberFunction ( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	switch ( Checksum )
	{
		...
		case Crc::ConstCRC("SkaterLoopingSound_TurnOff"):
			SetActive(false);
			break;
		...
		case Crc::ConstCRC( "PlayAnim"):
			...
			if (GetObj()->GetTags()
				...
				&& GetObj()->GetTags()->GetStructure(Crc::ConstCRC("AnimTags"), &p_anim_tags_struct)
				&& p_anim_tags_struct->ContainsFlag(Crc::ConstCRC("WalkToSkateTransition")))
			{
				Suspend(false);
				m_active = true;
			}
