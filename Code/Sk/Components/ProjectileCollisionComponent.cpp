...
{
	pParams->GetFloat(Crc::ConstCRC("radius"), &m_radius, Script::ASSERT);
	pParams->GetFloat(Crc::ConstCRC("scale"), &m_scale, Script::ASSERT);
	pParams->GetChecksum(Crc::ConstCRC("owner_id"), &m_owner_id, Script::ASSERT);
		...
		for( uint32 i = 0; i < NumSkaters; i++ )
		{
			...
			if( team_game )
			{
				if( src_player->m_Team == target_player->m_Team )
				{
					...
					enabled = gamenet_man->GetNetworkPreferences()->GetPreferenceChecksum( Crc::ConstCRC("friendly_fire"), Crc::ConstCRC("checksum"));
					if( enabled != Crc::ConstCRC("boolean_true"))
					{
						continue;
					}
				}
				...
				if(( score->GetTotalScore() - get_damage_amount() ) <= 0 )
				{
					...
					if( score->GetTotalScore() > 0 )
					{
						if( target_player->IsLocalPlayer() == false )
						{
							...
							params = new Script::CStruct;
							params->AddString( Crc::ConstCRC("name"), pSkater->GetDisplayName());
