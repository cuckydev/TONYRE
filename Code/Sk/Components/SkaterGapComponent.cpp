...
CBaseComponent::EMemberFunctionResult CSkaterGapComponent::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	switch ( Checksum )
	{
		...
		// @parmopt checksum | KeyCombo | | Name of a key combo, like Air_SquareD, you have to do trick with this button combo
		case Crc::ConstCRC("StartGapTrick"):
		...
		// in that case
        case Crc::ConstCRC("CheckGapTricks"):
        ...
        // see online doc for detailed description and examples
		case Crc::ConstCRC("DuplicateTrigger"):
		{
			...
			// a trigger script, and execute that instead
			{
				...
				if (node != -1)	 		// Failsafe
				{
					...
					for (i=0;i<links;i++)
					{
						...
						if (pDest->GetChecksum(Crc::ConstCRC("TriggerScript"),&script))
						{
...
void CSkaterGapComponent::GetDebugInfo(Script::CStruct *p_info)
{
	...
	= {
		Crc::ConstCRC("CANCEL_GROUND"),				// 0x00000001
		Crc::ConstCRC("CANCEL_AIR"),              	// 0x00000002
		Crc::ConstCRC("CANCEL_RAIL"),             	// 0x00000004
		Crc::ConstCRC("CANCEL_WALL"),             	// 0x00000008
		Crc::ConstCRC("CANCEL_LIP"),              	// 0x00000010
		Crc::ConstCRC("CANCEL_WALLPLANT"),			// 0x00000020
		Crc::ConstCRC("CANCEL_MANUAL"),				// 0x00000040
		Crc::ConstCRC("CANCEL_HANG"),     	    	// 0x00000080
		Crc::ConstCRC("CANCEL_LADDER"),	         	// 0x00000100
		Crc::ConstCRC("CANCEL_SKATE"),				// 0x00000200
		Crc::ConstCRC("CANCEL_WALK"),   				// 0x00000400
		Crc::ConstCRC("CANCEL_DRIVE"),				// 0x00000800
		Crc::ConstCRC("NO_SUCH_GAP_FLAG"),			// 0x00001000
		Crc::ConstCRC("NO_SUCH_GAP_FLAG"),			// 0x00002000
		Crc::ConstCRC("NO_SUCH_GAP_FLAG"),			// 0x00004000
		Crc::ConstCRC("NO_SUCH_GAP_FLAG"),			// 0x00008000
		Crc::ConstCRC("REQUIRE_GROUND"),				// 0x00010000
		Crc::ConstCRC("REQUIRE_AIR"),             	// 0x00020000
		Crc::ConstCRC("REQUIRE_RAIL"),            	// 0x00040000
		Crc::ConstCRC("REQUIRE_WALL"),            	// 0x00080000
		Crc::ConstCRC("REQUIRE_LIP"),             	// 0x00100000
		Crc::ConstCRC("REQUIRE_WALLPLANT"),			// 0x00200000
		Crc::ConstCRC("REQUIRE_MANUAL"),				// 0x00400000
		Crc::ConstCRC("REQUIRE_HANG"),	        	// 0x00800000
		Crc::ConstCRC("REQUIRE_LADDER"),	        	// 0x01000000
		Crc::ConstCRC("REQUIRE_SKATE"),				// 0x02000000
		Crc::ConstCRC("REQUIRE_WALK"),				// 0x04000000
		Crc::ConstCRC("REQUIRE_DRIVE"),				// 0x08000000
		Crc::ConstCRC("NO_SUCH_GAP_FLAG"),			// 0x10000000
		Crc::ConstCRC("NO_SUCH_GAP_FLAG"),			// 0x20000000
		Crc::ConstCRC("NO_SUCH_GAP_FLAG"),			// 0x40000000
		Crc::ConstCRC("NO_SUCH_GAP_FLAG")			// 0x80000000
	};
	...
	while (pGap)
	{
		...
		p_gap_struct->AddChecksum(Crc::ConstCRC("Id"), pGap->m_id);
		...
		p_gap_struct->AddStructurePointer(Crc::ConstCRC("Flags"), p_gap_flags);
		
		p_gaps_array->SetStructure(i, p_gap_struct);
		i++;
		pGap = static_cast< CGap* >(pGap->GetNext());
	}
	p_info->AddArrayPointer(Crc::ConstCRC("Gaps"), p_gaps_array);
...
void CSkaterGapComponent::AwardPendingGaps (   )
{
	...
	{
		Script::SpawnScript(Crc::ConstCRC("got_all_gaps_screen_create"));
	}
}
...
void CSkaterGapComponent::get_state_flags ( int& cancel, int& require )
{
	...
	else
	{
		...
		CCompositeObject* p_vehicle = static_cast< CCompositeObject* >(CCompositeObjectManager::Instance()->GetObjectByID(Crc::ConstCRC("PlayerVehicle")));
...
void CSkaterGapComponent::start_gap ( Script::CStruct *pParams, Script::CScript* pScript )
{
	...
	uint32 trickscript;
	if (pParams->GetChecksum(Crc::ConstCRC("trickscript"), &trickscript))
	{
		uint32 key_combo;
		...
		if (pParams->GetChecksum(Crc::ConstCRC("KeyCombo"), &key_combo))
		{
			...
			{
				pGap->m_requirePerfect = pParams->ContainsFlag(Crc::ConstCRC("perfect"));
			...
			pParams->GetInteger( Crc::ConstCRC("num_taps"), &pGap->m_numTaps, Script::NO_ASSERT );

			pGap->m_numTrickOccurrences = mp_score->GetPreviousNumberOfOccurrences( key_combo, pGap->m_spinMult, pGap->m_numTaps );
		}
		else if (pParams->GetString(Crc::ConstCRC("TrickText"), &tricktext))
		{
		...
		else if (pParams->GetArray(Crc::ConstCRC("tricks"), &p_gapTricks))
		{
	...
	{
		uint32 checksum;
		if (pParams->GetChecksum(Crc::ConstCRC("flags"), &checksum, false))
		{
			pGap->m_flags |= Script::GetInteger(checksum);
		}
	}
	...
	if (pParams->GetChecksum(Crc::ConstCRC("combined_flags"),&combined_flags))
	{
	...
	if (pParams->ContainsFlag(Crc::ConstCRC("carGap")))
	{
		// car gaps cancel when skating or walking
		pGap->m_flags |= car_cancel_flags;
	}
	else if (pParams->ContainsFlag(Crc::ConstCRC("complexGap")))
	{
...
void CSkaterGapComponent::start_gap_trick ( Script::CStruct *pParams, Script::CScript* pScript )
{
	...
	pParams->GetChecksum(Crc::ConstCRC("trickscript"), &pGapTrick->m_script, Script::ASSERT);
}
...
void CSkaterGapComponent::end_gap ( Script::CStruct *pParams, Script::CScript* pScript )
{
	...
	for (CGap* pGap = static_cast< CGap* >(m_gap_list.FirstItem()); pGap; pGap = pNext)
	{
		...
		const char *p_name;
		if (!pParams->GetString(Crc::ConstCRC("text"), &p_name))
		{
			p_name = "Unnamed Gap";
		}
		char gap_name[256];
		...
		int score = 0;
		if (pParams->GetInteger(Crc::ConstCRC("score"), &score))
		{
			...
			if (GetSkater()->IsLocalClient())
			{
				...
				GetObj()->SpawnAndRunScript(Crc::ConstCRC("DefaultGapScript"));
			}
		}
		
		//  execute the gap_script
		uint32 gap_script;
		if (pParams->GetChecksum(Crc::ConstCRC("gapscript"), &gap_script))
		{
			GetObj()->SpawnAndRunScript(
				gap_script,
				pScript->mNode,
				pParams->ContainsFlag(Crc::ConstCRC("NetEnabled")),
				pParams->ContainsFlag(Crc::ConstCRC("Permanent"))
			);
		}	  			  
...
{
	if (pParams->ContainsFlag(Crc::ConstCRC("NotInSameFrame")))
	{
