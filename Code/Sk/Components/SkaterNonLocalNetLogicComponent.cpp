...
void CSkaterNonLocalNetLogicComponent::Update()
{
		...
		!local_player->IsSurveying())
	{
		if( ( GameNet::Manager::Instance()->PlayerCollisionEnabled()) ||
			( skate_mod->GetGameMode()->GetNameChecksum() == Crc::ConstCRC("netfirefight")) ||
	...
	if (mp_state_component->GetDriving() && mp_state_history_component->GetCurrentVehicleControlType() != 0)
	{
		...
		pParams->AddChecksum(Crc::ConstCRC("control_type"), mp_state_history_component->GetCurrentVehicleControlType());
		RunScript(Crc::ConstCRC("NonLocalClientInVehicle"), pParams);
		delete pParams;
	}
...
void CSkaterNonLocalNetLogicComponent::snap_to_ground( void  )
{
	...
	// {
		// col_start = GetObj()->m_matrix[Y] * GetPhysicsFloat(Crc::ConstCRC("Physics_Ground_Snap_Up_SKITCHING"));	// much above feet
	// }
	// else
	// {
		col_start = GetObj()->m_matrix[Y] * GetPhysicsFloat(Crc::ConstCRC("Physics_Ground_Snap_Up"));	  		// bit above the feet
	// }
...
void CSkaterNonLocalNetLogicComponent::bounce_off_wall ( const Mth::Vector& normal, const Mth::Vector& point )
{
	...
	Mth::Vector up_offset = GetObj()->m_matrix[Y] * GetPhysicsFloat(Crc::ConstCRC("Skater_First_Forward_Collision_Height"));
					...
					( event->GetTime() <= cur_time ))
				{
					switch( event->m_MsgId )
					{
						...
						case GameNet::MSG_ID_CREATE_SPECIAL_ITEM:
						{
							...
							pLockParams->AddChecksum( Crc::ConstCRC("bone"), event->m_Bone );
							pLockParams->AddChecksum( Crc::ConstCRC("id"), GetObj()->GetID() );
