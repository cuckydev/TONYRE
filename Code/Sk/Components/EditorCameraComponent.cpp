...
static uint32 spKillScripts[]=
{
	...
	Crc::ConstCRC("RU_KillSkater2"),
	Crc::ConstCRC("NY_LeavingMessage"),
	Crc::ConstCRC("SJ_Televator"),
	Crc::ConstCRC("HI_KillSkater_LittleDogs"),
	Crc::ConstCRC("VC_CAG_boundary"),
};
...
static void s_check_if_kill_poly ( CFeeler* p_feeler )
{
	...
	p_node->GetChecksum(Crc::ConstCRC("TriggerScript"),&trigger_script);
...
bool CEditorCameraComponent::find_y(Mth::Vector start, Mth::Vector end, float *p_y)
{
	...
	if (feeler.GetCollision())
	{
		...
		if (normal[Y] < 0.087f/*sinf(Mth::DegToRad(90.0f-Script::GetFloat(Crc::ConstCRC("MaxSteepness"))))*/)
		{
...
bool CEditorCameraComponent::find_collision_at_offset(Mth::Vector oldCursorPos, float upOffset)
{
	...
	end[Y]+=Script::GetFloat(Crc::ConstCRC("EditorCam_CursorCollisionDownOffset"));
...
void CEditorCameraComponent::InitFromStructure( Script::CStruct* pParams )
{
	...
	m_min_height=0.0f;
	pParams->GetFloat(Crc::ConstCRC("min_height"),&m_min_height);
	
	m_radius_min=100.0f;
	pParams->GetFloat(Crc::ConstCRC("min_radius"),&m_radius_min);
	
	m_radius_max=2000.0f;
	pParams->GetFloat(Crc::ConstCRC("max_radius"),&m_radius_max);
	
	m_simple_collision=pParams->ContainsFlag(Crc::ConstCRC("SimpleCollision"));
	m_allow_movement_through_walls=pParams->ContainsFlag(Crc::ConstCRC("AllowMovementThroughWalls"));
}
...
void CEditorCameraComponent::Update()
{
	...
	m_tilt_angle_min=Script::GetFloat(Crc::ConstCRC("EditorCam_TiltMin"));
	m_tilt_angle_max=Script::GetFloat(Crc::ConstCRC("EditorCam_TiltMax"));
	
	CControlPad& control_pad = mp_input_component->GetControlPad();

	
	// Update m_angle
	float angle_vel=Script::GetFloat(Crc::ConstCRC("EditorCam_TurnSpeed"))*control_pad.m_scaled_rightX;
	m_angle+=angle_vel;
	...
	// Update m_tilt_angle
	angle_vel=-Script::GetFloat(Crc::ConstCRC("EditorCam_TiltSpeed"))*control_pad.m_scaled_rightY;
	...
	float radius_vel=Script::GetFloat(Crc::ConstCRC("EditorCam_InOutSpeed"));
	...
	// Update the height
	float height_vel_min=Script::GetFloat(Crc::ConstCRC("EditorCam_UpDownSpeedMin"));
	float height_vel_max=Script::GetFloat(Crc::ConstCRC("EditorCam_UpDownSpeedMax"));
	...
	float max_height=Script::GetFloat(Crc::ConstCRC("EditorCam_MaxHeight"));
	...
	float speed_min=Script::GetFloat(Crc::ConstCRC("EditorCam_MoveSpeedMin"));
	float speed_max=Script::GetFloat(Crc::ConstCRC("EditorCam_MoveSpeedMax"));
	...
	if (!find_collision_at_offset(old_pos, Script::GetFloat(Crc::ConstCRC("EditorCam_CursorCollisionFirstUpOffset"))))
	{
		...
		else if (!find_collision_at_offset(old_pos, Script::GetFloat(Crc::ConstCRC("EditorCam_CursorCollisionSecondUpOffset"))))
		{
			// Undo the movement so that the cursor does not move.
			m_cursor_pos=old_pos;
		}
	...
	m_camera_focus_pos[Y]+=(m_cursor_pos[Y]+m_cursor_height-m_camera_focus_pos[Y])*Script::GetFloat(Crc::ConstCRC("EditorCam_YCatchUpFactor"));
	...
	while (true)
	{
		...
		if (radius > Script::GetFloat(Crc::ConstCRC("EditorCam_CursorCollisionEnableDistMax")))
		{
...
CBaseComponent::EMemberFunctionResult CEditorCameraComponent::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	switch ( Checksum )
	{
		case 0xf362bbbf: // EditorCam_Initialise
			pParams->GetVector(Crc::ConstCRC("position"),&m_cursor_pos);
			m_camera_focus_pos=m_cursor_pos;
			m_radius=700.0f;
			pParams->GetFloat(Crc::ConstCRC("radius"),&m_radius);
			m_tilt_angle=0.6f;
			pParams->GetFloat(Crc::ConstCRC("tilt"),&m_tilt_angle);
			m_cursor_height=0.0f;
			pParams->GetFloat(Crc::ConstCRC("cursor_height"),&m_cursor_height);
			break;

		case 0x69cb3ae1: // EditorCam_SetCursorPos
		{
			Mth::Vector pos;
			pParams->GetVector(Crc::ConstCRC("position"),&pos,Script::ASSERT);
			m_cursor_pos=pos;
			m_cursor_height=0.0f;
