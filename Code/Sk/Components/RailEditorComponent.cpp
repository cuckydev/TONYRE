...
static bool s_angle_is_ok_to_grind(Mth::Vector &a, Mth::Vector &b, Mth::Vector &c)
{
	...
	if (cosine < cosf(Mth::DegToRad(Script::GetFloat(Crc::ConstCRC("Rail_Corner_Leave_Angle")))))
	{
		return false;
	}
...
static bool s_positions_OK(Mth::Vector &a, Mth::Vector &b)
{
	...
	if (diff.Length() < Script::GetFloat(Crc::ConstCRC("RailEditorMinimumPointSeparation")))
	{
		return false;
	}
	...
	diff.Normalize();
	if (fabs(diff[Y]) > sinf(Mth::DegToRad(Script::GetFloat(Crc::ConstCRC("RailEditorMaxSlope")))))
	{
		return false;
	}
...
void CEditedRailPoint::UpdateRailGeometry(Mth::Vector& rotateCentre, Mth::Vector& newCentre, float degrees)
{
	...
	{
		mpClonedRailSector=s_clone_sector(Crc::ConstCRC("Sk5Ed_RA_Dynamic"));
	}
	...
	if (mpNext->mpNext)
	{
		...
		a.Normalize();
		b.Normalize();
		if (Mth::DotProduct(a,b) < cosf(Mth::DegToRad(Script::GetFloat(Crc::ConstCRC("RailEditorMaxJoinAngle")))))
		{
...
#	endif
	...
	if (!mpPostSector)
	{
		mpPostSector=s_clone_sector(Crc::ConstCRC("Sk5Ed_RAp_Dynamic"));
	}
...
void CEditedRailPoint::Highlight(EFlash flash, EEndPosts includeEndPosts)
{
	...
	if (flash)
	{
		int flash_rate=Script::GetInteger(Crc::ConstCRC("RailEditorHighlightFlashRate"));
...
{
	Script::CArray* p_graffiti_col_tab = Script::GetArray( Crc::ConstCRC("graffitiColors") );
...
void CRailEditorComponent::WriteIntoStructure(Script::CStruct *p_info)
{
	...
	while (p_rail)
	{
		...
		while (p_point)
		{
			...
			p_point_info->AddVector(Crc::ConstCRC("Pos"),p_point->mPos[X],p_point->mPos[Y],p_point->mPos[Z]);
			if (p_point->mpPostSector)
			{
				p_point_info->AddChecksum(NONAME,Crc::ConstCRC("HasPost"));
			}	
		...
		p_rail_info->AddArrayPointer(Crc::ConstCRC("Points"),p_points);
	...
	p_info->AddArrayPointer(Crc::ConstCRC("CreatedRails"),p_rails);
}
...
void CRailEditorComponent::ReadFromStructure(Script::CStruct *p_info)
{
	...
	for (size_t i = 0; i < p_rails->GetSize(); ++i)
	{
		...
		for (size_t p = 0; p < p_points->GetSize(); ++p)
		{
			...
			p_point_info->GetVector(Crc::ConstCRC("pos"),&p_new_point->mPos);
			if (p_point_info->ContainsFlag(Crc::ConstCRC("HasPost")))
			{
...
void CRailEditorComponent::Hide( bool shouldHide )
{
	...
	{
		Script::RunScript(Crc::ConstCRC("rail_editor_create_cursor"));
...
void CRailEditorComponent::SetUpRailNodes(Script::CArray *p_nodeArray, int *p_nodeNum, uint32 firstID)
{
	...
	while (p_rail)
	{
		...
		while (p_point)
		{
			...
			p_node->AddVector(Crc::ConstCRC("Pos"),p_point->mPos[X],p_point->mPos[Y],p_point->mPos[Z]);
			p_node->AddVector(Crc::ConstCRC("Angles"),0.0f,0.0f,0.0f);
			
			p_node->AddChecksum(Crc::ConstCRC("Name"),rail_point_id++);
			
			p_node->AddChecksum(Crc::ConstCRC("Class"),Crc::ConstCRC("RailNode"));
			p_node->AddChecksum(NONAME,Crc::ConstCRC("CreatedAtStart"));
			
			p_node->AddChecksum(NONAME,Crc::ConstCRC("TrickObject"));
			sprintf(p_rail_name,"CreatedRailCluster%d",rail_index);
			p_node->AddChecksum(Crc::ConstCRC("Cluster"),Script::GenerateCRC(p_rail_name));
			
			// hardwire to metal for the moment
			p_node->AddChecksum(Crc::ConstCRC("TerrainType"),Crc::ConstCRC("TERRAIN_METALSMOOTH"));
			...
			if (p_point->mpNext)
			{
				...
				p_node->AddArrayPointer(Crc::ConstCRC("Links"), p_links);
			}			
			...
			//p_node->AddChecksum(Crc::ConstCRC("Cluster"),Script::GenerateCRC(p_cluster_name));
...
void CRailEditorComponent::DrawDottedLine(Mth::Vector& pos)
{
	...
	uint32 dotted_line_sector_name=Crc::ConstCRC("Sk5Ed_RAdot_Dynamic_Green");
	if (s_distance_is_too_long(p_last_point->mPos, pos))
	{
		...
		dotted_line_sector_name=Crc::ConstCRC("Sk5Ed_RADot_Dynamic");
	}
	...
	{
		int flash_rate=Script::GetInteger(Crc::ConstCRC("RailEditorRedLineFlashRate"));
		uint32 vis=0xffffffff;
		if (s_highlight_flash_counter >= flash_rate/2)
		{
			vis=0;
...
CBaseComponent::EMemberFunctionResult CRailEditorComponent::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	switch ( Checksum )
	{
		case 0x8f3e0b97: // GetCursorPosition
		{
			...
			pScript->GetParams()->AddVector(Crc::ConstCRC("Pos"),cursor_pos[X],cursor_pos[Y],cursor_pos[Z]);
			pScript->GetParams()->AddFloat(Crc::ConstCRC("Angle"),mp_editor_camera_component->GetCursorOrientation()*180.0f/3.141592654f);
			break;
		}
		...
		case 0x5828e852: // AddNewPosition
		{
			...
			if (mp_current_rail)
			{
				...
				if (p_point)
				{
					...
					p_point->mHasPost=pParams->ContainsFlag(Crc::ConstCRC("AddPost"));
					p_point->UpdateRailGeometry();
					p_point->UpdatePostGeometry();
					return CBaseComponent::MF_TRUE;
				}
		...
		{
			pParams->GetChecksum(Crc::ConstCRC("mode"),&m_mode);
			
			Script::CStruct *p_params=new Script::CStruct;
			p_params->AddChecksum(Crc::ConstCRC("mode"),Crc::ConstCRC("rail_placement"));
			Script::RunScript(Crc::ConstCRC("parked_set_helper_text_mode"),p_params);
			delete p_params;
			break;
		}
		...
		{
			pScript->GetParams()->AddChecksum(Crc::ConstCRC("mode"),m_mode);
			break;
		}	
		...
		case 0x6d2eee1b: // UpdateRailPointPosition
		{
			...
			if (p_rail)
			{
				...
				if (!p_rail->UpdateRailPointPosition(rail_point_index, pos, 
													 (CEditedRail::EUpdateSuperSectors)pParams->ContainsFlag(Crc::ConstCRC("UpdateSuperSectors"))))
				{
					return CBaseComponent::MF_FALSE;
				}	
			}	
		...
		case 0x8798e959: // HighlightRailPoint
		{
			...
			if (p_rail)
			{
				...
				p_point->Highlight((CEditedRailPoint::EFlash)pParams->ContainsFlag(Crc::ConstCRC("Flash")), 
								   (CEditedRailPoint::EEndPosts)pParams->ContainsFlag(Crc::ConstCRC("IncludeEndPosts")));
			}					   
			break;
		}
		...
		case 0xfb6a0888: // GetEditedRailInfo
		{
			...
			if (pParams->ContainsFlag(Crc::ConstCRC("CurrentRail")))
			{
		...
		case 0xd70781c3: // DestroyEditedRailSectors
		{
			...
			if (pParams->ContainsFlag(Crc::ConstCRC("DoNotUpdateSuperSectors")))
			{
...
void CRailEditorComponent::GetDebugInfo(Script::CStruct *p_info)
{
	...
	if (mp_current_rail)
	{
		...
		p_info->AddStructurePointer(Crc::ConstCRC("CurrentRail"),p_struct);
	}
	else
	{
	...
	if (num_rails)
	{
		...
		p_info->AddArrayPointer(Crc::ConstCRC("Rails"),p_array);
	}
...
{
	Obj::CCompositeObject *p_obj=(Obj::CCompositeObject*)Obj::CCompositeObjectManager::Instance()->GetObjectByID(Crc::ConstCRC("RailEditor"));
