...
void CSkaterProximityComponent::Update()
{
	...
	if ( mInnerRadiusSqr )
	{
		...
		if ( distToSkaterSqr <= mInnerRadiusSqr )
		{
			...
			FLAGEXCEPTION( Crc::ConstCRC("SkaterInRadius") );
			if (!GetObj()->IsDead())
			{
				FLAGEXCEPTION( Crc::ConstCRC("AnySkaterInRadius") );
			}
		}
		else
		{
	...
	if ( mOuterRadiusSqr )
	{
		...
		if ( distToSkaterSqr >= mOuterRadiusSqr )
		{
			if (!GetObj()->IsDead())
			{
				FLAGEXCEPTION( Crc::ConstCRC("SkaterOutOfRadius") ); 
			}
		}
	}
	...
	if ( mInnerAvoidRadiusSqr )
	{
		...
		if ( distToSkaterSqr <= mInnerAvoidRadiusSqr )
		{
			if (!GetObj()->IsDead())
			{
				FLAGEXCEPTION( Crc::ConstCRC("SkaterInAvoidRadius") );
			}
		}
	}
	...
	if ( mOuterAvoidRadiusSqr )
	{
		...
		if ( distToSkaterSqr >= mOuterAvoidRadiusSqr )
		{
			if (!GetObj()->IsDead())
			{
				FLAGEXCEPTION( Crc::ConstCRC("SkaterOutOfAvoidRadius") );
			}
		}
	}
}
...
void CSkaterProximityComponent::GetDebugInfo(Script::CStruct *p_info)
{
	...
	Dbg_MsgAssert(p_info,("nullptr p_info sent to CSkaterProximityComponent::GetDebugInfo"));
