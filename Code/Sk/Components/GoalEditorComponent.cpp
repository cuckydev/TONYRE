...
{
	uint32 level=0;
	p_params->GetChecksum(Crc::ConstCRC("level"),&level);
...
static uint32 s_generate_letter_node_type_checksum(uint32 goalType, int letterIndex)
{
	...
	switch (goalType)
	{
		case 0x54166acd: // Skate
			...
			{
				case 0:
					checksum=Crc::ConstCRC("Letter_S");
					break;
				case 1:
...
static const Script::CStruct *s_get_default_goal_params(uint32 goalType)
{
	...
	{
		case 0x54166acd: // Skate
			p_default_params=Script::GetStructure(Crc::ConstCRC("Goal_SkateLetters_genericParams"));
			break;
		case 0x4ec3cfb5: // Combo
			p_default_params=Script::GetStructure(Crc::ConstCRC("Goal_amateurCOMBOline_genericParams"));
			break;
		case 0x6fe44c6d: // HighScore
			p_default_params=Script::GetStructure(Crc::ConstCRC("Goal_HighScore_genericParams"));
			break;
		case 0xec414b76: // HighCombo
			p_default_params=Script::GetStructure(Crc::ConstCRC("goal_highcombo_genericParams"));
			break;
		case 0x7fe2238a: // SkateTris
		case 0x0fc6f698: // ComboSkateTris
		case 0xea7ba666: // TrickTris
			p_default_params=Script::GetStructure(Crc::ConstCRC("goal_tetris_genericParams"));
			break;
		case 0x61c5d092: // Gap
			p_default_params=Script::GetStructure(Crc::ConstCRC("Goal_Gaps_genericParams"));	
			break;
		default:
			break;
	}			
...
static const Script::CStruct *s_get_extra_goal_params(uint32 goalType)
{
	...
	{
		case 0x54166acd: // Skate
			p_extra_params=Script::GetStructure(Crc::ConstCRC("EditedGoal_ExtraParams_Skate"));
			break;
		case 0x4ec3cfb5: // Combo
			p_extra_params=Script::GetStructure(Crc::ConstCRC("EditedGoal_ExtraParams_Combo"));
			break;
		case 0x6fe44c6d: // HighScore
			p_extra_params=Script::GetStructure(Crc::ConstCRC("EditedGoal_ExtraParams_HighScore"));
			break;
		case 0xec414b76: // HighCombo
			p_extra_params=Script::GetStructure(Crc::ConstCRC("EditedGoal_ExtraParams_HighCombo"));
			break;
		case 0x7fe2238a: // SkateTris
			p_extra_params=Script::GetStructure(Crc::ConstCRC("EditedGoal_ExtraParams_SkateTris"));
			break;
		case 0x0fc6f698: // ComboSkateTris
			p_extra_params=Script::GetStructure(Crc::ConstCRC("EditedGoal_ExtraParams_ComboSkateTris"));
			break;
		case 0xea7ba666: // TrickTris	
			p_extra_params=Script::GetStructure(Crc::ConstCRC("EditedGoal_ExtraParams_TrickTris"));
			break;
		case 0x61c5d092: // Gap
			p_extra_params=Script::GetStructure(Crc::ConstCRC("EditedGoal_ExtraParams_Gap"));
			break;
		default:
			break;
	}
...
void CEditGoal::Clear()
{
	...
	m_control_type=Crc::ConstCRC("skate");
...
void CEditGoal::RefreshPositionsUsingCollisionCheck()
{
	for (int i=0; i<m_num_positions_set; ++i)
	{
		...
		Script::RunScript(Crc::ConstCRC("goal_editor_refresh_goal_object_position"),&params);
	}	
}
...
void CEditGoal::add_goal_params(Script::CStruct *p_params)
{
	...
	switch (m_goal_type)
	{
		case 0x54166acd: // Skate
		{
			p_params->AddChecksum(Crc::ConstCRC("S_obj_id"),s_generate_node_name_checksum(m_level_goal_index,2));
			p_params->AddChecksum(Crc::ConstCRC("K_obj_id"),s_generate_node_name_checksum(m_level_goal_index,3));
			p_params->AddChecksum(Crc::ConstCRC("A_obj_id"),s_generate_node_name_checksum(m_level_goal_index,4));
			p_params->AddChecksum(Crc::ConstCRC("T_obj_id"),s_generate_node_name_checksum(m_level_goal_index,5));
			p_params->AddChecksum(Crc::ConstCRC("E_obj_id"),s_generate_node_name_checksum(m_level_goal_index,6));
			break;
		}
		...
		case 0x4ec3cfb5: // Combo
		{
			...
			p_struct->AddChecksum(Crc::ConstCRC("obj_id"),s_generate_node_name_checksum(m_level_goal_index,2));
			p_struct->AddString(Crc::ConstCRC("text"),"C");
			p_array->SetStructure(0,p_struct);

			p_struct=new Script::CStruct;
			p_struct->AddChecksum(Crc::ConstCRC("obj_id"),s_generate_node_name_checksum(m_level_goal_index,3));
			p_struct->AddString(Crc::ConstCRC("text"),"O");
			p_array->SetStructure(1,p_struct);

			p_struct=new Script::CStruct;
			p_struct->AddChecksum(Crc::ConstCRC("obj_id"),s_generate_node_name_checksum(m_level_goal_index,4));
			p_struct->AddString(Crc::ConstCRC("text"),"M");
			p_array->SetStructure(2,p_struct);

			p_struct=new Script::CStruct;
			p_struct->AddChecksum(Crc::ConstCRC("obj_id"),s_generate_node_name_checksum(m_level_goal_index,5));
			p_struct->AddString(Crc::ConstCRC("text"),"B");
			p_array->SetStructure(3,p_struct);

			p_struct=new Script::CStruct;
			p_struct->AddChecksum(Crc::ConstCRC("obj_id"),s_generate_node_name_checksum(m_level_goal_index,6));
			p_struct->AddString(Crc::ConstCRC("text"),"O");
			p_array->SetStructure(4,p_struct);
			
			p_params->AddArrayPointer(Crc::ConstCRC("letter_info"),p_array);
			break;
		}	
	...
	if (mp_win_message[0])
	{
		Script::CArray *p_default_success_cam_anims=Script::GetArray(Crc::ConstCRC("EditedGoal_success_cam_anims"),Script::ASSERT);
		...
		p_success_cam_anims->GetStructure(0)->AddString(Crc::ConstCRC("cam_anim_text"), mp_win_message);
		p_success_cam_anims->GetStructure(0)->AddChecksum(Crc::ConstCRC("targetid"),s_generate_node_name_checksum(m_level_goal_index,0));
			
		p_params->AddArrayPointer(Crc::ConstCRC("success_cam_anims"), p_success_cam_anims);
	}

	p_params->AddString(Crc::ConstCRC("goal_name"),GetIDString());
	
	if (mp_goal_name[0])
	{
		p_params->AddString(Crc::ConstCRC("view_goals_text"),mp_goal_name);
	}	
		
	if (mp_ped_name[0])
	{
		p_params->AddString(Crc::ConstCRC("full_name"),mp_ped_name);
	}	
	
	p_params->AddInteger(Crc::ConstCRC("time"),m_time_limit);
	p_params->AddChecksum(Crc::ConstCRC("control_type"),m_control_type);		
	
	
	// This is so that the goal manager can tell that this is an edited goal, and hence not save its
	// details to the mem card.
	p_params->AddChecksum(NONAME,Crc::ConstCRC("edited_goal"));
}
...
void CEditGoal::create_marker(int index)
{
	...
	switch (mp_item_positions[index].mType)
	{
		...
		case CGoalPos::RESTART:
			params.AddChecksum(Crc::ConstCRC("Type"),Crc::ConstCRC("Restart"));
			params.AddString("Model","gameobjects\\p1_cursor\\p1_cursor.mdl");
			break;
					
		case CGoalPos::LETTER:
		{
			params.AddChecksum(Crc::ConstCRC("Type"),Crc::ConstCRC("Letter"));
			...
			params.AddChecksum(NONAME,Crc::ConstCRC("RotateAndHover"));
			break;
		}
		default:
			break;
...
{
	float default_min_dist_allowable=Script::GetFloat(Crc::ConstCRC("GoalEditor_DefaultMinDistBetweenPositions"));
	float letter_skater_min_dist_allowable=Script::GetFloat(Crc::ConstCRC("GoalEditor_MinDistBetweenLettersandSkater"));
...
void CEditGoal::WriteIntoStructure(Script::CStruct *p_info)
{
	...
	p_info->AddChecksum(Crc::ConstCRC("level"),m_level_script);
	p_info->AddInteger(Crc::ConstCRC("level_goal_index"),m_level_goal_index);
	p_info->AddChecksum(Crc::ConstCRC("goal_id"),GetId());
	p_info->AddInteger(Crc::ConstCRC("time_limit"),m_time_limit);
	p_info->AddInteger(Crc::ConstCRC("won_goal"),m_has_won_goal);
	...
	p_info->AddChecksum(Crc::ConstCRC("pro_name"),s_generate_node_name_checksum(m_level_goal_index,0));
	...
	if (m_num_positions_set)
	{
		...
		p_info->AddArrayPointer(Crc::ConstCRC("node_positions"),p_position_array);
	}	
}
...
void CEditGoal::ReadFromStructure(Script::CStruct *p_info)
{
	...
	p_info->GetInteger(Crc::ConstCRC("level_goal_index"),&m_level_goal_index);
	
	p_info->GetChecksum(Crc::ConstCRC("goal_type"),&m_goal_type);
	Dbg_MsgAssert(m_goal_type,("Zero m_goal_type"));
	// Calling SetType will set up m_num_positions_required
	SetType(m_goal_type);

	p_info->GetInteger(Crc::ConstCRC("score"),&m_score);

	p_info->GetInteger(Crc::ConstCRC("time_limit"),&m_time_limit);
	p_info->GetChecksum(Crc::ConstCRC("control_type"),&m_control_type);
	
	const char *p_string="";
	p_info->GetString(Crc::ConstCRC("view_goals_text"),&p_string);
...
void CEditGoal::generate_key_combos_array(Script::CStruct *p_info)
{
	if (!m_num_combo_sets)
	{
		...
		p_info->AddChecksum(Crc::ConstCRC("goal_tetris_key_combos"),Crc::ConstCRC("emergency_key_combos"));
		return;
	}
	...
	for (int i=0; i<m_num_combo_sets; ++i)
	{
		...
		for (uint32 j=0; j<p_key_combos->GetSize(); ++j)
		{
			...
			{
				p_entry->AddChecksum(Crc::ConstCRC("key_combo"),p_key_combos->GetChecksum(j));
			}
			else
			{
			...
			{
				p_entry->AddInteger(Crc::ConstCRC("num_taps"),mp_combo_sets[i].mNumTaps);
			}
			if (mp_combo_sets[i].mRequirePerfect)
			{
				p_entry->AddChecksum(NONAME,Crc::ConstCRC("require_perfect"));
			}
			//if (mp_combo_sets[i].mSpin)
			//{
			//	p_entry->AddInteger(Crc::ConstCRC("random_spin"),mp_combo_sets[i].mSpin*180);
			//}
			...
			if (m_spin)
			{
				...
				p_entry->AddInteger(Crc::ConstCRC("random_spin"),m_spin);
			}
	...
	p_info->AddArrayPointer(Crc::ConstCRC("goal_tetris_key_combos"),p_goal_tetris_key_combos);
}
...
void CEditGoal::write_combo_sets(Script::CStruct *p_info)
{
	...
	for (int i=0; i<m_num_combo_sets; ++i)
	{
		...
		p_struct->AddInteger(Crc::ConstCRC("set_index"),mp_combo_sets[i].mSetIndex);
		if (mp_combo_sets[i].mRequirePerfect)
		{
			p_struct->AddChecksum(NONAME,Crc::ConstCRC("require_perfect"));
		}
		p_struct->AddInteger(Crc::ConstCRC("spin"),mp_combo_sets[i].mSpin*180);
		p_struct->AddInteger(Crc::ConstCRC("num_taps"),mp_combo_sets[i].mNumTaps);
...
void CEditGoal::write_skatetris_params(Script::CStruct *p_info, EWriteDest dest)
{
	...
	{
		p_info->AddInteger(Crc::ConstCRC("spin"),m_spin);
...
void CEditGoal::read_skatetris_params(Script::CStruct *p_info)
{
	...
	p_info->GetInteger(Crc::ConstCRC("spin"),&m_spin);
...
void CEditGoal::WriteGoalSpecificParams(Script::CStruct *p_info, EWriteDest dest)
{
	...
	{
		case 0xea7ba666: // TrickTris
			p_info->AddChecksum(NONAME,Crc::ConstCRC("tricktris"));
			p_info->AddInteger(Crc::ConstCRC("tricktris_block_size"),m_tricktris_block_size);
			p_info->AddInteger(Crc::ConstCRC("tricktris_total_to_win"),m_tricktris_total_to_win);
		...
		case 0x0fc6f698: // ComboSkateTris
			p_info->AddChecksum(NONAME,Crc::ConstCRC("combo"));
			p_info->AddInteger(Crc::ConstCRC("combo_size"),m_combo_size);
			if (m_single_combo)
			{
				p_info->AddChecksum(NONAME,Crc::ConstCRC("single_combo"));
		...
		case 0xec414b76: // HighCombo
		{
			...
			if (dest==WRITING_TO_GOAL_MANAGER)
			{
				...
				{
					p_text=Script::GetString(Crc::ConstCRC("edited_high_score_goal_text"));
				}	
				char p_temp[100];
				sprintf(p_temp,p_text,m_score);
				p_info->AddString(Crc::ConstCRC("goal_text"),p_temp);
			}
			break;
		}
		...
		case 0x61c5d092: // Gap
		{
			p_info->AddString(Crc::ConstCRC("required_trick_name"),mp_trick_name);
			...
			if (dest==WRITING_TO_GOAL_MANAGER)
			{
				...
				p_info->AddArrayPointer(Crc::ConstCRC("goal_flags"),p_goal_flags);
				...
				p_info->AddArrayPointer(Crc::ConstCRC("required_gaps"),p_required_gaps);
			}
			else
			{
				if (m_num_gaps)
				{
					...
					p_info->AddArrayPointer(Crc::ConstCRC("Gaps"),p_gaps);
				}	
			}
			break;
		}
...
void CEditGoal::ReadGoalSpecificParams(Script::CStruct *p_params)
{
	...
	{
		case 0xea7ba666: // TrickTris
			p_params->GetInteger(Crc::ConstCRC("tricktris_block_size"),&m_tricktris_block_size);
			p_params->GetInteger(Crc::ConstCRC("tricktris_total_to_win"),&m_tricktris_total_to_win);
			read_skatetris_params(p_params);
			break;
			
		case 0x0fc6f698: // ComboSkateTris
			p_params->GetInteger(Crc::ConstCRC("combo_size"),&m_combo_size);
			...
			if (p_params->ContainsFlag(Crc::ConstCRC("single_combo")))
			{
				m_single_combo=true;
			}	
		...
		case 0xec414b76: // HighCombo
		{
			p_params->GetInteger(Crc::ConstCRC("score"),&m_score);
			break;
		}
...
void CEditGoal::RemoveGoalSpecificFlag(Script::CStruct *p_params)
{
	...
	{
		case 0x0fc6f698: // ComboSkateTris
			if (flag==Crc::ConstCRC("single_combo"))
			{
				m_single_combo=false;
			}	
			break;
...
bool CEditGoal::SetPosition(const Mth::Vector& pos, float height, float angle)
{
	...
	if (m_current_position_index==0)
	{
		...
		Script::RunScript(Crc::ConstCRC("create_cag_helper_text"));
	}
...
bool CEditGoal::BackUp(const Mth::Vector& pos, float height, float angle)
{
	...
	if (m_current_position_index == 0)
	{
		...
		p_params->AddChecksum(NONAME,Crc::ConstCRC("PedPosition"));
		Script::RunScript(Crc::ConstCRC("create_cag_helper_text"), p_params);
		delete p_params;
	}	
...
void CEditGoal::SetType(uint32 type)
{
	...
	if (p_default_params)
	{
		p_default_params->GetInteger(Crc::ConstCRC("time_limit"),&m_time_limit);
		p_default_params->GetInteger(Crc::ConstCRC("score"),&m_score);
		
		const char *p_string="";
		p_default_params->GetString(Crc::ConstCRC("view_goals_text"),&p_string);
		Dbg_MsgAssert(strlen(p_string)<GOAL_NAME_BUFFER_SIZE,("goal name '%s' too long",p_string));
		strcpy(mp_goal_name,p_string);
	}	
}
...
void CEditGoal::write_position_into_node(uint index, uint32 nodeName)
{
	...
	p_node->AddVector(Crc::ConstCRC("Pos"),	mp_item_positions[index].mPos.GetX(),
												mp_item_positions[index].mPos.GetY()+mp_item_positions[index].mHeight,
												mp_item_positions[index].mPos.GetZ());
	p_node->AddVector(Crc::ConstCRC("Angles"),0,
	...
	if (index >=2 )
	{
		p_node->AddChecksum(Crc::ConstCRC("Type"),s_generate_letter_node_type_checksum(m_goal_type,index-2));
		p_node->AddString(Crc::ConstCRC("Model"),s_generate_letter_model_filename(m_goal_type,index-2));
	}	
}
...
void CGoalEditorComponent::WriteIntoStructure(Script::CStruct *p_info)
{
	...
	for (int i=0; i<MAX_GOALS_TOTAL; ++i)
	{
		...
		if (mp_goals[i].Used() && mp_goals[i].GetLevel() != Crc::ConstCRC("Load_Sk5Ed"))
		{
			++goal_count;
		}
	}		
	...
	p_info->AddArrayPointer(Crc::ConstCRC("Goals"),p_goals_array);
}
...
void CGoalEditorComponent::ReadFromStructure(Script::CStruct *p_info, EBoolLoadingParkGoals loadingParkGoals)
{
	...
	for (uint32 array_index=0; array_index<array_size; ++array_index)
	{
		...
		if ( (loadingParkGoals && s_get_level_checksum(p_struct) == Crc::ConstCRC("Load_Sk5Ed")) ||
			 (!loadingParkGoals && s_get_level_checksum(p_struct) != Crc::ConstCRC("Load_Sk5Ed")))
		{
			for (int i=0; i<MAX_GOALS_TOTAL; ++i)
			{
				if (!mp_goals[i].Used())
...
void CGoalEditorComponent::Hide( bool shouldHide )
{
	...
	else
	{
		...
		Script::CStruct *p_params=new Script::CStruct;
		p_params->AddInteger(Crc::ConstCRC("player"),0);
...
{
	return (Obj::CCompositeObject*)Obj::CTracker::Instance()->GetObj(Crc::ConstCRC("GoalEditorCursor"));
}
...
void CGoalEditorComponent::refresh_cursor_object()
{
	...
	{
		params.AddChecksum(Crc::ConstCRC("Type"),0);
		params.AddString(Crc::ConstCRC("Model"),p_model_filename);
	}
	
	Script::RunScript(Crc::ConstCRC("goal_editor_create_cursor"),&params,GetObj());
...
{
	uint32 id=0;
	if (p_params->GetChecksum(Crc::ConstCRC("goal_id"),&id))
	{
		return find_goal(id);
	}
	...
	int index=0;
	if (p_params->GetInteger(Crc::ConstCRC("goal_index"),&index))
	{
...
void CGoalEditorComponent::update_cursor_position()
{
	...
	params.AddVector(Crc::ConstCRC("Pos"),cursor_pos[X],cursor_pos[Y],cursor_pos[Z]);
	params.AddFloat(Crc::ConstCRC("Angle"),mp_editor_camera_component->GetCursorOrientation());
	
	Script::RunScript(Crc::ConstCRC("goal_editor_update_cursor_position"),&params);
}
...
CBaseComponent::EMemberFunctionResult CGoalEditorComponent::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	switch ( Checksum )
	{
		...
		case 0xcca11c78: // AddKeyComboSet
		{
			...
			{
				int set_index=0;
				pParams->GetInteger(Crc::ConstCRC("set_index"),&set_index);
				int spin=0;
				pParams->GetInteger(Crc::ConstCRC("spin"),&spin);
				int num_taps=0;
				pParams->GetInteger(Crc::ConstCRC("num_taps"),&num_taps);
				p_goal->AddKeyComboSet(set_index,
									   pParams->ContainsFlag(Crc::ConstCRC("require_perfect")),
									   spin,
									   num_taps);
			}	
			break;
		...
		case 0xe5ed63b7: // GetKeyComboSet
		{
			...
			if (p_goal)
			{
				...
				if (p_keycombo_set)
				{
					pScript->GetParams()->AddInteger(Crc::ConstCRC("Spin"),p_keycombo_set->mSpin*180);
					pScript->GetParams()->AddInteger(Crc::ConstCRC("num_taps"),p_keycombo_set->mNumTaps);
					if (p_keycombo_set->mRequirePerfect)
					{
						pScript->GetParams()->AddChecksum(NONAME,Crc::ConstCRC("require_perfect"));
					}	
				}
				else
				{
		...
		case 0x96874358: // GetNumEditedGoals
		{
			...
			int n=CountGoalsForLevel(level);
			if (level==0 && pParams->ContainsFlag(Crc::ConstCRC("ExcludeParkEditorGoals")))
			{
				n-=CountGoalsForLevel(Crc::ConstCRC("Load_Sk5Ed"));
				Dbg_MsgAssert(n>=0,("More editor goals (%d) than total goals? (%d)",CountGoalsForLevel(Crc::ConstCRC("Load_Sk5Ed")),CountGoalsForLevel(0)));
			}
		...
		case 0xb525b113: // AddEditedGoalToGoalManager
		{
			...
			if (p_goal)
			{
				p_goal->AddGoalToGoalManager(pParams->ContainsFlag(Crc::ConstCRC("MarkUnbeaten")));
			}	
			break;
		}
		...
		{
			if (pParams->ContainsFlag(Crc::ConstCRC("OnlyParkEditorGoals")))
			{
				ClearOnlyParkGoals();
			}
			else
		...
		{
			pScript->GetParams()->AddInteger(Crc::ConstCRC("max_goals"),MAX_GOALS_PER_LEVEL);
			break;
		}
		...
		case 0xec316a9c: // SetGoalControlType
		{
			...
			if (mp_current_goal)
			{
				uint32 control_type=Crc::ConstCRC("skate");
		...
		case 0x20b566b1: // RefreshGoalCursorPosition
		{
			if (mp_current_goal)
			{
				...
				Script::CStruct params;
				params.AddVector(Crc::ConstCRC("Pos"),cursor_pos[X],cursor_pos[Y]+height,cursor_pos[Z]);
				params.AddFloat(Crc::ConstCRC("Angle"),angle);
				Script::RunScript(Crc::ConstCRC("goal_editor_update_cursor_position"),&params);
			}	
			break;
		}
...
{
	Obj::CCompositeObject *p_obj=(Obj::CCompositeObject*)Obj::CCompositeObjectManager::Instance()->GetObjectByID(Crc::ConstCRC("GoalEditor"));
