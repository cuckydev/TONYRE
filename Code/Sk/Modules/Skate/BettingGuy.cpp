...
void CBettingGuy::SetupParams()
{
	...
	pBetParams->GetInteger( Crc::ConstCRC("bet_amount"), &m_betAmount, Script::ASSERT );
	
	mp_params->RemoveComponent( Crc::ConstCRC("bet_amount") );
	mp_params->AddInteger( Crc::ConstCRC("bet_amount"), m_betAmount );
	
	mp_params->RemoveComponent( Crc::ConstCRC("current_challenge") );
	if ( pBetParams->GetInteger( Crc::ConstCRC("bet_challenge"), &m_currentChallenge, Script::NO_ASSERT ) )
	{
		// m_currentChallenge = current_challenge;
		printf("got a challenge: %i\n", m_currentChallenge);
		mp_params->AddInteger( Crc::ConstCRC("current_challenge"), m_currentChallenge );
	}

	int max_attempts;
	pBetParams->GetInteger( Crc::ConstCRC("tries"), &max_attempts, Script::ASSERT );
	...
	if ( p_minigameParams->ContainsFlag( Crc::ConstCRC("trickspot") ) )
	{
		Script::CArray* p_keyCombos;
		pBetParams->GetArray( Crc::ConstCRC("key_combos"), &p_keyCombos, Script::ASSERT );
		...
		mp_params->AddString( Crc::ConstCRC("location"), location );
	}
	else 
	{
