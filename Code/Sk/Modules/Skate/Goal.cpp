...
void CGoal::Init()
{
	...
	mp_params->GetInteger( Crc::ConstCRC("unlimited_time"), &unlimited_time );
	if ( unlimited_time )
	{
		m_unlimitedTime = true;
	}
...
void CGoal::CreateGoalFlags( Script::CStruct* pParams )
{
    ...
    mp_goalFlags->Clear();
	m_numflags = 0;
	if ( pParams->GetArray( Crc::ConstCRC("goal_flags"), &pTempArray ) )
    {
	...
	mp_params->AddInteger( Crc::ConstCRC("num_flags"), m_numflags );
}
...
void CGoal::SetTimer()
{
	...
	if ( IsMinigame() )
	{
		if ( !mp_params->ContainsComponentNamed( Crc::ConstCRC("time") ) )
			return;
	}
    ...
    m_timeLeft = 120;
	int time;
	if ( mp_params->GetInteger( Crc::ConstCRC("time"), &time, Script::NO_ASSERT ) )
	{
...
/*	else
	{
		...
		if ( mp_params->GetArray( Crc::ConstCRC("time"), &pTimeLimits, Script::NO_ASSERT ) )
		{
			CGoalManager* pGoalManager = GetGoalManager();
			Dbg_Assert( pGoalManager );
...
bool CGoal::Activate()
{
    if ( !IsActive() )
	{
		...
		mp_params->RemoveFlag( Crc::ConstCRC("combo_started") );
...
bool CGoal::Deactivate( bool force, bool affect_tree )
{
	...
	if ( IsActive() )
	{
		...
		if ( IsSpecialGoal() )
			mp_params->RemoveComponent( Crc::ConstCRC("key_combos") );
...
bool CGoal::Win( void )
{
	mp_params->AddInteger( Crc::ConstCRC("win_record"), 0 );
	...
	if ( mp_children->m_relative == 0 )
	{
		...
		uint32 record_type;
		if ( !gamenet_man->InNetGame() && mp_params->GetChecksum( Crc::ConstCRC("record_type"), &record_type, Script::NO_ASSERT ) )
		{
			...
			if ( record_type == Crc::ConstCRC("score") )
			{
				...
				int custom_record;
				if ( mp_params->GetInteger( Crc::ConstCRC("custom_score_record"), &custom_record, Script::NO_ASSERT ) )
				{
					if ( custom_record > m_winScoreRecord )
					{
						m_winScoreRecord = custom_record;
						mp_params->AddInteger( Crc::ConstCRC("win_record"), m_winScoreRecord );
						set_new_record = true;
					}
				}
				else
				{
					...
					int total_score;
					if ( mp_params->ContainsFlag( Crc::ConstCRC("high_combo") ) )
			...
			else if ( record_type == Crc::ConstCRC("time") )
			{
				...
				if ( m_elapsedTime < m_elapsedTimeRecord )
				{
					...
					mp_params->AddInteger( Crc::ConstCRC("win_record"), (int)m_elapsedTimeRecord );
					set_new_record = true;
				}
			}
		}
		...
		if ( !HasWonGoal() && !gamenet_man->InNetGame() )
		{
			...
			if ( set_new_record )
				p_reward_params->AddChecksum( NONAME, Crc::ConstCRC("set_new_record") );
	
			mp_params->AddStructurePointer( Crc::ConstCRC("reward_params"), p_reward_params );
	
			UnlockRewardGoals();
		}
		else
			mp_params->RemoveComponent( Crc::ConstCRC("reward_params") );
		
		// update set_new_record flag
		if ( set_new_record )
			mp_params->AddChecksum( NONAME, Crc::ConstCRC("set_new_record") );
		else
			mp_params->RemoveFlag( Crc::ConstCRC("set_new_record") );
		...
		if( ShouldUseTimer()  )
		{
			if ( ( (int)m_timeLeft >= 1 ) )
			{
				...
				if ( (int)m_timeLeft < 10000 && ( (int)( old_time_left / 1000 ) > (int)( (int)m_timeLeft / 1000 ) ) )
				{
					...
					{
						Script::RunScript( Crc::ConstCRC("timer_runout_beep") );
					}
				}
			}
		}
		...
		if ( ShouldRequireCombo() )
		{
			if ( !mp_params->ContainsFlag( Crc::ConstCRC( "combo_started" ) ) )
			{
				...
				if ( pSkater )
				{
					...
					{
						mp_params->AddChecksum( NONAME, Crc::ConstCRC("combo_started") );
					}
				}
			}
		}
...
bool CGoal::SetGoalFlag( uint32 flag, int value )
{
	...
	if ( mp_goalFlags->ContainsComponentNamed( flag ) )
	{
		...
		if ( old_value != value )
		{
			...
			{
				m_flagsSet++;
				if ( !mp_params->ContainsFlag( Crc::ConstCRC("no_midgoal_sound") ) )
					Script::RunScript( Crc::ConstCRC("goal_got_flag_sound") );
			}
			if ( mp_params->ContainsFlag( Crc::ConstCRC("use_hud_counter") ) )
				Script::RunScript( Crc::ConstCRC("goal_update_counter"), mp_params );

			mp_params->AddInteger( Crc::ConstCRC("num_flags_set"), m_flagsSet );
		}		
...
bool CGoal::GetCreatedGoalGap(int gapIndex)
{
	...
	mp_params->GetString(Crc::ConstCRC("required_trick_name"),&p_required_trick_name);
...
//    printf("%i flags, %i set\n", m_numflags, m_flagsSet);
	...
	mp_params->GetInteger( Crc::ConstCRC("num_flags_to_win"), &num_flags_to_win, Script::NO_ASSERT );
...
bool CGoal::NextTourSpot()
{
	...
	if ( !mp_params->GetArray( Crc::ConstCRC("tour_spots"), &p_waypoints, Script::NO_ASSERT ) )
		mp_params->GetArray( Crc::ConstCRC("horse_spots"), &p_waypoints, Script::ASSERT );
    ...
    for ( uint32 i = 0; i < p_waypoints->GetSize(); i++ )
    {
        ...
        uint32 flagId;
        p_waypoint->GetChecksum( Crc::ConstCRC("flag"), &flagId, Script::ASSERT );
        ...
        PauseGoal();
		int time;
		p_waypoint->GetInteger( Crc::ConstCRC("time"), &time, Script::ASSERT );
        // add time if necessary
		if ( mp_params->ContainsFlag( Crc::ConstCRC("cumulative_timer") ) )
		{
			AddTime( time );
		}
		else
		{
			mp_params->AddInteger( Crc::ConstCRC("time"), time );
			SetTimer();
		}
            
        // add the goalId and trigger id to the params
        p_waypoint->AddChecksum( Crc::ConstCRC("goal_id"), GetGoalId() );
		...
		if ( mp_params->ContainsFlag( Crc::ConstCRC("quick_start") ) )
			p_waypoint->AddChecksum( NONAME, Crc::ConstCRC("quick_start") );
		else
			p_waypoint->RemoveFlag( Crc::ConstCRC("quick_start") );
		...
		if ( mp_params->ContainsFlag( Crc::ConstCRC("horse") ) )
			...
			p_script=Script::SpawnScript( Crc::ConstCRC("goal_tour_next_spot"), p_waypoint );
		p_script->SetCommentString("Spawned from CGoal::NextTourSpot()");
		#else	
		// backward compatible with horse goal
		if ( mp_params->ContainsFlag( Crc::ConstCRC("horse") ) )
			Script::SpawnScript( Crc::ConstCRC("goal_horse_next_spot"), p_waypoint );
		else
			Script::SpawnScript( Crc::ConstCRC("goal_tour_next_spot"), p_waypoint );
		#endif			
...
void CGoal::InitTrickObjects()
{
    ...
    for ( uint32 i = 0; i < p_trickObjectArray->GetSize(); i++ )
    {
        ...
        p_trickObject->GetChecksum( Crc::ConstCRC("id"), &goal_object, Script::ASSERT );
...
{
	return ( mp_params->ContainsFlag( Crc::ConstCRC("graffiti") ) );
}
...
{
	if ( mp_params->ContainsFlag( Crc::ConstCRC("graffiti") ) || mp_params->ContainsFlag( Crc::ConstCRC("SkateTheLine") ) )
	{
		return true;
	}
	return false;
...
void CGoal::GotGraffitiCluster( uint32 clusterId, int score )
{
    ...
    mp_params->GetArray( Crc::ConstCRC("kill_clusters"), &pClusterArray, Script::ASSERT );
    ...
    for ( uint32 i = 0; i < pClusterArray->GetSize(); i++ )
    {
		...
		if ( !pCluster->GetInteger( Crc::ConstCRC("score"), &minScore, Script::NO_ASSERT ) )
			mp_params->GetInteger( Crc::ConstCRC("score"), &minScore, Script::ASSERT );
        ...
        if ( id == clusterId && score > minScore )
        {
			...
			uint32 flag;
            pCluster->GetChecksum( Crc::ConstCRC("flag"), &flag, Script::ASSERT );
...
{
    return ( mp_params->ContainsFlag( Crc::ConstCRC("special") ) );
}
...
bool CGoal::CheckSpecialGoal()
{
    ...
    mp_params->GetString( Crc::ConstCRC("trick_string"), &trick_string, Script::ASSERT );
...
{
    return ( mp_params->ContainsFlag( Crc::ConstCRC("tetris") ) );
}
...
int CGoal::GetRandomIndexFromKeyCombos( Script::CArray* p_keyCombos )
{
	...
	{        
		...
		if ( p_keyCombos->GetType() == ESYMBOLTYPE_NAME )
		{
			...
			p_keyCombos->GetStructure( i )->GetChecksum( Crc::ConstCRC("key_combo"), &key_combo, Script::ASSERT );
		...
		if ( max_tries == 0 )
		{
			...
			for ( uint32 j = 0; j < size; j++ )
			{
				if ( p_keyCombos->GetType() == ESYMBOLTYPE_NAME )
					...
					p_keyCombos->GetStructure( j )->GetChecksum( Crc::ConstCRC("key_combo"), &key_combo, Script::ASSERT );
				p_trickMappings->GetChecksum( key_combo, &trick_name_checksum, Script::NO_ASSERT );
				if ( trick_name_checksum )
				{
					return i;
...
void CGoal::GotCounterObject()
{
    ...
    mp_params->GetInteger( Crc::ConstCRC("number_collected"), &number_collected, Script::NO_ASSERT );
    // printf("alredy collected %i\n", number_collected);
    number_collected++;

    mp_params->AddInteger( Crc::ConstCRC("number_collected"), number_collected );
...
bool CGoal::CounterGoalDone()
{
    ...
    int number;
    mp_params->GetInteger( Crc::ConstCRC("number"), &number, Script::ASSERT );
...
bool CGoal::CheckMinigameRecord( int value )
{
	...
	mp_params->AddInteger( Crc::ConstCRC("last_attempt"), value );
	if ( value > record )
	{
		mp_params->AddInteger( Crc::ConstCRC("minigame_record"), value );
		return true;
	}
	return false;
}
...
int CGoal::GetMinigameRecord()
{
	...
	mp_params->GetInteger( Crc::ConstCRC("minigame_record"), &minigame_record, Script::NO_ASSERT );
...
bool CGoal::CanRetry()
{
	return ( !mp_params->ContainsFlag( Crc::ConstCRC("no_restart") ) );
}
...
{
	return ( mp_params->ContainsFlag( Crc::ConstCRC("minigame") ) );
}
...
{
	mp_params->AddInteger( Crc::ConstCRC("current_time"), 0 );
}
...
void CGoal::UpdateComboTimer()
{
	if ( !IsPaused() )
	{
		...
		int time;
		mp_params->GetInteger( Crc::ConstCRC("current_time"), &time, Script::NO_ASSERT );
		current_time = (Tmr::Time)time;
	
		current_time +=  (Tmr::Time)( Tmr::FrameLength() * 1000 );
	
		mp_params->AddInteger( Crc::ConstCRC("current_time"), (int)current_time );

		// add normal units for display
		mp_params->AddInteger( Crc::ConstCRC("current_time_seconds"), (int)( current_time / 1000 ) );
		mp_params->AddInteger( Crc::ConstCRC("current_time_fraction"), (int)( ( current_time / 10 ) % 100 ) );
	}
}
...
void CGoal::SetStartHeight()
{
	...
	mp_params->AddInteger( Crc::ConstCRC("start_height"), start_height );
...
bool CGoal::CheckHeightRecord()
{
	if ( !IsPaused() )
	{
		...
		mp_params->GetInteger( Crc::ConstCRC("start_height"), &start_height, Script::NO_ASSERT );
		...
		mp_params->AddInteger( Crc::ConstCRC("current_height_feet"), feet );
		mp_params->AddInteger( Crc::ConstCRC("current_height_inches"), inches );
		
		if ( current_height > record )
		{
			mp_params->AddInteger( Crc::ConstCRC("minigame_record"), current_height );
...
//	pObj = Obj::CMovingObject::m_hash_table.GetItem( objId );
	...
	if ( pSkater && pObj )
	{
		...
		mp_params->AddInteger( Crc::ConstCRC("current_distance_feet"), feet );
		mp_params->AddInteger( Crc::ConstCRC("current_distance_inches"), inches );
...
{
	mp_params->RemoveFlag( Crc::ConstCRC("quick_start") );
}
...
{
	return ( mp_params->ContainsComponentNamed( Crc::ConstCRC("pro_goal") ) );
}
...
{
	return ( mp_params->ContainsFlag( Crc::ConstCRC("competition") ) );
}
...
{
	return ( mp_params->ContainsFlag( Crc::ConstCRC("betting_game") ) );
}
...
{
	return ( mp_params->ContainsFlag( Crc::ConstCRC("betting_guy") ) );
}
...
bool CGoal::AddTempSpecialTrick()
{
	...
	uint32 trickType;
	bool is_cat = mp_params->ContainsFlag( Crc::ConstCRC("create_a_trick") );
	...
	if ( is_cat )
	{
		Script::CArray* pPremadeCatArray = Script::GetArray( Crc::ConstCRC("Premade_CATS"), Script::ASSERT );
		Script::CStruct* pPremadeCat = pPremadeCatArray->GetStructure( trickName );
		const char* pPremadeCatName;
		pPremadeCat->GetString( Crc::ConstCRC("name"), &pPremadeCatName, Script::ASSERT );
		mp_params->AddString( Crc::ConstCRC("trick_string"), pPremadeCatName );
	}
	else
	{
	...
	// Script::CStruct* p_trickMapping = p_SkaterProfile->GetTrickMapping( Crc::ConstCRC("trick_mapping") );
	...
	if ( !found_trick )
	{
		...
		if ( is_cat )
		{
			...
			pScriptParams->AddChecksum( Crc::ConstCRC("trickSlot"), trickSlot );
			pScriptParams->AddInteger( Crc::ConstCRC("index"), trickName );
			pScriptParams->AddInteger( Crc::ConstCRC("special_trick_index"), numTricks );
			Script::RunScript( Crc::ConstCRC("load_premade_cat"), pScriptParams );
...
void CGoal::RemoveTempSpecialTrick()
{
	...
	Dbg_MsgAssert( (int)pSkaterProfile->GetNumSpecialTrickSlots() <= Script::GetInteger( Crc::ConstCRC("max_number_of_special_trickslots") ), ( "You have too many trickslots!" ) );
...
*/
	...
	uint32 team_pro;
	pFlags->GetChecksum( Crc::ConstCRC("team_pro"), &team_pro, Script::ASSERT );
	if ( team_pro != 0 )
	{
		mp_params->AddChecksum( Crc::ConstCRC("team_pro"), team_pro );
	}
}
...
*/
	...
	uint32 team_pro = 0;
	mp_params->GetChecksum( Crc::ConstCRC("team_pro"), &team_pro, Script::NO_ASSERT );
	pFlags->AddChecksum( Crc::ConstCRC("team_pro"), team_pro );
}
...
{	
	...
	Script::CStruct* p_key_combo_map = Script::GetStructure( Crc::ConstCRC("goal_tetris_trick_text"), Script::ASSERT );
	...
	for ( int i = 0; i < size; i++ )
	{
		...
		else
		{
			...
			{
				Script::CArray* pPremadeCats = Script::GetArray( Crc::ConstCRC("Premade_CATS"), Script::ASSERT );
				Script::CStruct* pPremadeCat = pPremadeCats->GetStructure( premade_cat_index );
				pPremadeCat->GetString( Crc::ConstCRC("name"), &p_trick_name, Script::ASSERT );
			}
			else
			{
				Script::CStruct* p_trick_struct = Script::GetStructure( trick_checksum, Script::ASSERT );
				Script::CStruct* p_trick_params;
				p_trick_struct->GetStructure( Crc::ConstCRC("Params"), &p_trick_params, Script::ASSERT );
...
int CGoal::GetNumberCollected()
{
	...
	if ( mp_params->GetInteger( Crc::ConstCRC("number_collected"), &number_collected, Script::NO_ASSERT ) )
...
void CGoal::GetRewardsStructure( Script::CStruct *p_structure, bool give_cash )
{
	...
	if( gamenet_man->InNetGame() == false )
	{
		...
		if ( mp_params->ContainsFlag( "reward_trickslot" ) && !mp_params->ContainsFlag( Crc::ConstCRC( "create_a_trick" ) ) )
		{
			...
			if ( num_slots <= Script::GetInteger( Crc::ConstCRC("max_number_of_special_trickslots") ) )
			{
...
{
	return mp_params->ContainsFlag( Crc::ConstCRC("horse") );
}
...
{
	return mp_params->ContainsFlag( Crc::ConstCRC("require_combo") );
}
