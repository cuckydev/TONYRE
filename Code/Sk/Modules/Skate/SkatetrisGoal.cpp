...
bool CSkatetrisGoal::Win()
{
	...
	if (testing_goal)
	{
		mp_params->AddChecksum(NONAME,Crc::ConstCRC("testing_goal"));
	}
...
bool CSkatetrisGoal::Update()
{
	...
	mp_params->GetInteger( Crc::ConstCRC("wait_to_add_tricks"), &wait_to_add_tricks, Script::NO_ASSERT );
	...
	{
		if ( mp_params->GetInteger( Crc::ConstCRC("time_to_stop_adding_tricks"), &minTimeLeft, Script::NO_ASSERT ) )
			minTimeLeft *= 1000;
	}
	...
	if ( IsTricktris() )
	{
		if ( IsActive() && !IsPaused() )
		{
			...
			mp_params->GetInteger( Crc::ConstCRC("tricks_completed"), &tricks_completed, Script::NO_ASSERT );
			int total_to_win;
			mp_params->GetInteger( Crc::ConstCRC("tricktris_total_to_win"), &total_to_win, Script::ASSERT );
	...
	if ( IsTricktris() )
	{
		if ( ( m_unlimitedTime || (int)m_timeLeft > 0.0f ) && !IsPaused() )
		{
			...
			mp_params->GetInteger( Crc::ConstCRC("tricktris_block_size"), &desired_stack_size, Script::ASSERT );
	...
	else if ( IsActive() && ( m_unlimitedTime || (int)m_timeLeft >= minTimeLeft ) && !IsPaused() )
	{
		...
		if ( IsSingleComboSkatetris() )
		{
			...
			if ( !mp_params->ContainsFlag( Crc::ConstCRC("single_combo_added") ) && m_tetrisTime > 50 )
			{
				int combo_size;
				mp_params->GetInteger( Crc::ConstCRC("combo_size"), &combo_size, Script::ASSERT );
				AddTetrisTrick( combo_size );
				mp_params->AddChecksum( NONAME, Crc::ConstCRC("single_combo_added") );
			}
		}
		else
		{
			int trickTime;
			mp_params->GetInteger( Crc::ConstCRC("adjusted_trick_time"), &trickTime, Script::ASSERT );
...
void CSkatetrisGoal::EndTetrisGoal()
{
	...
	mp_params->RemoveFlag( Crc::ConstCRC("single_combo_added") );
}
...
{
    int max_tricks;
	mp_params->GetInteger( Crc::ConstCRC("max_tricks"), &max_tricks, Script::ASSERT );
...
/*	if ( add_combo )
	{
		mp_params->GetInteger( Crc::ConstCRC("combo_size"), &tricks_in_group, Script::ASSERT );
    ...
    else 
    {
		...
		while ( tricks_in_group > 0 )
		{
			...
			if ( !p_keyCombos )
			{
				...
				const char* p_text;
				p_trick->GetString( Crc::ConstCRC("text"), &p_text, Script::ASSERT );
				strcpy( trick_text, p_text );

				if ( !p_trick->GetChecksum( Crc::ConstCRC("key_combo"), &key_combo, Script::NO_ASSERT ) )
					p_trick->GetString( Crc::ConstCRC("key_combo"), &p_key_combo_text, Script::NO_ASSERT );
			...
			else
			{
				...
				else
				{
					...
					p_subStruct->GetChecksum( Crc::ConstCRC("key_combo"), &key_combo, Script::ASSERT );
					
					int spin_mult = 0;
					if ( p_subStruct->GetInteger( Crc::ConstCRC("random_spin"), &spin_mult, Script::NO_ASSERT ) )
					{
					...
					if ( p_subStruct->GetInteger( Crc::ConstCRC("spin"), &spin_mult, Script::NO_ASSERT ) )
					{
						// make sure it's a multiple of 180
						Dbg_MsgAssert( spin_mult % 180 == 0, ( "Skatetris goal %s has bad spin_mult value %i", Script::FindChecksumName( GetGoalId() ), spin_mult ) );
					...
					if ( p_subStruct->ContainsFlag( Crc::ConstCRC("perfect") ) )
						m_tetrisTricks[index].require_perfect = true;

					if ( p_subStruct->GetInteger( Crc::ConstCRC("num_taps"), &num_taps, Script::NO_ASSERT ) )
				...
				if ( !p_text )
				{
					...
					while ( num_taps > 1 )
					{
						...
						if ( p_trick_params->GetChecksum( Crc::ConstCRC("ExtraTricks"), &extra_trick, Script::NO_ASSERT ) )
						{
							...
							p_extra_trick_struct->GetStructure( Crc::ConstCRC("Params"), &p_trick_params, Script::ASSERT );
						}
						else
						{
							num_taps = 1;
					...
					p_trick_params->GetLocalText( Crc::ConstCRC("Name"), &p_text, Script::ASSERT );
				}
			...
			// set the parent
			Front::CScreenElement* p_vmenu = p_screen_elem_man->GetElement(Crc::ConstCRC("tetris_tricks_menu"), Front::CScreenElementManager::ASSERT );
			...
			if ( p_key_combo_text || key_combo )
			{
				...
				if ( m_tetrisTricks[index].num_taps == 2 )
					p_button_combos = Script::GetStructure( Crc::ConstCRC("goal_tetris_trick_text_double_tap"), Script::ASSERT );
				else
					p_button_combos = Script::GetStructure( Crc::ConstCRC("goal_tetris_trick_text"), Script::ASSERT );
			...
			if ( !IsTricktris() && !IsSingleComboSkatetris() && m_numTetrisTricks > ( .75 * (float)max_tricks ) )
				script = Crc::ConstCRC("goal_tetris_add_red_trick");
			...
			if ( !IsTricktris() && !IsSingleComboSkatetris() && m_numTetrisTricks > ( .75 * (float)max_tricks ) )
			{
				mp_params->AddInteger( Crc::ConstCRC("list_is_red"), 1 );
...
void CSkatetrisGoal::CheckTetrisTricks()
{
	...
	int trick_flag;
	if ( mp_params->GetInteger( Crc::ConstCRC("trick_flag"), &trick_flag, Script::NO_ASSERT ) )
	{
		if ( trick_flag == 0 )
			return;
		else 
			mp_params->AddInteger( Crc::ConstCRC("trick_flag"), 0 );
	}
...
void CSkatetrisGoal::CheckTetrisCombos()
{
	...
	int group_size;
	mp_params->GetInteger( Crc::ConstCRC("combo_size"), &group_size, Script::ASSERT );
	...
	for ( int group_id = 0; group_id < Game::vMAXTETRISTRICKS; group_id++ )
	{
		...
		if ( group_total >= group_size )
		{
			...
			for ( int j = 0; j < Game::vMAXTETRISTRICKS; j++ )
			{
				...
				{
					if ( !trick_removed )
						Script::RunScript( Crc::ConstCRC("goal_tetris_play_trick_removed_sound") );
					trick_removed = true;
...
{	
	...
	p_temp_params->AddChecksum( Crc::ConstCRC("id"), id );
	Script::RunScript( Crc::ConstCRC("goal_tetris_reset_trick_container"), p_temp_params );
	...
	int list_is_red = 0;
	mp_params->GetInteger( Crc::ConstCRC("list_is_red"), &list_is_red, Script::NO_ASSERT );
	if ( ( list_is_red == 1 ) && ( ( m_numTetrisTricks - 1 ) < ( .75 * (float)max_tricks ) ) )
	{
		mp_params->AddInteger( Crc::ConstCRC("list_is_red"), 0 );
		for ( int j = 0; j < Game::vMAXTETRISTRICKS; j++ )
		{
			if ( !m_tetrisTricks[j].invalid )
			{
	...
	// remove the trick
	script = Crc::ConstCRC("goal_tetris_remove_trick");
	Script::CStruct* pScriptParams = new Script::CStruct();
	
	// flag for removing tricktris tricks immediately
	if ( IsTricktris() )
		pScriptParams->AddChecksum( NONAME, Crc::ConstCRC("tricktris") );
	...
	if ( acceleration_interval && ( tricks_completed % acceleration_interval == 0 ) )
	{
		...
		mp_params->AddInteger( Crc::ConstCRC("adjusted_trick_time"), trick_time );
		// printf("setting trick time to %i\n", trick_time);
	}
	// store the number of tricks completed
	// printf("%i tricks completed\n", tricks_completed );
	mp_params->AddInteger( Crc::ConstCRC("tricks_completed"), tricks_completed );
...
{
	int combo_test = 0;
	mp_params->GetInteger( Crc::ConstCRC("combo"), &combo_test, Script::NO_ASSERT );
	return ( combo_test || mp_params->ContainsFlag( Crc::ConstCRC("combo") ) );
}
...
bool CSkatetrisGoal::IsSingleComboSkatetris()
{
	...
	mp_params->GetInteger( Crc::ConstCRC("single_combo"), &single_combo_test, Script::NO_ASSERT );
	return ( single_combo_test || mp_params->ContainsFlag( Crc::ConstCRC("single_combo") ) );
}
...
bool CSkatetrisGoal::IsTricktris()
{
	...
	mp_params->GetInteger( Crc::ConstCRC("tricktris"), &tricktris_test, Script::NO_ASSERT );
	return ( tricktris_test || mp_params->ContainsFlag( Crc::ConstCRC("tricktris") ) );
}
