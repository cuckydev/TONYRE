...
bool CGoalManager::AddGoal( uint32 goalId, Script::CStruct* pParams )
{
	...
	if ( pParams->ContainsFlag( Crc::ConstCRC("competition") ) )
	{
	...
	else if ( pParams->ContainsFlag( Crc::ConstCRC("race") ) )
	{
		// printf("adding race goal\n");
		pGoal = new CRaceGoal( pParams );
	}
	else if ( pParams->ContainsFlag( Crc::ConstCRC("minigame") ) )
	{
		// printf("adding minigame\n");
		pGoal = new CMinigame( pParams );
	}
	else if ( pParams->ContainsFlag( Crc::ConstCRC("betting_guy") ) )
	{
		// printf("adding betting guy\n");
		pGoal = new CBettingGuy( pParams );
	}
	else if ( pParams->ContainsFlag( Crc::ConstCRC("tetris") ) )
	{
		// printf("adding skatetris goal\n");
		pGoal = new CSkatetrisGoal( pParams );
	}
	else if ( pParams->ContainsFlag( Crc::ConstCRC("horse") ) )
	{
	...
	if ( CFuncs::ScriptInMultiplayerGame( nullptr, nullptr ))
	{
		...
		pGoal->GetParams()->AddString( Crc::ConstCRC("full_name"), Script::GetString( Crc::ConstCRC("judge_full_name") ) );
		// END SKATE SPECIFIC
	}
	else
	{
		...
		uint32 pro_name;
		if ( pGoal->GetParams()->GetString( Crc::ConstCRC("pro"), &pFirstName, Script::NO_ASSERT ) )
		{
			// printf("pro string: %s\n", pFirstName );
			Script::CStruct* pLastNames = Script::GetStructure( Crc::ConstCRC("goal_pro_last_name_checksums") );
			if ( pLastNames->GetChecksum( Script::GenerateCRC( pFirstName ), &pro_name, Script::NO_ASSERT ) )
			{
				...
				{
					const char* p_generic_pro_first_name = Script::GetString( Crc::ConstCRC("generic_pro_first_name"), Script::ASSERT );
					pGoal->GetParams()->AddString( Crc::ConstCRC("pro"), p_generic_pro_first_name );
					const char* p_generic_pro_full_name = Script::GetString( Crc::ConstCRC("generic_pro_full_name"), Script::ASSERT );
					pGoal->GetParams()->AddString( Crc::ConstCRC("full_name"), p_generic_pro_full_name );					
				}
			}
		}
		// END SKATE SPECIFIC
	...
	if ( pGoal )
	{
		...
		if ( !pGoal->HasWonGoal() )
		{
			...
			else
			{
				...
				if ( pGoal->GetChildren()->m_relative == 0 )
					if ( !pGoal->GetParams()->ContainsFlag( Crc::ConstCRC("edited_goal") ) )
						m_numGoalsBeaten++;
			}
...
void CGoalManager::RestartLastGoal() 
{
	if ( m_lastGoal != 0 )
    {
		...
		Script::RunScript(( Crc::ConstCRC("kill_blur") ) );
...
void CGoalManager::CheckTrickText()
{
	...
	if ( pGoal->IsSpecialGoal() || p_goal_params->ContainsFlag( Crc::ConstCRC("create_a_trick") ) )
    {		
...
bool CGoalManager::HasWonGoal( uint32 goalId )
{
	...
	else
	{
		...
		if ( mp_goalFlags->GetStructure( goalId, &pGoalParams, Script::NO_ASSERT ) )
			pGoalParams->GetInteger( Crc::ConstCRC("hasBeaten"), &hasWonGoal, Script::ASSERT );
...
void CGoalManager::LevelUnload()
{
	...
	p_manager_params->AddInteger( Crc::ConstCRC("goalPoints"), m_goalPoints );
	p_manager_params->AddInteger( Crc::ConstCRC("totalGoalPointsEarned"), m_totalGoalPointsEarned );
	p_manager_params->AddInteger( Crc::ConstCRC("cash"), m_cash );
	p_manager_params->AddInteger( Crc::ConstCRC("total_cash"), m_totalCash );
	// p_manager_params->AddInteger( "numGoalsToPro", m_numGoalsToPro );
	p_manager_params->AddInteger( Crc::ConstCRC("numGoalsBeaten"), m_numGoalsBeaten );

	p_manager_params->AddInteger( Crc::ConstCRC("currentChapter"), m_currentChapter );
	p_manager_params->AddInteger( Crc::ConstCRC("currentStage"), m_currentStage );
	p_manager_params->AddChecksum( Crc::ConstCRC("sponsor"), m_sponsor );
	p_manager_params->AddStructure( Crc::ConstCRC("team"), mp_team );
	p_manager_params->AddStructure( Crc::ConstCRC("difficulty_levels"), mp_difficulty_levels );
	
/*	int proChallengesUnlocked = 0;
	if ( m_proChallengesUnlocked )
		proChallengesUnlocked = 1;
	p_manager_params->AddInteger( Crc::ConstCRC("proChallengesUnlocked"), proChallengesUnlocked );
*/
	int isPro = 0;
	if ( m_isPro )
		isPro = 1;
	p_manager_params->AddInteger( Crc::ConstCRC("isPro"), isPro );

	// p_manager_params->AddStructure( Crc::ConstCRC("proSpecificChallenges"), mp_proSpecificChallenges );

	mp_goalFlags->AddStructure( Crc::ConstCRC("GoalManager_Params"), p_manager_params );
	...
	for ( int i = 0; i < GetNumGoals(); i++ )
    {
		...
		if ( !pGoalParams->ContainsFlag( Crc::ConstCRC("null_goal") ) && !pGoal->IsEditedGoal() ) // K: Don't add edited goals
		{
...
void CGoalManager::LevelLoad()
{
    ...
    while ( p_comp )
    {
		...
		if ( p_comp->mNameChecksum == Crc::ConstCRC( "GoalManager_Params" ) )
		{
			...
			p_comp->mpStructure->GetInteger( Crc::ConstCRC("cash"), &m_cash, Script::NO_ASSERT );
			p_comp->mpStructure->GetInteger( Crc::ConstCRC("total_cash"), &m_totalCash, Script::NO_ASSERT );
			// p_comp->mpStructure->GetInteger( "numGoalsToPro", &m_numGoalsToPro, Script::NO_ASSERT );
			p_comp->mpStructure->GetInteger( Crc::ConstCRC("numGoalsBeaten"), &m_numGoalsBeaten, Script::NO_ASSERT );

			int auto_change_chapter_and_stage = 0;
			auto_change_chapter_and_stage = Script::GetInteger( Crc::ConstCRC("auto_change_chapter_and_stage"), Script::NO_ASSERT );
			if ( auto_change_chapter_and_stage == 0 )
			{
				p_comp->mpStructure->GetInteger( Crc::ConstCRC("currentChapter"), &m_currentChapter, Script::NO_ASSERT );
				p_comp->mpStructure->GetInteger( Crc::ConstCRC("currentStage"), &m_currentStage, Script::NO_ASSERT );				
			}
			p_comp->mpStructure->GetChecksum( Crc::ConstCRC("sponsor"), &m_sponsor, Script::ASSERT );
			
			Script::CStruct* p_copy;
			p_comp->mpStructure->GetStructure( Crc::ConstCRC("team"), &p_copy, Script::ASSERT );
			mp_team->Clear();
			mp_team->AppendStructure( p_copy );

			Script::CStruct* p_diff_copy;
			p_comp->mpStructure->GetStructure( Crc::ConstCRC("difficulty_levels"), &p_diff_copy, Script::ASSERT );
			mp_difficulty_levels->Clear();
			mp_difficulty_levels->AppendStructure( p_diff_copy );
			
			int isPro;
			p_comp->mpStructure->GetInteger( Crc::ConstCRC("isPro"), &isPro, Script::ASSERT );
			if ( isPro != 0 )
				TurnPro();
			
/*			int proChallengesUnlocked;
			p_comp->mpStructure->GetInteger( Crc::ConstCRC("proChallengesUnlocked"), &proChallengesUnlocked, Script::ASSERT );
			if ( proChallengesUnlocked != 0 )
				m_proChallengesUnlocked = true;

			Script::CStruct* p_copy;
			p_comp->mpStructure->GetStructure( Crc::ConstCRC("proSpecificChallenges"), &p_copy, Script::ASSERT );
...
int CGoalManager::NumGoalsBeatenInLevel( int levelNum, bool count_pro_specific_challenges )
{
    ...
    while ( p_comp )
    {
        ...
        if ( p_comp->mpStructure->GetInteger( Crc::ConstCRC("levelNum"), &p_comp_levelNum, Script::NO_ASSERT ) )
        {
            // printf("found levelNum\n");
/*			if ( !count_pro_specific_challenges )
			{
				int isProSpecificChallenge;
				p_comp->mpStructure->GetInteger( Crc::ConstCRC("isProSpecificChallenge"), &isProSpecificChallenge, Script::ASSERT );
...
*/
			int hasBeaten = 0;
			p_comp->mpStructure->GetInteger( Crc::ConstCRC("hasBeaten"), &hasBeaten, Script::NO_ASSERT );
...
void CGoalManager::PlayGoalStream( Script::CStruct* pParams )
{
	...
	pParams->GetChecksum( Crc::ConstCRC("vo"), &stream_checksum, Script::NO_ASSERT );
	if ( stream_checksum )
	{
...
void CGoalManager::AddViewGoalsList()
{
	...
	for ( int i = 0; i < numGoals; i++ )
	{
		...
		if ( !IsPro() && pGoalParams->ContainsFlag( Crc::ConstCRC("pro_goal") ) )
			continue;

		// skip pro challenges if we haven't unlocked them.
		// if ( !ProSpecificChallengesUnlocked() && pGoalParams->ContainsFlag( Crc::ConstCRC("pro_specific_challenge") ) )
			// continue;
		
		// skip pro specific challenges if we're not the right skater
/*		if ( !pGoal->HasWonGoal() && !IsPro() && pGoalParams->ContainsFlag( Crc::ConstCRC("pro_specific_challenge") ) )
		{
			// check the current skater against the listed pro
			Script::CStruct* pLastNames = Script::GetStructure( Crc::ConstCRC("goal_pro_last_name_checksums"), Script::ASSERT );
			const char* pProChallengeProName;
			pGoalParams->GetString( Crc::ConstCRC("pro_challenge_pro_name"), &pProChallengeProName, Script::ASSERT );
			uint32 last_name = 0;
...
*/
		...
		if ( pGoal->HasWonGoal() )
		{
			...
			if ( pGoal->IsCompetition() )
			{
				...
				p_elem_params->AddString( Crc::ConstCRC("win_record_string"), record_string );
			}			
			else if ( pGoalParams->GetChecksum( Crc::ConstCRC("record_type"), &record_type, Script::NO_ASSERT ) )
			{
				...
				else if ( record_type == ( Crc::ConstCRC("time") ) )
				{
					int record = 0;
					pGoalParams->GetInteger( Crc::ConstCRC("win_record"), &record, Script::NO_ASSERT );
		...
		// create the item
		Script::RunScript( Crc::ConstCRC("view_goals_menu_add_item"), p_elem_params );
...
void CGoalManager::AddGoalChoices( bool selected_only )
{
	...
	for ( int i = 0; i < GetNumGoals(); i++ )
	{
		...
		if( selected_only )
		{
			Script::RunScript( Crc::ConstCRC("view_selected_goals_menu_set_events"), p_elem_params);
		}
		else
		{
			Script::RunScript( Crc::ConstCRC("choose_goals_menu_set_events"), p_elem_params);
		}
...
bool CGoalManager::GoalIsLocked( uint32 goalId )
{
	...
	else
	{
		...
		Script::CStruct* pGoalParams;
		if ( mp_goalFlags->GetStructure( goalId, &pGoalParams, Script::NO_ASSERT ) )
			pGoalParams->GetInteger( Crc::ConstCRC("isLocked"), &goalLocked, Script::ASSERT );
...
void CGoalManager::GetGoalAnimations( uint32 goalId, const char* type, Script::CScript* pScript )
{
	...
	// never know
	pScript->GetParams()->RemoveComponent( Crc::ConstCRC("pro_stuff") );
	if ( array_name )
		pScript->GetParams()->AddChecksum( Crc::ConstCRC("pro_stuff"), array_name );
}
...
bool CGoalManager::ReplaceTrickText( uint32 goalId, const char* p_old_string, char* p_new_string, uint out_buffer_size )
{
	...
	p_goal_params->GetArray( Crc::ConstCRC("key_combos"), &p_key_combo, Script::ASSERT );
	...
	if ( p_goal_params->ContainsFlag( Crc::ConstCRC("create_a_trick") ) )
	{
		p_goal_params->GetInteger( Crc::ConstCRC("trickName"), &premade_cat_index, Script::ASSERT );
	}
...
void CGoalManager::UnlockProSpecificChallenges()
{
	...
	for ( int i = 0; i < numGoals; i++ )
	{
		...
		if ( pGoal->GetParams()->ContainsFlag( Crc::ConstCRC("pro_specific_challenge") ) )
		{
...
Script::CStruct* CGoalManager::GetGoalAttemptInfo()
{
	...
	attemptInfo->AddInteger( Crc::ConstCRC("totalStarts"), totalStarts );
	return attemptInfo;
}
...
void CGoalManager::Land()
{
	...
	else 
	{
		...
		for ( int i = 0; i < numGoals; i++ )
		{
			...
			else if ( pGoal->IsActive() && pGoal->GetParams()->ContainsFlag( Crc::ConstCRC("high_combo") ) )
			{
				...
				if ( pSkater )
				{
					...
					if ( pScore->GetLastScoreLanded() > 0 )
					{
						...
						pScriptParams->AddInteger( Crc::ConstCRC("last_score_landed"), pScore->GetLastScoreLanded() );
						Script::RunScript( Crc::ConstCRC("goal_highcombo_check_score"), pScriptParams );
						delete pScriptParams;
					}
				}
			}
			else if ( pGoal->IsActive() && pGoal->ShouldRequireCombo() && pGoal->GetParams()->ContainsFlag( Crc::ConstCRC("combo_started") ) )
			{
			...
			else if ( pGoal->IsActive() && pGoal->GetParams()->ContainsFlag( Crc::ConstCRC( "score_goal" ) ) )
			{
				...
				if ( pSkater )
				{
					Mdl::Score* pScore = pSkater->GetScoreObject();
					int required_score;
					pGoal->GetParams()->GetInteger( Crc::ConstCRC("score"), &required_score, Script::ASSERT );
					if ( pScore->GetTotalScore() + pScore->GetScorePotValue() >= required_score )
					{
						pGoal->GetParams()->AddInteger( Crc::ConstCRC("winning_score"), pScore->GetTotalScore() + pScore->GetScorePotValue() );
						WinGoal( pGoal->GetGoalId() );
						pGoal->GetParams()->RemoveComponent( Crc::ConstCRC("winning_score") );
					}
				}
			}
		}
...
void CGoalManager::Bail()
{
	...
	for ( int i = 0; i < numGoals; i++ )
	{
		...
		if ( pGoal->IsActive() && pGoal->ShouldRequireCombo() && pGoal->GetParams()->ContainsFlag( Crc::ConstCRC("combo_started") ) )
		{
			// lose the goal next frame
			Script::CStruct* pScriptParams = new Script::CStruct();
			pScriptParams->AddChecksum( Crc::ConstCRC("goal_id"), pGoal->GetGoalId() );
		...
		if ( pGoal->IsActive() && pGoal->IsTetrisGoal() )
		{
			...
			if ( pGoal->GetParams()->GetInteger( Crc::ConstCRC( "trick_flag" ), &trick_flag, Script::NO_ASSERT ) )
			{
				if ( trick_flag != 0 )
				{
					pGoal->GetParams()->AddInteger( Crc::ConstCRC("trick_flag"), 0 );
				}
			}
		}
	}
...
void CGoalManager::ResetCareer()
{
	...
	// mp_proSpecificChallenges->AppendStructure( Script::GetStructure( Crc::ConstCRC("goal_pro_specific_challenges_beaten"), Script::ASSERT ) );
	...
	for ( int i = 0; i < numGoals; i++ )
	{
		...
		if ( !pGoal->IsMinigame() && !pGoal->HasWonGoal() )
		{
			...
			if ( pRewardParams->GetInteger(	 "cash", &cash, Script::NO_ASSERT ) )
			{
				...
				pGoal->GetParams()->GetString( Crc::ConstCRC("goal_name"), &p_goal_name, Script::ASSERT );
...
{
	uint32 goalId = 0;
	pParams->GetChecksum( Crc::ConstCRC("name"), &goalId, assert );
	return goalId;
}
...
void CGoalManager::CreateGoalLevelObjects( void )
{
	for ( int i = 0; i < GetNumGoals(); i++ )
	{
		...
		if ( pParams->GetArray( Crc::ConstCRC("create_geometry"), &pGeomArray ) )
		{
			uint32 j, geometry;
...
void CGoalManager::RunLastStageScript()
{
	...
	Script::CArray* pChapterScriptsArray = Script::GetArray( Crc::ConstCRC("CHAPTER_COMPLETION_SCRIPTS"), Script::ASSERT );
	...
	if ( pStageScriptStruct->GetChecksum( Crc::ConstCRC("script_name"), &script_name, Script::NO_ASSERT ) )
	{
...
{
	mp_team->AddString( Crc::ConstCRC("team_name"), pTeamName );
}
...
bool ScriptAddGoal(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	pParams->GetStructure( Crc::ConstCRC("params"), &pSubParams, Script::ASSERT );
...
bool ScriptDeactivateAllGoals(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	if( pParams->ContainsComponentNamed( Crc::ConstCRC("force_all")))
	{
...
bool ScriptUninitializeGoal(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	uint32 goalId;
	pParams->GetChecksum( Crc::ConstCRC("name"), &goalId, Script::ASSERT );
	if ( pParams->ContainsFlag( Crc::ConstCRC("affect_tree") ) )
	{
...
bool ScriptHasActiveGoals(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	bool count_all = pParams->ContainsFlag( Crc::ConstCRC("count_all") );
...
bool ScriptCreateGoalFlag(Script::CStruct *pParams, Script::CScript *pScript)
{
    ...
    uint32 flag;
    pParams->GetChecksum( Crc::ConstCRC("flag"), &flag, Script::ASSERT ); 
...
bool ScriptCreateGoalName( Script::CStruct* pParams, Script::CScript* pScript )
{
    ...
    pParams->GetText( Crc::ConstCRC("goal_type"), &goal_type, Script::ASSERT );
	...
	p_level_structure->GetString( Crc::ConstCRC("level"), &p_level_prefix, Script::ASSERT );
...
*/
    ...
    // add to pParams
    pScript->GetParams()->AddChecksum( Crc::ConstCRC("goal_id"), Script::GenerateCRC( goal_prefix ) );
	pScript->GetParams()->AddString( Crc::ConstCRC("goal_name"), goal_prefix );
...
bool ScriptGetLevelPrefix( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	pScript->GetParams()->AddString( Crc::ConstCRC("level_prefix"), p_level_prefix);
...
bool ScriptGetNumberOfGoalPoints(Script::CStruct* pParams, Script::CScript* pScript)
{
    ...
    int goal_points = 0;
	if ( pParams->ContainsFlag( Crc::ConstCRC("total") ) )
		goal_points = pGoalManager->GetTotalNumberOfGoalPointsEarned();
	else
		goal_points = pGoalManager->GetNumberOfGoalPoints();

    pScript->GetParams()->AddInteger( Crc::ConstCRC("goal_points"), goal_points );
    return true;
}
...
bool ScriptGetCash(Script::CStruct* pParams, Script::CScript* pScript)
{
	...
	bool get_total = pParams->ContainsFlag( Crc::ConstCRC("total") );
    
    pScript->GetParams()->AddInteger( Crc::ConstCRC("cash"), pGoalManager->GetCash( get_total ) );
    return true;
}
...
bool ScriptGetProsetNotPrefix(Script::CStruct* pParams, Script::CScript* pScript)
{
    ...
    pScript->GetParams()->AddString( Crc::ConstCRC("proset_not_prefix"), temp_string );
    return true;
}
...
bool ScriptNumGoalsBeatenInLevel(Script::CStruct* pParams, Script::CScript* pScript)
{
    ...
    int numbeat = pGoalManager->NumGoalsBeatenInLevel( levelNum, pParams->ContainsFlag( Crc::ConstCRC("count_pro_specific_challenges") ) );
    pScript->GetParams()->AddInteger( Crc::ConstCRC("num_beaten"), numbeat );
    return true;
}
...
bool ScriptAddGoalChoices( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	selected_only = pParams->ContainsFlag( Crc::ConstCRC("selected_only") );
...
bool ScriptIsInCompetition( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	if ( pGoal )
	{
		pScript->GetParams()->AddChecksum( Crc::ConstCRC("goal_id"), pGoal->GetGoalId() );
		return true;
	}
	return false;
}
...
{
	uint32 key_combo;
	pParams->GetChecksum( Crc::ConstCRC("key_combo"), &key_combo, Script::ASSERT );
	...
	if ( !pParams->ContainsFlag( Crc::ConstCRC("special") ) )
    {
        if( pTricks->GetInteger( key_combo, &cat_num, Script::NO_ASSERT ) )
        {
    		...
    		pScript->GetParams()->AddInteger( Crc::ConstCRC("cat_num"), cat_num );
            return true;
        }
        
        if( pTricks->GetChecksum( key_combo, &trick_checksum, Script::NO_ASSERT ) )
    	{
    		if ( trick_checksum == ( Crc::ConstCRC("Unassigned") ) )
               	return false;
    		...
    		p_trick_params->GetLocalString( Crc::ConstCRC("Name"), &p_trick_string, Script::ASSERT );
    
    		pScript->GetParams()->AddString( Crc::ConstCRC("trick_string"), p_trick_string );
    		pScript->GetParams()->AddChecksum( Crc::ConstCRC("trick_checksum"), trick_checksum );
    		...
    		if ( !p_trick_params->GetChecksum( Crc::ConstCRC("ExtraTricks"), &extra_trick, Script::NO_ASSERT ) )
    		{
    			...
    			if ( p_trick_params->GetArray( Crc::ConstCRC("ExtraTricks"), &p_extra_tricks, Script::NO_ASSERT ) )
    		...
    		if ( extra_trick )
    		{
    			...
    			p_extra_trick->GetStructure( Crc::ConstCRC("Params"), &p_extra_trick_params, Script::ASSERT );
    			const char* p_extra_trick_string;
    			p_extra_trick_params->GetString( Crc::ConstCRC("Name"), &p_extra_trick_string, Script::ASSERT );
    			pScript->GetParams()->AddString( Crc::ConstCRC("extra_trick_string"), p_extra_trick_string );
    			pScript->GetParams()->AddChecksum( Crc::ConstCRC("extra_trick_checksum"), extra_trick );
    		}
    		return true;
    	}
    }
	else if ( pParams->ContainsFlag( Crc::ConstCRC("special") ) )
	{		
		...
		for ( int i = 0; i < numTricks; i++ )
		{
			...
			{
				if ( trick_info.m_TrickName == ( Crc::ConstCRC("Unassigned") ) )
					return false;
                ...
                {
                    pScript->GetParams()->AddInteger( Crc::ConstCRC("trick_checksum"), trick_info.m_TrickName );
                }
                else
                {
                    pScript->GetParams()->AddChecksum( Crc::ConstCRC("trick_checksum"), trick_info.m_TrickName );
                }
                pScript->GetParams()->AddInteger( Crc::ConstCRC("current_index"), i );
				return true;
			}
		}
	}
...
bool ScriptReplaceTrickText( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	uint32 goalId;
	if ( pParams->ContainsFlag( Crc::ConstCRC("all") ) )
	{
		pGoalManager->ReplaceAllTrickText();
		return true;
	}
	else if ( pParams->GetChecksum( Crc::ConstCRC("name"), &goalId, Script::NO_ASSERT ) )
	{
		const char* p_old_string;
		if ( pParams->GetString( Crc::ConstCRC("text"), &p_old_string, Script::NO_ASSERT ) )
		{
			char new_string[NEW_STRING_LENGTH];
			if ( pGoalManager->ReplaceTrickText( goalId, p_old_string, new_string, NEW_STRING_LENGTH ) )
			{
                pScript->GetParams()->RemoveComponent( Crc::ConstCRC("TrickText") );
				pScript->GetParams()->AddString( Crc::ConstCRC("TrickText"), new_string );
			}
		}
		else 
...
bool ScriptGetNumberCollected( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	pScript->GetParams()->AddInteger( Crc::ConstCRC("number_collected"), number_collected );
	return true;
}
...
bool ScriptGetNumberOfFlags( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	{
		pScript->GetParams()->AddInteger( Crc::ConstCRC("number_of_flags"), number_of_flags );
		return true;
	}
	return false;
}
...
bool ScriptColorTrickObjects( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	bool clear = pParams->ContainsFlag( Crc::ConstCRC("clear") );

	int skaterId = 0;
	pParams->GetInteger( Crc::ConstCRC("skater"), &skaterId, Script::NO_ASSERT );
...
bool ScriptGetNumberOfTimesGoalStarted( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	pScript->GetParams()->AddInteger( Crc::ConstCRC("times_started"), numTimesLost );
	return true;
}
...
bool ScriptGetLastGoalId( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	if ( goalId )
	{
		pScript->GetParams()->AddChecksum( Crc::ConstCRC("goal_id"), goalId );
		return true;
	}
	return false;
}
...
{
	uint32 skater_name;
	pParams->GetChecksum( Crc::ConstCRC("skater"), &skater_name, Script::ASSERT );
...
bool ScriptSetEndRunType( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	if ( pParams->ContainsFlag( Crc::ConstCRC("Goal_EndOfRun") ) )
		newEndRunType = Game::vGOALENDOFRUN;
	else if ( pParams->ContainsFlag( Crc::ConstCRC("EndOfRun") ) )
		newEndRunType = Game::vENDOFRUN;
	else if ( pParams->ContainsFlag( Crc::ConstCRC("none") ) )
...
bool ScriptIsLeafNode( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	return pGoalManager->CallMemberFunction( ( Crc::ConstCRC("IsLeafNode") ), pParams, pScript );
}
...
bool ScriptIsRootNode( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	return pGoalManager->CallMemberFunction( ( Crc::ConstCRC("IsRootNode") ), pParams, pScript );
}
...
bool ScriptSuspendGoalPedLogic( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	int suspend = 1;
	pParams->GetInteger( Crc::ConstCRC("suspend"), &suspend, Script::NO_ASSERT );
...
bool ScriptRememberLevelStructureName( Script::CStruct *pParams, Script::CScript* pScript )
{
	...
	uint32 level;
	pParams->GetChecksum( Crc::ConstCRC("level_structure_name"), &level, Script::ASSERT );
...
bool ScriptGetDifficultyLevel( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	pScript->GetParams()->AddInteger( Crc::ConstCRC("difficulty_level"), level );
	return true;
}
...
bool ScriptGetGoalParam( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	pParams->GetChecksum( Crc::ConstCRC("type"), &type_checksum, Script::ASSERT );

	// convert type
	switch ( type_checksum )
	{
		case Crc::ConstCRC("checksum"):
			type=ESYMBOLTYPE_NAME;
			break;
		case Crc::ConstCRC("float"):
			type=ESYMBOLTYPE_FLOAT;
			break;
		case Crc::ConstCRC("integer"):
			type=ESYMBOLTYPE_INTEGER;
			break;
		case Crc::ConstCRC("structure"):
			type=ESYMBOLTYPE_STRUCTURE;
			break;
		case Crc::ConstCRC("array"):
...
bool ScriptGetSponsor( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	pScript->GetParams()->AddChecksum( Crc::ConstCRC("sponsor"), pGoalManager->GetSponsor() );
	return true;
}
...
bool ScriptGetTeam( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	pScript->GetParams()->AddInteger( Crc::ConstCRC("num_team_members"), total );
...
bool ScriptSetTeamMember( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	Dbg_Assert( pGoalManager );
	uint32 pro;
	pParams->GetChecksum( Crc::ConstCRC("pro"), &pro, Script::ASSERT );
	bool remove = pParams->ContainsFlag( Crc::ConstCRC("remove") );
	bool mark_used = pParams->ContainsFlag( Crc::ConstCRC("mark_used") );
