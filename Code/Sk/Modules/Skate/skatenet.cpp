...
int	Skate::handle_msg_relay( Net::MsgHandlerContext* context )
{
			...
			player = gamenet_man->NextPlayerInfo( sh, true ))
	{
		...
		else if( context->m_MsgId == GameNet::MSG_ID_SPAWN_PROJECTILE )
		{
			...
			if(	( mod->GetGameMode()->GetNameChecksum() == Crc::ConstCRC("firefight") ) ||
				( mod->GetGameMode()->GetNameChecksum() == Crc::ConstCRC("netfirefight")))
			{
...
int	Skate::handle_projectile( Net::MsgHandlerContext* context )
{
	...
	Script::RunScript( Crc::ConstCRC("ClientLaunchFireball"), pParams, skater );
	delete pParams;
...
int	Skate::handle_cheat_list( Net::MsgHandlerContext* context )
{
	...
	for( i = 0; i < msg->m_NumCheats; i++ )
	{
		...
		pParams = new Script::CStruct;
		pParams->AddChecksum( Crc::ConstCRC("on"), 0 );
		pParams->AddChecksum( Crc::ConstCRC("cheat_flag"), msg->m_Cheats[i] );
		Script::RunScript( Crc::ConstCRC("client_toggle_cheat"), pParams );
...
int Skate::handle_cheats( Net::MsgHandlerContext* context )
{
	...
	pParams->AddChecksum( Crc::ConstCRC("cheat_flag"), msg->m_Cheat );
...
int	Skate::handle_score_update( Net::MsgHandlerContext* context )
{
	...
	{           
		...
		if( obj )
		{
				...
				( mod->GetGameMode()->ShouldTrackTrickScore() == false ))
			{
				...
				if(	( mod->GetGameMode()->GetNameChecksum() == Crc::ConstCRC("netfirefight" )) &&
					( skater_obj->IsLocalClient() == false ))
				{
					...
					if(( pScore->GetTotalScore() > 0 ) && ( score == 0 ))
					{
						...
						params->AddString( Crc::ConstCRC("name"), skater_obj->GetDisplayName());
						Script::RunScript( Crc::ConstCRC("announce_elimination"), params );
						delete params;
	...
	if( msg->m_Cheats != mod->GetCheatChecksum())
	{
		...
		{
			Script::RunScript( Crc::ConstCRC("notify_cheating"));
			gamenet_man->CheatingOccured();
		}
	}
	...
	if( mod->GetGameMode()->IsTeamGame())
	{
		...
		if( leading_team != GameNet::vNO_TEAM )
		{
			...
			if( msg->m_Final )
			{
				...
				{
					my_player->m_Skater->SelfEvent(Crc::ConstCRC("WonGame"));
				}
				else
				{
					if( my_player->m_Skater )
					{
						my_player->m_Skater->SelfEvent(Crc::ConstCRC("LostGame"));
					}
				}
			}
	...
	else
	{
		...
		{   
			...
			if( msg->m_Final )
			{
				...
				{
					leader->m_Skater->SelfEvent(Crc::ConstCRC("WonGame"));
				}
				else
				{
					if( my_player->m_Skater )
					{
						leader->m_Skater->SelfEvent( Crc::ConstCRC("LostGame") );
					}
				}
			}
			else if( ( gamenet_man->GetCurrentLeader() != leader ) &&
					 ( num_players > 1 ) &&
					 ( mod->GetGameMode()->IsTrue( Crc::ConstCRC("show_leader_messages") ) ) &&
	...
	
			{
				...
				{   
					Script::CScriptStructure* pTempStructure = new Script::CScriptStructure;
					pTempStructure->Clear();
					pTempStructure->AddComponent( Crc::ConstCRC("String0"), ESYMBOLTYPE_STRING, leader->m_Name );
					Script::RunScript( Crc::ConstCRC("NewScoreLeaderOther"), pTempStructure, my_player->m_Skater );
					delete pTempStructure;
				}
...
int	Skate::handle_play_sound( Net::MsgHandlerContext* context )
{
	...
	Env::CTerrainManager::sPlaySfx( (Env::ETerrainActionType) msg->m_WhichArray, (ETerrainType) msg->m_SurfaceFlag, pos, vol, pitch, choice, false );
...
{   
	...
	
    {
		...
		if( msg->m_GameMode != Script::GenerateCRC("netlobby" ))
		{
			...
			{
				if( ( msg->m_GameMode != Crc::ConstCRC("firefight")) &&
					( msg->m_GameMode != Crc::ConstCRC("netfirefight")))
				{
					Script::CArray* pArray = new Script::CArray;
	...
	{   
		...
		if( value == msg->m_GameMode )
		{
			...
			if( msg->m_TimeLimit == 0 )
			{
				params->AddInteger( Crc::ConstCRC("unlimited_time"), 1 );
			}
			else
			{
				params->AddInteger( Crc::ConstCRC("unlimited_time"), 0 );
			}
...
void	Skate::SendCheatList( GameNet::PlayerInfo* player )
{
	...
	if( mp_career->GetCheat( Crc::ConstCRC("CHEAT_ALWAYS_SPECIAL")))
	{
		cheat_msg.m_Cheats[cheat_msg.m_NumCheats++] = Crc::ConstCRC("CHEAT_ALWAYS_SPECIAL");
	}
	if( mp_career->GetCheat( Crc::ConstCRC("CHEAT_PERFECT_MANUAL")))
	{
		cheat_msg.m_Cheats[cheat_msg.m_NumCheats++] = Crc::ConstCRC("CHEAT_PERFECT_MANUAL");
	}
	if( mp_career->GetCheat( Crc::ConstCRC("CHEAT_PERFECT_RAIL")))
	{
		cheat_msg.m_Cheats[cheat_msg.m_NumCheats++] = Crc::ConstCRC("CHEAT_PERFECT_RAIL");
	}
	if( mp_career->GetCheat( Crc::ConstCRC("CHEAT_PERFECT_SKITCH")))
	{
		cheat_msg.m_Cheats[cheat_msg.m_NumCheats++] = Crc::ConstCRC("CHEAT_PERFECT_SKITCH");
	}
	if( mp_career->GetCheat( Crc::ConstCRC("CHEAT_ALWAYS_SPECIAL")))
	{
		cheat_msg.m_Cheats[cheat_msg.m_NumCheats++] = Crc::ConstCRC("CHEAT_ALWAYS_SPECIAL");
	}
	if( mp_career->GetCheat( Crc::ConstCRC("CHEAT_STATS_13")))
	{
		cheat_msg.m_Cheats[cheat_msg.m_NumCheats++] = Crc::ConstCRC("CHEAT_STATS_13");
	}
	if( mp_career->GetCheat( Crc::ConstCRC("FLAG_G_EXPERT_MODE_NO_REVERTS")))
	{
		cheat_msg.m_Cheats[cheat_msg.m_NumCheats++] = Crc::ConstCRC("FLAG_G_EXPERT_MODE_NO_REVERTS");
	}
	if( mp_career->GetCheat( Crc::ConstCRC("FLAG_G_EXPERT_MODE_NO_WALKING")))
	{
		cheat_msg.m_Cheats[cheat_msg.m_NumCheats++] = Crc::ConstCRC("FLAG_G_EXPERT_MODE_NO_WALKING");
	}
	if( mp_career->GetCheat( Crc::ConstCRC("FLAG_G_EXPERT_MODE_NO_MANUALS")))
	{
		cheat_msg.m_Cheats[cheat_msg.m_NumCheats++] = Crc::ConstCRC("FLAG_G_EXPERT_MODE_NO_MANUALS");
	}
	if( mp_career->GetCheat( Crc::ConstCRC("NO_G_DISPLAY_BALANCE")))
	{
		cheat_msg.m_Cheats[cheat_msg.m_NumCheats++] = Crc::ConstCRC("NO_G_DISPLAY_BALANCE");
	}
...
uint32	Skate::GetCheatChecksum( void )
{
	...
	if( mp_career->GetCheat( Crc::ConstCRC("CHEAT_ALWAYS_SPECIAL")))
	{
		checksum |= Crc::ConstCRC("CHEAT_ALWAYS_SPECIAL");
	}
	if( mp_career->GetCheat( Crc::ConstCRC("CHEAT_PERFECT_MANUAL")))
	{
		checksum |= Crc::ConstCRC("CHEAT_PERFECT_MANUAL");
	}
	if( mp_career->GetCheat( Crc::ConstCRC("CHEAT_PERFECT_RAIL")))
	{
		checksum |= Crc::ConstCRC("CHEAT_PERFECT_RAIL");
	}
	if( mp_career->GetCheat( Crc::ConstCRC("CHEAT_PERFECT_SKITCH")))
	{
		checksum |= Crc::ConstCRC("CHEAT_PERFECT_SKITCH");
	}
	if( mp_career->GetCheat( Crc::ConstCRC("CHEAT_ALWAYS_SPECIAL")))
	{
		checksum |= Crc::ConstCRC("CHEAT_ALWAYS_SPECIAL");
	...
	if( mp_career->GetCheat( Crc::ConstCRC("CHEAT_STATS_13")))
	{
		checksum |= Crc::ConstCRC("CHEAT_STATS_13");
	}
	if( mp_career->GetCheat( Crc::ConstCRC("FLAG_G_EXPERT_MODE_NO_REVERTS")))
	{
		checksum |= Crc::ConstCRC("FLAG_G_EXPERT_MODE_NO_REVERTS");
	}
	if( mp_career->GetCheat( Crc::ConstCRC("FLAG_G_EXPERT_MODE_NO_WALKING")))
	{
		checksum |= Crc::ConstCRC("FLAG_G_EXPERT_MODE_NO_WALKING");
	}
	if( mp_career->GetCheat( Crc::ConstCRC("FLAG_G_EXPERT_MODE_NO_MANUALS")))
	{
		checksum |= Crc::ConstCRC("FLAG_G_EXPERT_MODE_NO_MANUALS");
	}
	if( mp_career->GetCheat( Crc::ConstCRC("NO_G_DISPLAY_BALANCE")))
	{
		checksum |= Crc::ConstCRC("NO_G_DISPLAY_BALANCE");
	}
