...
{
	pParams->GetChecksum( Crc::ConstCRC("goal_id"), &m_goalId, Script::ASSERT );
...
bool CGoalPed::GetStreamChecksum( uint32* p_streamChecksum, int cam_anim_index, bool success, const char* p_speaker_name, bool last_anim )
{
	...
	pGoalParams->GetText( Crc::ConstCRC("goal_name"), &p_goal_name, Script::ASSERT );
	...
	if ( p_speaker_name && Script::GenerateCRC( p_speaker_name ) == Crc::ConstCRC("skater") )
	{
		Mdl::Skate * pSkate = Mdl::Skate::Instance();
		Obj::CSkaterProfile* pSkaterProfile = pSkate->GetCurrentProfile();	
		int male_test;
		pSkaterProfile->GetInfo()->GetInteger( Crc::ConstCRC("is_male"), &male_test, Script::ASSERT );
		if ( !male_test )
		{
...
void CGoalPed::PlayGoalStream( uint32 stream_checksum, bool use_pos_info, uint32 speaker_obj_id, bool should_load_fam )
{
	...
	{		
		...
		p_tempParams->AddChecksum( Crc::ConstCRC("stream_checksum"), stream_checksum );

		if ( !use_pos_info )
		{
			p_tempParams->AddInteger( Crc::ConstCRC("use_pos_info"), 0 );
			...
			p_tempParams->AddChecksum( Crc::ConstCRC("streamId"), streamId );
			m_lastStreamId = streamId;
	
			// Script::KillSpawnedScriptsThatReferTo( Crc::ConstCRC("goal_play_stream") );
			...
			{
				p_tempParams->AddChecksum( NONAME, Crc::ConstCRC("play_anim") );
				m_lastFamObj = trigger_obj_id;
				
				/*Script::CStruct* pTempStruct = new Script::CStruct;
				pTempStruct->AddChecksum( Crc::ConstCRC("type"), Crc::ConstCRC("partialAnim") );
				...
				pTempStruct->AddChecksum( Crc::ConstCRC("from"), Crc::ConstCRC("start") );
				pTempStruct->AddChecksum( Crc::ConstCRC("to"), Crc::ConstCRC("end") );
		...
		Script::CScript* pSpawnedScript = Script::SpawnScript( Crc::ConstCRC("goal_play_stream"), p_tempParams );
...
void CGoalPed::PlayGoalStream( const char* type, Script::CStruct* pParams )
{
	...
	// check for any flags
	bool play_random = pParams->ContainsFlag( Crc::ConstCRC("play_random") );
	bool call_player_by_name = pParams->ContainsFlag( Crc::ConstCRC("call_player_by_name") );
	...
	if ( pParams->GetChecksum( Crc::ConstCRC("vo"), &stream_checksum, Script::NO_ASSERT ) )
	{
		// play it!
	...
	if ( ( call_player_by_name || play_random ) && !found_player_name_vo )
	{
		...
		{
		case ( Crc::ConstCRC("wait") ):
			PlayGoalWaitStream();
			break;
		// case ( Crc::ConstCRC("Success") ):
			// PlayGoalWinStream();
			// break;
		case ( Crc::ConstCRC("Midgoal") ):
			PlayGoalMidStream();
			break;
		// case ( Crc::ConstCRC("Talking") ):
			// PlayGoalStartStream();
			// break;
		default:
			//Pcm::StopStreams();
...
void CGoalPed::PlayGoalStartStream( Script::CStruct* pParams )
{
	...
	{		
		...
		pParams->GetChecksum( Crc::ConstCRC("speaker_obj_id"), &speaker_obj_id, Script::NO_ASSERT );
...
void CGoalPed::UnloadLastFam()
{
	...
	{		
		...
		{			
			...
			Script::StopScriptsUsingThisObject_Proper( p_pos_obj, Crc::ConstCRC("goal_play_stream") );
			Script::CStruct* pTempStruct = new Script::CStruct;
			pTempStruct->AddChecksum( Crc::ConstCRC("type"), Crc::ConstCRC("partialAnim") );
			pTempStruct->AddChecksum( Crc::ConstCRC("id"), Crc::ConstCRC("jawRotation") );
...
void CGoalPed::StopCurrentStream()
{
	...
	uint32 current_vo_stream = 0;
	if ( pParams->GetChecksum( Crc::ConstCRC("current_vo_stream"), &current_vo_stream, Script::NO_ASSERT ) )
	{
		pParams->RemoveComponent( Crc::ConstCRC("current_vo_stream") );
...
bool CGoalPed::ProIsCurrentSkater()
{
	...
	Script::CStruct* p_last_names = Script::GetStructure( Crc::ConstCRC("goal_pro_last_name_checksums"), Script::ASSERT );
	const char* first_name_string;
	pParams->GetString( Crc::ConstCRC("pro_challenge_pro_name"), &first_name_string, Script::ASSERT );
	uint32 last_name;
...
uint32 CGoalPed::GetGoalAnimations( const char* type )
{
	...
	const char* p_goal_name;
	pParams->GetString( Crc::ConstCRC("goal_name"), &p_goal_name, Script::ASSERT );
...
void CGoalPed::SuspendGoalLogic( bool suspend )
{
	...
	if ( suspend )
	{
		Script::RunScript( Crc::ConstCRC("goal_pro_stop_anim_scripts"), GetGoalParams() );
		StopCurrentStream();
	}
}
...
void CGoalPed::Hide( bool hide )
{
	...
	if ( trigger_obj_id )
	{
		...
		pTempParams->AddChecksum( Crc::ConstCRC("goal_id"), m_goalId );
		if ( hide )
			Script::RunScript( Crc::ConstCRC("goal_ped_kill_arrow"), pTempParams );
		else
		{
			...
			if ( should_add_arrow )
				Script::RunScript( Crc::ConstCRC("goal_add_ped_arrow"), pTempParams );
		}
		delete pTempParams;
	}
}
