...
int PlaySoundPlease( PlatformWaveInfo *pInfo, sVolume *p_vol, float pitch, bool no_reverb )
{
	...
	if ( no_reverb )
	{
		...
		for ( i = NUM_NO_REVERB_START_VOICE; i < ( NUM_NO_REVERB_VOICES + NUM_NO_REVERB_START_VOICE ); i++ )
		{
			...
			{
				return ( PS2Sfx_PlaySfx( pInfo, i + (NUM_NO_REVERB_CORE * NUM_VOICES_PER_CORE), p_vol->GetChannelVolume( 0 ), p_vol->GetChannelVolume( 1 ), pitch ));
			}
		}
...
//		Dbg_MsgAssert(0, ("Ran out of NO_REVERB voices"));
	...
	for ( i = 0; i < NUM_VOICES_PER_CORE; i++ )
	{
		if ( voiceAvailableFlags & ( 1 << i ) )
		{
			...
			return ( PS2Sfx_PlaySfx( pInfo, i, p_vol->GetChannelVolume( 0 ), p_vol->GetChannelVolume( 1 ), pitch ));
		}
	}
	...
	for ( i = 0; i < NUM_VOICES_PER_CORE; i++ )
	{
		if ( voiceAvailableFlags & ( 1 << i ) )
		{
			...
			return ( PS2Sfx_PlaySfx( pInfo, i + NUM_VOICES_PER_CORE, p_vol->GetChannelVolume( 0 ), p_vol->GetChannelVolume( 1 ), pitch ));
		}
	}
...
//--------------------------------------------------------------------------------------------
// Function    : void PlaySfx(int voiceNumber,int pitch,int volume)
...
//-------------------------------------------------------------------------------------------
int PS2Sfx_PlaySfx( PlatformWaveInfo *pInfo, int voiceNumber, float volL, float volR, float pitch )
{
