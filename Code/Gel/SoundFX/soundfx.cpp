...
int CSfxManager::GetVoiceFromID(uint32 id)
{
	...
	for( i = 0; i < NUM_VOICES; i++ )
	{
		...
		{
			if( Script::GetInteger( 0x6d2e270e , Script::NO_ASSERT ) ) // DebugSoundFxUpdate
			{
...
{
	return 0x3FFFFFFF; // return GetMemAvailable();
}
...
//			Dbg_Message( "sound %s auto off", Script::FindChecksumName( pInfo->checksum ) );
	...
	if(( p_dropoff_pos || (dist < dropoffDist) ) )
	{
	...
	if( !Config::CD())
	{
		if( Script::GetInteger( 0x6d2e270e , Script::NO_ASSERT ) && !vol.IsSilent()) // DebugSoundFxUpdate
		{
...
bool CSfxManager::LoadSound( const char *sfxName,  int flags, float dropoff, float pitch, float volume )
{
	...
	if( nullptr != GetWaveTableIndex( checksum ))
	{
		if( !Config::CD())
		{
			if( Script::GetInt( 0xd7bb618d , false )) // DebugSoundFx
			{
...
uint32 CSfxManager::PlaySfx( uint32 checksum, sVolume *p_vol, float pitch, uint32 controlID, SoundUpdateInfo *pUpdateInfo, const char *pSoundName )
{
	...
	if( !Config::CD())
	{
		if( Script::GetInteger( 0xd7bb618d , Script::NO_ASSERT )) // DebugSoundFx
		{
	...
	if( !waveTableIndex->platformWaveInfo.looping )
	{
		...
		{
//			Replay::WritePlaySfx( checksum, volL, volR, pitch );
		}	
	}
...
/******************************************************************/
/*																  */
/* Send in the uniqueID returned from PlaySfx()				  */
/*																  */
/******************************************************************/
...
//	}
	...
	uint32 uniqueID = PlaySfx( soundChecksum, &vol, pUpdateInfo->pitch, 0, pUpdateInfo );
