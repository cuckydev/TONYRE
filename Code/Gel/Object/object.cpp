...
{	
	...
	m_id = id;
	SetChecksumTag(Crc::ConstCRC("id"), m_id);
}
...
void CObject::SetType(sint type) 
{
	...
	m_type = type;
	SetChecksumTag(Crc::ConstCRC("type"), m_type);
}
...
bool CObject::CallMemberFunction( uint32 Checksum, Script::CScriptStructure *pParams, Script::CScript *pScript )
{
	...
	switch(Checksum)
	{
		...
		case 0xfbd89cd5: // Obj_KillSpawnedScript
		{
			...
			if (pParams->GetChecksum(Crc::ConstCRC("Name"),&ScriptChecksum))
			{
		...
		// case	Crc::ConstCRC("SetProperties"):
		case	Crc::ConstCRC("SetProps"):
		{
...
/*		
		case	Crc::ConstCRC("ClearEventHandler"):
		{
			uint32 type;
		...
		case	Crc::ConstCRC("ClearEventHandlerGroup"):
		{
...
//		case 0x2c0c9e7b: // OnExceptionRun	
		case Crc::ConstCRC("OnExceptionRun"):
		{
