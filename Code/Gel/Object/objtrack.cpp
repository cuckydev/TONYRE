...
bool ScriptLaunchEvent(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	uint32 target = pManager->ResolveComplexID(pParams, Crc::ConstCRC("target"));
	if (target)	
	{
		if (target == Crc::ConstCRC("system"))
	...
	uint32 source = pManager->ResolveComplexID(pParams, Crc::ConstCRC("source"));
	if (source)	
	{
		if (source == Crc::ConstCRC("system"))
	...
	uint32 type;
	if (pParams->GetChecksum(Crc::ConstCRC("type"), &type))
	{
...
bool ScriptTerminateObjectsScripts(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	bool use_proper = pParams->ContainsFlag( Crc::ConstCRC("use_proper_version") );
	
	// see if array of script names
	Script::CArray *p_array;
	if (pParams->GetArray(Crc::ConstCRC("scripts"), &p_array))
	{
	...
	else
	{
		...
		pParams->GetChecksum(Crc::ConstCRC("script_name"), &script_to_stop);
		if ( use_proper )
...
bool ScriptAssignAlias(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	uint32 alias;
	pParams->GetChecksum(Crc::ConstCRC("alias"), &alias, Script::ASSERT);
