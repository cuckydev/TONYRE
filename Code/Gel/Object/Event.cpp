	...
	if (mp_tab && m_num_entries)
	{
		...
		for (int i=0; i<m_num_entries; ++i)
		{
			if (mp_tab[i].script != vDEAD_ENTRY)
			{
				...
				p_ex->AddChecksum(Crc::ConstCRC("Type"), mp_tab[i].type);		
				p_ex->AddChecksum(Crc::ConstCRC("Event_Script"), mp_tab[i].script);		
				p_ex->AddChecksum(Crc::ConstCRC("Group"), mp_tab[i].group);
				p_array->SetStructure(i,p_ex);
			}
			else
			{
				Script::CStruct *p_ex = new Script::CStruct;
				p_ex->AddChecksum(NO_NAME, Crc::ConstCRC("DeadEntry"));
				p_array->SetStructure(i,p_ex);
			}
		}
		p_info->AddArrayPointer(Crc::ConstCRC("mp_event_handler_table"),p_array);
...
/*
	...
	if (new_entries == 1)
	{
		...
		if (p_entry)
		{
			...
			if (pEventStruct->GetStructure(Crc::ConstCRC("params"),&p_params))
			{
			...
			p_entry->exception = pEventStruct->ContainsFlag(Crc::ConstCRC("Exception"));
...
void CEventHandlerTable::remove_group(uint32 group)
{
	for (int i = 0; i < m_num_entries; i++)
	{
		if (mp_tab[i].group == group || group == Crc::ConstCRC("all_groups"))
		{
			mp_tab[i].script = vDEAD_ENTRY;
			if (mp_tab[i].p_params)
			{
...
//		}		
		...
		if (p_entry->type == pEvent->GetType() && p_entry->script != vDEAD_ENTRY && p_entry->enabled)
		{
			...
			#ifdef	__NOPT_ASSERT__
			if (Script::GetInteger(Crc::ConstCRC("Display_event_arrows")))
			{
