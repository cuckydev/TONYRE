...
{
	uint32 NameChecksum;
	if ( pParams->GetChecksum( Crc::ConstCRC("name"), &NameChecksum ))
	{
		SetID(NameChecksum);
	}
...
#	ifdef __USE_PROFILER__
	...
	pParams->GetInteger( Crc::ConstCRC("ProfileColor"), &ProfileColor );
	SetProfileColor(ProfileColor);
#	endif // __USE_PROFILER__

	if (pParams->ContainsFlag(Crc::ConstCRC("permanent")))
	{
...
bool 	CCompositeObject::CallMemberFunction( uint32 Checksum, Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	switch ( Checksum )
	{
		...
		{									 
			pScript->GetParams()->AddVector( Crc::ConstCRC("pos"), m_pos[X], m_pos[Y], m_pos[Z] );
			return true;
		}
		break;
		...
		{									 
			pParams->GetVector(Crc::ConstCRC("Position"),&m_pos);
			return true;
		}
		break;
		...
		case 0xc6f3baa5: // Obj_SetOrientation
		{
			...
			if (pParams->GetVector(Crc::ConstCRC("dir"),&dir))
			{
				Mth::Matrix mat;
				mat.Ident();
		...
		{								   
			pScript->GetParams()->AddVector( Crc::ConstCRC("vel"), m_vel[X], m_vel[Y], m_vel[Z] );
			return true;
		}
		break;
		...
		{								   
			pScript->GetParams()->AddFloat( Crc::ConstCRC("speed"), m_vel.Length() );
			return true;
		}
		break;
...
// MOVED FROM CMOVINGOBJECT.CPP		
		...
		case 0x2a26ffa2: // Obj_GetOrientationToObject	
		{
			...
			pScript->GetParams()->AddFloat(Crc::ConstCRC("DotProd"),dotProd);
			pScript->GetParams()->AddFloat(Crc::ConstCRC("Orientation"),orientation);
			return true;
		}

        // @script | Backwards | Returns true if the skater is
        // facing backwards from his direction of travel.
		case Crc::ConstCRC("Backwards"):
        ...
        // @uparm 1.0 | speed
		case Crc::ConstCRC("SpeedGreaterThan"):
		{
			float TestSpeed = 0;
			pParams->GetFloat(NO_NAME, &TestSpeed);
        ...
        // @uparm 1.0 | speed
		case Crc::ConstCRC("SpeedLessThan"):
		{
			float TestSpeed = 0;
			pParams->GetFloat(NO_NAME, &TestSpeed);
			return m_vel.Length() < TestSpeed ? CBaseComponent::MF_TRUE : CBaseComponent::MF_FALSE; 
...
void CCompositeObject::GetDebugInfo(Script::CStruct *p_info)
{
	...
	{
		p_info->AddChecksum(NONAME,Crc::ConstCRC("NoNode"));
	}	
	...
	switch (m_type)
	{
		case SKATE_TYPE_UNDEFINED:
			type_name=Crc::ConstCRC("SKATE_TYPE_UNDEFINED");
			break;
		case SKATE_TYPE_SKATER: 			
			type_name=Crc::ConstCRC("SKATE_TYPE_SKATER");
			break;
		case SKATE_TYPE_PED:
			type_name=Crc::ConstCRC("SKATE_TYPE_PED");
			break;
		case SKATE_TYPE_CAR:
			type_name=Crc::ConstCRC("SKATE_TYPE_CAR");
			break;
		case SKATE_TYPE_GAME_OBJ:			
			type_name=Crc::ConstCRC("SKATE_TYPE_GAME_OBJ");
			break;
		case SKATE_TYPE_BOUNCY_OBJ:		
			type_name=Crc::ConstCRC("SKATE_TYPE_BOUNCY_OBJ");
			break;
		case SKATE_TYPE_CASSETTE:			
			type_name=Crc::ConstCRC("SKATE_TYPE_CASSETTE");
			break;
		case SKATE_TYPE_ANIMATING_OBJECT:	
			type_name=Crc::ConstCRC("SKATE_TYPE_ANIMATING_OBJECT");
			break;
		case SKATE_TYPE_CROWN:	
			type_name=Crc::ConstCRC("SKATE_TYPE_CROWN");
			break;
		case SKATE_TYPE_PARTICLE:			
			type_name=Crc::ConstCRC("SKATE_TYPE_PARTICLE");
			break;
		case SKATE_TYPE_REPLAY_DUMMY:		
			type_name=Crc::ConstCRC("SKATE_TYPE_REPLAY_DUMMY");
			break;
		case SKATE_TYPE_COMPOSITE:	
			type_name=Crc::ConstCRC("SKATE_TYPE_COMPOSITE");
			break;
		default:
			type_name=0;
			break;
