...
ETerrainType			CTerrainManager::sGetTerrainFromChecksum(uint32 checksum)
{
	...
	case Crc::ConstCRC("TERRAIN_GRINDCONC"):
		return vTERRAIN_GRINDCONC;

	case Crc::ConstCRC("TERRAIN_GRINDROUNDMETALPOLE"):
		return vTERRAIN_GRINDROUNDMETALPOLE;

	case Crc::ConstCRC("TERRAIN_GRINDCHAINLINK"):
	...
	case Crc::ConstCRC("TERRAIN_GRINDWOOD"):
		return vTERRAIN_GRINDWOOD;

	case Crc::ConstCRC("TERRAIN_GRINDPLASTIC"):
		return vTERRAIN_GRINDPLASTIC;

	case Crc::ConstCRC("TERRAIN_GRINDELECTRICWIRE"):
		return vTERRAIN_GRINDELECTRICWIRE;

	case Crc::ConstCRC("TERRAIN_GRINDCABLE"):
		return vTERRAIN_GRINDCABLE;

	case Crc::ConstCRC("TERRAIN_GRINDCHAIN"):
		return vTERRAIN_GRINDCHAIN;

	case Crc::ConstCRC("TERRAIN_GRINDPLASTICBARRIER"):
		return vTERRAIN_GRINDPLASTICBARRIER;

	case Crc::ConstCRC("TERRAIN_GRINDNEONLIGHT"):
		return vTERRAIN_GRINDNEONLIGHT;

	case Crc::ConstCRC("TERRAIN_GRINDGLASSMONSTER"):
		return vTERRAIN_GRINDGLASSMONSTER;

	case Crc::ConstCRC("TERRAIN_GRINDBANYONTREE"):
		return vTERRAIN_GRINDBANYONTREE;

	case Crc::ConstCRC("TERRAIN_GRINDBRASSRAIL"):
		return vTERRAIN_GRINDBRASSRAIL;

	case Crc::ConstCRC("TERRAIN_GRINDCATWALK"):
		return vTERRAIN_GRINDCATWALK;

	case Crc::ConstCRC("TERRAIN_GRINDTANKTURRET"):
...
bool					CTerrainManager::s_get_terrain_actions_struct(ETerrainType terrain, Script::CStruct *p_actions)
{
	...
	Script::CArray *pDefaultTerrainArray = Script::GetArray(Crc::ConstCRC("standard_terrain"));
	...
	for (uint i = 0; (!p_default_actions || !p_terrain_actions) && i < pDefaultTerrainArray->GetSize(); i++)
	{
		...
		pTerrainStruct->GetChecksum(Crc::ConstCRC("Terrain"), &terrain_checksum, Script::ASSERT);
		terrain_type = sGetTerrainFromChecksum(terrain_checksum);

		if (terrain_type == vTERRAIN_DEFAULT)
		{
			pTerrainStruct->GetStructure(Crc::ConstCRC("SoundActions"), &p_default_actions);
			//Dbg_Message("Found default terrain:");
			//Script::PrintContents(p_default_actions);
		}
		else if (terrain_type == terrain)
		{
			pTerrainStruct->GetStructure(Crc::ConstCRC("SoundActions"), &p_terrain_actions);
	...
	Script::CArray *pLevelTerrainArray = Script::GetArray(Crc::ConstCRC("level_terrain"));
	if (pLevelTerrainArray)
	{
		for (uint i = 0; i < pLevelTerrainArray->GetSize(); i++)
		{
			...
			if (terrain_type == vTERRAIN_DEFAULT)
			{
				pTerrainStruct->GetStructure(Crc::ConstCRC("SoundActions"), &p_level_default_actions);
			} else if (terrain_type == terrain)
			{
				pTerrainStruct->GetStructure(Crc::ConstCRC("SoundActions"), &p_level_terrain_actions);
			}
...
Script::CStruct *		CTerrainManager::s_choose_sound_action(Script::CStruct *p_sound_action_struct)
{
	...
	if (p_sound_action_struct->GetStructure(Crc::ConstCRC("soundAction"), &p_sound))
	{
		Dbg_MsgAssert(!p_sound->ContainsComponentNamed(Crc::ConstCRC("chance")),
	...
	for (size_t sound_idx = p_random_indexed_sound_array->GetSize(); sound_idx--; )
	{
		...
		int chance;
		p_sound->GetInteger(Crc::ConstCRC("chance"), &chance, Script::ASSERT);
...
Script::CStruct *		CTerrainManager::s_get_action_sound_struct(Script::CStruct *p_actions, ETerrainActionType action, float soundChoice)
{
	...
	Dbg_MsgAssert(!p_float_indexed_sound_array->GetStructure(p_float_indexed_sound_array->GetSize() - 1)->ContainsComponentNamed(Crc::ConstCRC("useUpTo")),
	...
	for (uint32 sound_action_idx = 0; sound_action_idx < p_float_indexed_sound_array->GetSize(); sound_action_idx++)
	{
		...
		float up_to_speed;
		if (!p_sound_action_struct->GetFloat(Crc::ConstCRC("useUpTo"), &up_to_speed))
		{
...
bool					CTerrainManager::s_get_sound_info(Script::CStruct *p_sound, STerrainSoundInfo *p_info)
{
	...
	p_sound->GetFloat( Crc::ConstCRC("loadVol"), &p_info->m_loadVol );
...
void					CTerrainManager::sPlaySfx( ETerrainActionType action, ETerrainType terrain, const Mth::Vector &pos,
