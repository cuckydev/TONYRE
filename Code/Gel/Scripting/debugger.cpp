...
static Nx::ERenderMode s_change_render_mode(Obj::CCompositeObject *p_ob, Nx::ERenderMode new_render_mode)
{
	...
	if (p_ob)
	{
		Obj::CModelComponent *p_model=(Obj::CModelComponent*)p_ob->GetComponent(Crc::ConstCRC("Model"));
		if (p_model)
		{
...
void CScriptDebugger::transmit_cscript_list()
{
	...
	p_list->AddInteger(Crc::ConstCRC("NumScripts"),num_scripts);
	//p_list->AddInteger(Crc::ConstCRC("NumScriptsUsingBigBuffer"),total_scripts_using_big_buffer);
	//p_list->AddFloat(Crc::ConstCRC("AverageNumReturnAddresses"),((float)total_num_return_addresses)/num_scripts);
	...
	while (p_script)
	{
		...
		//p_script_info->AddInteger(Crc::ConstCRC("NumReturnAddresses"),p_script->GetNumReturnAddresses());
		//p_script_info->AddInteger(Crc::ConstCRC("UsingBigCallstackBuffer"),p_script->UsingBigCallstackBuffer());
		...
		{
			p_script_info->AddChecksum(NONAME,Crc::ConstCRC("Spawned"));
		}
...
void CScriptDebugger::setup_default_camera_viewing_position()
{
	...
	if (mp_viewed_object)
	{
		Obj::CModelComponent *p_model_component=(Obj::CModelComponent*)mp_viewed_object->GetComponent(Crc::ConstCRC("Model"));
