...
{
	p_info->AddChecksum(NONAME,Crc::ConstCRC("ScriptCachingDisabled"));
}
#endif
...
void CScriptCache::GetDebugInfo( Script::CStruct* p_info )
{
	...
	p_script_cache_info->AddInteger(Crc::ConstCRC("NumUsedScripts"),m_num_used_scripts);
	p_script_cache_info->AddInteger(Crc::ConstCRC("MaxUsedScripts"),m_max_used_scripts);
	...
	p_script_cache_info->AddInteger(Crc::ConstCRC("MaxDecompressCount"),max);
	p_script_cache_info->AddInteger(Crc::ConstCRC("AverageDecompressCount"),total/MAX_DECOMPRESS_COUNTS);
	...
	if (num_scripts_in_cache)
	{
		...
		p_script_cache_info->AddInteger(Crc::ConstCRC("TotalScriptSizes"),alloc_total);
		p_script_cache_info->AddArrayPointer(Crc::ConstCRC("CachedScripts"),p_scripts_array);
	}
		
	p_info->AddStructurePointer(Crc::ConstCRC("ScriptCacheInfo"),p_script_cache_info);
}
