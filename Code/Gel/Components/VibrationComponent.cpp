...
CBaseComponent::EMemberFunctionResult CVibrationComponent::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	switch ( Checksum )
	{
		...
		case Crc::ConstCRC("Vibrate"):
		{
			// NOTE: ignoring pause state in VibrationComponent for now
			// Don't let it be switched on if skater is paused
			if (!pParams->ContainsFlag(Crc::ConstCRC("Off")) && (Mdl::FrontEnd::Instance()->GamePaused() || GetObj()->IsPaused())) break;
			...
			int actuator_idx = 0;
			pParams->GetInteger(Crc::ConstCRC("actuator"), &actuator_idx);
			...
			float percent = 0;
			pParams->GetFloat(Crc::ConstCRC("percent"), &percent);
