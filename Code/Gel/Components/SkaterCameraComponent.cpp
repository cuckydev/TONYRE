...
CBaseComponent::EMemberFunctionResult CSkaterCameraComponent::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	switch ( Checksum )
	{
		...
		case Crc::ConstCRC("SC_ShakeCamera"):
		{
			float duration	= 0.0f;
			float vert_amp	= 0.0f;
		...
		case Crc::ConstCRC("SC_SetMode"):
		{
		
			int mode = 0;
			pParams->GetInteger(Crc::ConstCRC("mode"),&mode, true);
...
void CSkaterCameraComponent::SetMode( ESkaterCamMode mode, float time )
{
	...
	p_struct->GetFloat( Crc::ConstCRC("lerp_y"),					&mLerpY );
	p_struct->GetFloat( Crc::ConstCRC("vert_air_lerp_xz"),			&mVertAirLerpXZ );
	p_struct->GetFloat( Crc::ConstCRC("vert_air_lerp_y"),			&mVertAirLerpY );
	p_struct->GetFloat( Crc::ConstCRC("grind_lerp"),				&mGrindLerp );
	...
	p_struct->GetFloat( Crc::ConstCRC("zoom_lerp"),			&zoom_lerp );
	p_struct->GetFloat( Crc::ConstCRC("grind_zoom"),			&mGrindZoom );
	p_struct->GetFloat( Crc::ConstCRC("big_air_trick_zoom"),	&mBigAirTrickZoom );
	p_struct->GetFloat( Crc::ConstCRC("lip_trick_zoom"),		&mLipTrickZoom );
	mZoomLerp			= zoom_lerp;

	// Get camera style name.
	p_struct->GetText( Crc::ConstCRC("name"), &p_name );
