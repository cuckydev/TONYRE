...
{
	int nodeNum = -1;
	pParams->GetInteger( Crc::ConstCRC("nodeIndex"), &nodeNum, Script::NO_ASSERT );
...
CBaseComponent::EMemberFunctionResult CMotionComponent::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	switch ( Checksum )
	{
		...
		{	
			float range=100.0f;
			pParams->GetFloat(Crc::ConstCRC("Range"),&range);
			range*=12.0f;
			
			pScript->GetParams()->RemoveComponent( Crc::ConstCRC("Ob") );
			
			CCompositeObject* p_closest_ob = GetNextObjectOnPath( range );

			if ( p_closest_ob )
				pScript->GetParams()->AddChecksum( Crc::ConstCRC("Ob"), p_closest_ob->GetID() );			
			break;
		}
		...
		{
			float dist=0.0f;
			pParams->GetFloat(Crc::ConstCRC("Dist"),&dist);
		...
		case 0xe606eba2: // Obj_Hover
		{
			...
			else
			{
				...
				pParams->GetFloat(Crc::ConstCRC("Amp"),&m_hover_amp);
				float f=1.0f;
				pParams->GetFloat(Crc::ConstCRC("Freq"),&f);
...
//				printf( "hover period = %d random number = %d\n", m_hover_period, random_number );
		...
		{
			m_jump_speed = 150.0f;
			pParams->GetFloat( Crc::ConstCRC("Speed"), &m_jump_speed );
			m_jump_original_speed = m_jump_speed;

			m_jump_gravity = -360.0f;
			pParams->GetFloat( Crc::ConstCRC("Gravity"), &m_jump_gravity );
...
#else
		GetObj()->SpawnScriptPlease( scriptChecksum, pScriptParams );
#endif
		return;
	}
...
void CMotionComponent::GetDebugInfo( Script::CStruct* p_info )
{
	...
	p_info->AddFloat( Crc::ConstCRC("m_deceleration"), m_deceleration );
	p_info->AddFloat( Crc::ConstCRC("m_max_vel"), m_max_vel );
	p_info->AddFloat( Crc::ConstCRC("m_vel_z"), m_vel_z );
	p_info->AddFloat( Crc::ConstCRC("m_stop_dist"), m_stop_dist );
	...
	p_info->AddInteger( Crc::ConstCRC("m_point_stick_to_ground"), m_point_stick_to_ground );
	
	p_info->AddFloat( Crc::ConstCRC("m_moveto_dist"), m_moveto_dist );
	p_info->AddFloat( Crc::ConstCRC("m_moveto_dist_traveled"), m_moveto_dist_traveled );
	p_info->AddFloat( Crc::ConstCRC("m_moveto_speed_target"), m_moveto_speed_target );
	p_info->AddFloat( Crc::ConstCRC("m_moveto_acceleration"), m_moveto_acceleration );	
	
	p_info->AddInteger( Crc::ConstCRC("m_moveto_node_num"), m_moveto_node_num );
	...
	p_info->AddVector( Crc::ConstCRC("m_stored_pos"), m_stored_pos );
	p_info->AddInteger( Crc::ConstCRC("m_stored_node"), m_stored_node );
	...
	if (mp_pathOb)
	{
		...
		p_info->AddStructure( Crc::ConstCRC("mp_pathOb"), p_pathOb_params );
...
void CMotionComponent::FollowLeader_Init( Script::CStruct* pParams )
{
	...
	float distance=0.0f;
	pParams->GetFloat(Crc::ConstCRC("Distance"),&distance);
	mp_follow_ob->SetDistance(distance);
	
	uint32 name=0;
	pParams->GetChecksum(Crc::ConstCRC("Name"),&name);
	mp_follow_ob->SetLeaderName(name);
	
	if (pParams->ContainsFlag(Crc::ConstCRC("OrientY")))
	{
