...
{
	return ( Script::GetInt( Crc::ConstCRC("ped_debug"), false ) );
}
...
//			Dbg_Message( "Ped jumps forward to avoid skater" );
			pParams->GetChecksum( Crc::ConstCRC("JumpForward"), &animChecksum, true );
			break;

		case ( 1 ):	// JUMP LEFT
			{
//				Dbg_Message( "Ped jumps left to avoid skater" );
				if ( pParams->GetChecksum( Crc::ConstCRC("JumpLeft"), &animChecksum, false ) )
				{
					flipped = 0;
				}
				else
				{
					pParams->GetChecksum( Crc::ConstCRC("JumpRight"), &animChecksum, true );
					flipped = 1;
				}
...
//			Dbg_Message( "Ped jumps back to avoid skater" );
			pParams->GetChecksum( Crc::ConstCRC("JumpBack"), &animChecksum, true );
			break;

		case ( 3 ):	// JUMP RIGHT
			{
//				Dbg_Message( "Ped jumps right to avoid skater" );
				if ( pParams->GetChecksum( Crc::ConstCRC("JumpRight"), &animChecksum, false ) )
				{
					flipped = 0;
				}
				else
				{
					pParams->GetChecksum( Crc::ConstCRC("JumpLeft"), &animChecksum, true );
					flipped = 1;
				}
...
*/
		...
		case 0xd2aa4d2d:	// Ped_BackOnOriginalPath
		{
			...
			Mth::Vector posPlus;
			Mth::Vector posMinus;
			posPlus = GetObj()->m_pos + normalToOriginalPath * safeDist;
			posMinus = GetObj()->m_pos - normalToOriginalPath * safeDist;
