...
CBaseComponent::EMemberFunctionResult CSpecialItemComponent::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	switch ( Checksum )
	{
		...
		case 0x827c6bd1:  // CreateSpecialItem
			{
				int index;
				uint32 bone = 0;
				pParams->GetInteger( Crc::ConstCRC("index"), &index, true );
		...
		case 0xce887492:  // SpecialItemExists
			{
				int index;
				if ( pParams->GetInteger( Crc::ConstCRC("index"), &index, false ) )
				{
...
//	pGameObj->FlagAsSpecialItem();
	...
	if ( pNodeData->GetChecksum( Crc::ConstCRC("CleanupScript"), &cleanupScript ) )
	{
		pTempStructure->AddChecksum( Crc::ConstCRC("CleanupScript"), cleanupScript );
	}
