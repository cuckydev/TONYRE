...
{
	pParams->GetChecksum(Crc::ConstCRC("bone"), &m_bone_name, Script::ASSERT);
	pParams->GetInteger(Crc::ConstCRC("num_links"), &m_num_links, Script::ASSERT);
	pParams->GetFloat(Crc::ConstCRC("link_length"), &m_link_length, Script::ASSERT);
	pParams->GetChecksum(Crc::ConstCRC("color"), &m_color, Script::ASSERT);
...
void CRibbonComponent::Update()
{
	...
	float min_y = GetObj()->GetPos()[Y] + 0.1f;
	for (int n = m_num_links; n--; )
	{
