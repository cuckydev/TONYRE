...
{
	// optimization KLUDGE
//	if (Script::GetInteger(Crc::ConstCRC("vehicle_mode")))
//	{
//		GetObj()->Pause(true);
//		return;
//	}
...
//	float horiz_control = GetInputComponentFromObject( GetObj())->GetControlPad().m_scaled_rightX;
	...
	else if (Mth::DotProduct(target_facing, subject_facing) < cosf(Mth::DegToRad(s_get_gunslinger_param(Crc::ConstCRC("lock_angle")))))
	{
		use_subject_facing = false;
	}
	...
	else // if (!mp_target_walk_component->UseDPadCamera())
	{
		...
		{
			full_slerp_speed = s_get_gunslinger_param(Crc::ConstCRC("full_slerp_speed"));
			min_slerp_speed = s_get_gunslinger_param(Crc::ConstCRC("min_slerp_speed"));
		}
		else
		{
...
Mth::Vector CWalkCameraComponent::get_tripod_pos( bool instantly )
{
	...
	{
		float lerp_xz = GetTimeAdjustedSlerp(s_get_gunslinger_param(Crc::ConstCRC("lerp_xz")), Tmr::FrameLength());
		float lerp_y = GetTimeAdjustedSlerp(s_get_gunslinger_param(Crc::ConstCRC("lerp_y")), Tmr::FrameLength());
