...
//			break;
	...
	{
		m_curb_float_height = Mth::Lerp(m_curb_float_height, 0.0f, s_get_gunslinger_param(Crc::ConstCRC("curb_float_lerp_down_rate")) * m_frame_length);
	}
...
//	if (should_bail_from_frame()) return;
	...
	switch (m_anim_scale_speed)
	{
		...
		case HANGMOVE:
			mp_animation_component->SetAnimSpeed(m_anim_effective_speed / s_get_gunslinger_param(Crc::ConstCRC("hang_move_speed")), false, false);
			break;
...
CBaseComponent::EMemberFunctionResult CWalkComponent::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	switch ( Checksum )
	{
		...
		case Crc::ConstCRC("Walk_Jump"):
		{
			...
			jump();
//			jump(Mth::Lerp(
//				s_get_gunslinger_param(Crc::ConstCRC("min_jump_factor")), 
//				1.0f,
//				Mth::ClampMax(mp_input_component->GetControlPad().m_x.GetPressedTime() / s_get_gunslinger_param(Crc::ConstCRC("hold_time_for_max_jump")), 1.0f)
//			));
			break;
		}
		
//		case Crc::ConstCRC("Walk_HangJump"):
//		{
//			// jump strength scales with the length the jump button has been held
//			jump(s_get_gunslinger_param(Crc::ConstCRC("hang_jump_factor")), true);
//			break;
//		}
		...
		{
			uint32 checksum;
			if (m_anim_effective_speed < s_get_gunslinger_param(Crc::ConstCRC("max_slow_walk_speed")))
			{
...
void CWalkComponent::calculate_horizontal_speed_and_facing ( float &horizontal_speed )
{
	...
	// setup frame's event
	if (desired_speed <= s_get_gunslinger_param(Crc::ConstCRC("max_stand_speed")))
	{
	...
	if( m_control_magnitude > 0.0f )
	{
		...
		if (horizontal_speed < s_get_gunslinger_param(Crc::ConstCRC("max_rotate_in_place_speed")) && dot < cosf(Mth::DegToRad(s_get_gunslinger_param(Crc::ConstCRC("max_rotate_in_place_angle")))))
		{
			// low speed rotate to desired orientation with no speed change
			
			float delta_angle = Mth::DegToRad(s_get_gunslinger_param(Crc::ConstCRC("rotate_in_place_rate"))) * m_control_magnitude * m_frame_length;
			bool left_turn = -m_facing[Z] * m_control_direction[X] + m_facing[X] * m_control_direction[Z] < 0.0f;
			
			if (!m_run_toggle)
			{
				delta_angle *= s_get_gunslinger_param(Crc::ConstCRC("walk_rotate_factor"));
			}
		...
		{
			if (dot > -cosf(Mth::DegToRad(s_get_gunslinger_param(Crc::ConstCRC("max_reverse_angle")))))
			{
				// if the turn angle is soft
				
				// below a speed threshold, scale up the turn rate
				float turn_factor;
				if (horizontal_speed < s_get_gunslinger_param(Crc::ConstCRC("max_pop_speed")))
				{
					// quick turn
					turn_factor = Mth::Lerp(s_get_gunslinger_param(Crc::ConstCRC("best_turn_factor")), s_get_gunslinger_param(Crc::ConstCRC("worse_turn_factor")), horizontal_speed / s_get_gunslinger_param(Crc::ConstCRC("max_pop_speed"))); 
				}
				else
				{
					// slower turn
					turn_factor = s_get_gunslinger_param(Crc::ConstCRC("worse_turn_factor"));
				}
			...
			else
			{
				...
				if (horizontal_speed > s_get_gunslinger_param(Crc::ConstCRC("min_skid_speed")))
				{
	...
	{
		horizontal_speed = Mth::Lerp(horizontal_speed, desired_speed, s_get_gunslinger_param(Crc::ConstCRC("decel_factor")) * m_frame_length);
		
		if (desired_speed == 0.0f)
		{
			if (horizontal_speed > s_get_gunslinger_param(Crc::ConstCRC("walk_speed")))
			{
				m_frame_event = Crc::ConstCRC("Skid");
			}
			else if (m_last_frame_event == Crc::ConstCRC("Skid") && horizontal_speed > s_get_gunslinger_param(Crc::ConstCRC("stop_skidding_speed")))
			{
				m_frame_event = Crc::ConstCRC("Skid");
			}
		}
	...
	else
	{
		if (m_run_toggle)
		{
			horizontal_speed += s_get_gunslinger_param(Crc::ConstCRC("run_accel_rate")) * m_frame_length;
		}
		else
		{
			horizontal_speed += s_get_gunslinger_param(Crc::ConstCRC("walk_accel_rate")) * m_frame_length;
		}
		if (horizontal_speed > desired_speed)
		{
...
bool  CWalkComponent::adjust_horizonal_vel_for_environment ( void )
{
	...
	float feeler_length = s_get_gunslinger_param(Crc::ConstCRC("feeler_length"));
	float feeler_height = s_get_gunslinger_param(Crc::ConstCRC("feeler_height"));
	...
	for (int n = 0; n < vNUM_FEELERS + 1; n++)
	{
		...
		if (n == vNUM_FEELERS)
		{
			...
			feeler.m_end[Y] += m_vertical_vel * m_frame_length + 0.5f * -s_get_gunslinger_param(Crc::ConstCRC("gravity")) * Mth::Sqr(m_frame_length);
		}
		else
		{
			feeler.m_start = m_pos;
	...
	if (m_state == WALKING_GROUND)
	{
		if (check_for_wall_push())
		{
			...
			if (Tmr::ElapsedTime(m_wall_push_test.test_start_time) > s_get_gunslinger_param(Crc::ConstCRC("hop_delay")))
			{
				if (maybe_climb_up_ladder() || /*maybe_hop_to_hang() ||*/ maybe_jump_low_barrier()) return false;
			}
			else if (Tmr::ElapsedTime(m_wall_push_test.test_start_time) > s_get_gunslinger_param(Crc::ConstCRC("barrier_jump_delay")))
			{
...
void CWalkComponent::adjust_facing_for_adjusted_horizontal_vel (   )
{
	...
	float turn_ratio;
	if (horizontal_speed > s_get_gunslinger_param(Crc::ConstCRC("max_wall_turn_speed_threshold")))
	{
		turn_ratio = s_get_gunslinger_param(Crc::ConstCRC("wall_turn_factor")) * m_frame_length;
	}
	else
	{
		turn_ratio = Mth::LinearMap(
			0.0f,
			s_get_gunslinger_param(Crc::ConstCRC("wall_turn_factor")) * m_frame_length,
			horizontal_speed,
			s_get_gunslinger_param(Crc::ConstCRC("wall_turn_speed_threshold")),
			s_get_gunslinger_param(Crc::ConstCRC("max_wall_turn_speed_threshold"))
		);
	}
...
float CWalkComponent::adjust_desired_speed_for_slope ( float desired_speed )
{
	...
	// skip if not running
	if (desired_speed <= s_get_gunslinger_param(Crc::ConstCRC("walk_speed"))) return desired_speed;
...
void CWalkComponent::respond_to_ground (   )
{
	...
	CFeeler feeler;
	feeler.m_start = m_pos;
	feeler.m_start[Y] += s_get_gunslinger_param(Crc::ConstCRC("snap_up_height"));
	feeler.m_end = m_pos;
	feeler.m_end[Y] -= s_get_gunslinger_param(Crc::ConstCRC("snap_down_height"));
...
//			if (should_bail_from_frame()) return;
	...
	if (Mth::Abs(snap_distance) > s_get_gunslinger_param(Crc::ConstCRC("max_unnoticed_ground_snap")))
	{
...
void CWalkComponent::adjust_curb_float_height (   )
{
	...
	feeler.m_end = m_pos + s_get_gunslinger_param(Crc::ConstCRC("curb_float_feeler_length")) * feeler_direction;
    feeler.m_end[Y] += 0.5f;
	#ifdef __USER_DAN__
	if (Script::GetInteger(Crc::ConstCRC("walking_debug_lines")))
	{
	...
	if (feeler.GetCollision())
	{
		...
		feeler.m_start[Y] = m_pos[Y] + s_get_gunslinger_param(Crc::ConstCRC("snap_up_height"));
		#ifdef __USER_DAN__
		if (Script::GetInteger(Crc::ConstCRC("walking_debug_lines")))
		{
		...
		if (feeler.GetCollision())
		{
			...
			if (Mth::Abs(m_curb_float_height) < 0.01f && m_control_magnitude == 0.0f && m_horizontal_vel.LengthSqr() < Mth::Sqr(s_get_gunslinger_param(Crc::ConstCRC("min_curb_height_adjust_vel"))))
			{
			...
			else
			{
				...
				m_curb_float_height = Mth::Lerp(m_curb_float_height, appropriate_curb_float_height, s_get_gunslinger_param(Crc::ConstCRC("curb_float_lerp_up_rate")) * m_frame_length);
			}
...
//	if (should_bail_from_frame()) return;
	...
	m_pos += m_horizontal_vel * m_frame_length;
	m_pos[Y] += m_vertical_vel * m_frame_length + 0.5f * -s_get_gunslinger_param(Crc::ConstCRC("gravity")) * Mth::Sqr(m_frame_length);
	
	m_vertical_vel += -s_get_gunslinger_param(Crc::ConstCRC("gravity")) * m_frame_length;
	...
	if (mp_input_component->GetControlPad().m_R1.GetPressed() && !m_ignore_grab_button
		&& ((m_previous_state != WALKING_HANG && m_previous_state != WALKING_LADDER) || Tmr::ElapsedTime(m_state_timestamp) > s_get_gunslinger_param(Crc::ConstCRC("rehang_delay"))))
	{
...
void CWalkComponent::control_horizontal_vel (   )
{
	...
	float perp_desired_vel = s_get_gunslinger_param(Crc::ConstCRC("jump_adjust_speed")) * adjust_magnitude * Mth::DotProduct(m_control_direction, perp_direction);
	...
	perp_vel = Mth::Lerp(perp_vel, perp_desired_vel, s_get_gunslinger_param(Crc::ConstCRC("jump_accel_factor")) * m_frame_length);
		
	// adjust velocity parallel to jump direction
	
	// desired parallel velocity
	float para_desired_vel = s_get_gunslinger_param(Crc::ConstCRC("jump_adjust_speed")) * adjust_magnitude * Mth::DotProduct(m_control_direction, m_primary_air_direction);
	...
	else
	{
		...
		para_vel = Mth::Lerp(para_vel, para_desired_vel, s_get_gunslinger_param(Crc::ConstCRC("jump_accel_factor")) * m_frame_length);
	}
...
void CWalkComponent::adjust_vertical_vel_for_ceiling (   )
{
	...
	feeler.m_start = m_pos;
	feeler.m_end = m_pos;
	feeler.m_end[Y] += s_get_gunslinger_param(Crc::ConstCRC("walker_height"));
...
bool CWalkComponent::maybe_jump_low_barrier (   )
{
	...
	float top_feeler_height = s_get_gunslinger_param(Crc::ConstCRC("lowest_hang_height"));
	float feeler_length = 2.0f * s_get_gunslinger_param(Crc::ConstCRC("feeler_length"));
	float height_increment = (top_feeler_height - s_get_gunslinger_param(Crc::ConstCRC("feeler_height"))) / vNUM_BARRIER_HEIGHT_FEELERS;
	...
	for (int i = 0; i < 3; i++)
	{
		...
		if (Mth::DotProduct(mp_contacts[n].normal, m_control_direction) > -cosf(Mth::DegToRad(s_get_gunslinger_param(Crc::ConstCRC("barrier_jump_max_angle")))))
		{
	...
	float jump_height = lowest_height + height_increment + s_get_gunslinger_param(Crc::ConstCRC("barrier_jump_min_clearance")) - m_pos[Y];
	float required_vertical_velocity = sqrtf(2.0f * s_get_gunslinger_param(Crc::ConstCRC("gravity")) * jump_height);
...
{
	float horizontal_reach = 1.0f + s_get_gunslinger_param(Crc::ConstCRC("feeler_length"));
	float vertical_height = 1.0f + s_get_gunslinger_param(Crc::ConstCRC("walker_height"));;
	float vertical_depth = 1.0f + s_get_gunslinger_param(Crc::ConstCRC("snap_down_height"));
...
void CWalkComponent::get_controller_input (   )
{
	...
	if (Mth::Abs(angle) < Mth::DegToRad(s_get_gunslinger_param(Crc::ConstCRC("forward_tolerance"))))
	{
		angle = 0.0f;
	}
	...
	if (control_pad.m_leftX == 0.0f && control_pad.m_leftY == 0.0f)
	{
		...
		else
		{
			...
			if (m_state == WALKING_GROUND)
			{
				...
				Tmr::Time full_run_dpad_delay = static_cast< Tmr::Time >(s_get_gunslinger_param(Crc::ConstCRC("full_run_dpad_delay")));
				Tmr::Time start_run_dpad_delay = static_cast< Tmr::Time >(s_get_gunslinger_param(Crc::ConstCRC("start_run_dpad_delay")));
				
				if (elapsed_time < start_run_dpad_delay)
				{
					m_control_magnitude = s_get_gunslinger_param(Crc::ConstCRC("walk_point"));
				}
				else if (elapsed_time < full_run_dpad_delay)
				{
					m_control_magnitude = Mth::SmoothMap(
						s_get_gunslinger_param(Crc::ConstCRC("walk_point")),
						1.0f,
						elapsed_time,
        ...
        if (m_state == WALKING_GROUND && Mth::Abs(angle) < Mth::DegToRad(90.0f + 5.0f) && (forced_run() || m_control_magnitude > s_get_gunslinger_param(Crc::ConstCRC("walk_point"))))
        {
			if (forced_run() || m_control_magnitude == 1.0f)
			{
				m_control_direction += s_get_gunslinger_param(Crc::ConstCRC("dpad_control_damping_factor")) * camera_forward;
			}
			else
			{
				// smoothly interpolate between damping and no damping
				m_control_direction += Mth::SmoothMap(0.0f, s_get_gunslinger_param(Crc::ConstCRC("dpad_control_damping_factor")), m_control_magnitude, s_get_gunslinger_param(Crc::ConstCRC("walk_point")), 1.0f)
					* camera_forward;
			}
...
float CWalkComponent::calculate_desired_speed (   )
{
	...
	float walk_point = s_get_gunslinger_param(Crc::ConstCRC("walk_point"));
	if (m_control_magnitude <= walk_point)
	{
		m_run_toggle = false;
		return Mth::LinearMap(0.0f, s_get_gunslinger_param(Crc::ConstCRC("walk_speed")), m_control_magnitude, 0.3f, walk_point);
	}
	else
	{
		m_run_toggle = true;
		return Mth::LinearMap(s_get_gunslinger_param(Crc::ConstCRC("walk_speed")), s_get_gunslinger_param(Crc::ConstCRC("run_speed")), m_control_magnitude, walk_point, 1.0f);
	}
}
...
bool CWalkComponent::determine_stand_pos ( const Mth::Vector& proposed_stand_pos, Mth::Vector& stand_pos, CFeeler& feeler )
{
	...
	float max_height_adjustment = s_get_gunslinger_param(Crc::ConstCRC("feeler_height")) / s_get_gunslinger_param(Crc::ConstCRC("push_feeler_length")) * s_get_gunslinger_param(Crc::ConstCRC("pull_up_offset_forward"));
