...
void CCameraLookAroundComponent::Update()
{
	...
	if (Script::GetInt(Crc::ConstCRC("Viewer_controller"), false))
	{
	...
	if (!Script::GetInt(Crc::ConstCRC("select_shift")))
	{
		if (p_control_pad->m_select.GetTriggered())
		{
			...
			if (GetObj()->GetID() == Crc::ConstCRC("SkaterCam0"))
			{
				Script::RunScript(Crc::ConstCRC("UserSelectSelect"));
			}
			else
			{
				Script::RunScript(Crc::ConstCRC("UserSelectSelect2"));
			}
		}
	}
...
CBaseComponent::EMemberFunctionResult CCameraLookAroundComponent::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	switch ( Checksum )
	{
		...
		case 0xb8f23899:	// SC_SetSkaterCamOverride
		{
			...
			pParams->GetFloat( Crc::ConstCRC("tilt"), &tilt, Script::ASSERT );
			pParams->GetFloat( Crc::ConstCRC("time"), &time, Script::ASSERT );
			pParams->GetFloat( Crc::ConstCRC("zoom"), &zoom );
...
void DrawMap (   )
{
	...
	Mth::Matrix matrix = GetObj()->GetMatrix();
