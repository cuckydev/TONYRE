...
void CVehicleComponent::InitFromStructure( Script::CStruct* pParams )
{
	...
	else if (pParams->ContainsFlag(Crc::ConstCRC("no_debug")))
	{
		m_draw_debug_lines = 0;
	}
	...
	if (pParams->ContainsComponentNamed(Crc::ConstCRC("suspension_center_of_mass")))
	{
		float suspension_center_of_mass_offset;
		pParams->GetFloat(Crc::ConstCRC("suspension_center_of_mass"), &suspension_center_of_mass_offset);
	...
	if (pParams->ContainsComponentNamed(Crc::ConstCRC("mass")))
	{
		pParams->GetFloat(Crc::ConstCRC("mass"), &m_inv_mass);
	...
	if (pParams->ContainsComponentNamed(Crc::ConstCRC("moment_of_inertia")))
	{
		pParams->GetVector(Crc::ConstCRC("moment_of_inertia"), &m_inv_moment_body);
	...
	pParams->GetFloat(Crc::ConstCRC("body_restitution"), &m_body_restitution);
	
	// vehicle body's coefficient of friction
	pParams->GetFloat(Crc::ConstCRC("body_friction"), &m_body_friction);
	...
	pParams->GetFloat(Crc::ConstCRC("in_air_slerp_velocity_cutoff"), &m_in_air_slerp_vel_cutoff);
	
	// time over which in-air slerping lerps to full strength after takeoff
	pParams->GetFloat(Crc::ConstCRC("in_air_slerp_time_delay"), &m_in_air_slerp_time_delay);
	
	// in-air slerping strength
	pParams->GetFloat(Crc::ConstCRC("in_air_slerp_strength"), &m_in_air_slerp_strength);
	
	// special slerping below standard velocity threshold
	m_vert_correction = pParams->ContainsFlag(Crc::ConstCRC("vert_correction"));
	
	// maximum steering angle (degrees)
	if (pParams->ContainsComponentNamed(Crc::ConstCRC("max_steering_angle")))
	{
        pParams->GetFloat(Crc::ConstCRC("max_steering_angle"), &m_max_steering_angle);
	...
	pParams->GetFloat(Crc::ConstCRC("constant_rotational_damping"), &m_const_rotvel_damping);
	pParams->GetFloat(Crc::ConstCRC("quadratic_rotational_damping"), &m_quad_rotvel_damping);
	
	// if the car can be exited via triangle
	m_exitable = pParams->ContainsFlag(Crc::ConstCRC("exitable"));
	
	// if the car has a handbrke
	m_no_handbrake = pParams->ContainsFlag(Crc::ConstCRC("no_handbrake"));
	
	// the vehicle's body's interaction with the environment is expressed as an array of rectangular colliders
	if (pParams->ContainsComponentNamed(Crc::ConstCRC("colliders")))
	{
		Script::CArray* p_colliders_array;
		pParams->GetArray(Crc::ConstCRC("colliders"), &p_colliders_array, Script::ASSERT);
	...
	if (pParams->ContainsComponentNamed(Crc::ConstCRC("engine")))
	{
		Script::CStruct* p_engine_struct;
		pParams->GetStructure(Crc::ConstCRC("engine"), &p_engine_struct, Script::ASSERT);
		
		// base drive torque of the engine; multiplied by the gear and differential ratio (ft-lbs)
		if (p_engine_struct->ContainsComponentNamed(Crc::ConstCRC("drive_torque")))
		{
			p_engine_struct->GetFloat(Crc::ConstCRC("drive_torque"), &m_engine.drive_torque);
			m_engine.drive_torque *= 12.0f;
		}
		
		// base drag torque of the engine; multiplied by the square of the gear ratio (ft-lbs)
		if (p_engine_struct->ContainsComponentNamed(Crc::ConstCRC("drag_torque")))
		{
			p_engine_struct->GetFloat(Crc::ConstCRC("drag_torque"), &m_engine.drag_torque);
			m_engine.drag_torque *= 12.0f;
		}
		
		// when rpm reaches this speed, the transmition upshifts (rpm)
		if (p_engine_struct->ContainsComponentNamed(Crc::ConstCRC("upshift_rpm")))
		{
			p_engine_struct->GetFloat(Crc::ConstCRC("upshift_rpm"), &m_engine.upshift_rotvel);
		...
		p_engine_struct->GetFloat(Crc::ConstCRC("differential_ratio"), &m_engine.differential_ratio);
		
		// instead of having a true reverse gear, all gears are scaled down by this ratio when in reverse
		p_engine_struct->GetFloat(Crc::ConstCRC("reverse_torque_ratio"), &m_engine.reverse_torque_ratio);
		
		// gear ratios; torque and rpm are multiplied by these when translated up and down the drive train
		if (p_engine_struct->ContainsComponentNamed(Crc::ConstCRC("gear_ratios")))
		{
			Script::CArray* p_engine_gear_ratios_array;
			p_engine_struct->GetArray(Crc::ConstCRC("gear_ratios"), &p_engine_gear_ratios_array, Script::ASSERT);
	...
	if (pParams->ContainsComponentNamed(Crc::ConstCRC("all_wheels")))
	{
	...
	if (pParams->ContainsComponentNamed(Crc::ConstCRC("save")))
	{
		Mth::Vector orientation_vector;
		orientation_vector.Set();
		pParams->GetVector(Crc::ConstCRC("orientation"), &orientation_vector);
	...
	m_pos = GetObj()->GetPos();
	if (pParams->ContainsComponentNamed(Crc::ConstCRC("pos")))
	{
		pParams->GetVector(Crc::ConstCRC("pos"), &m_pos);
		GetObj()->SetPos(m_pos);
	}
	
	m_skater_visible = m_skater_visible || pParams->ContainsFlag(Crc::ConstCRC("make_skater_visible"));
	if (pParams->ContainsFlag(Crc::ConstCRC("make_skater_visible")))
	{
		pParams->GetVector(Crc::ConstCRC("skater_pos"), &m_skater_pos, Script::ASSERT);
...
CBaseComponent::EMemberFunctionResult CVehicleComponent::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	switch ( Checksum )
	{
		...
		case Crc::ConstCRC("Vehicle_Kick"):
		{
			Mth::Vector linear;
			Mth::Vector angular;
		...
		case Crc::ConstCRC("Vehicle_Wake"):
			m_state = AWAKE;
			m_consider_sleeping_count = 0;
			break;

		// @script | Vehicle_MoveToRestart | teleport the vehicle to the restart node
		case Crc::ConstCRC("Vehicle_MoveToRestart"):
		{
		...
		case Crc::ConstCRC("Vehicle_PlaceBeforeCamera"):
		{
		...
		case Crc::ConstCRC("Vehicle_AdjustGravity"):
			pParams->GetFloat(Crc::ConstCRC("Percent"), &m_gravity_override_fraction, Script::ASSERT);
			m_gravity_override_fraction *= 1.0f / 100.0f;
			pParams->GetFloat(Crc::ConstCRC("Duration"), &m_gravity_override_timer, Script::ASSERT);
		...
		// case Crc::ConstCRC("Vehicle_ForceBrake"):
			// m_force_brake = true;
			// break;
			
		// @script | Vehicle_HandbrakeActive | returns true if the car has a handbrake
		case Crc::ConstCRC("Vehicle_HandbrakeActive"):
		...
		case Crc::ConstCRC("Vehicle_AllWheelsAreInContact"):
		...
		case Crc::ConstCRC("Vehicle_LostCollision"):
		{
			Mth::Vector offset;
			pParams->GetVector(Crc::ConstCRC("Offset"), &offset, Script::ASSERT);
			ApplyArtificialCollision(offset);
			break;
		}
		
		// @script | Vehicle_IsSkaterVisible | true if skater should be visible while driving
		case Crc::ConstCRC("Vehicle_IsSkaterVisible"):
...
void CVehicleComponent::GetDebugInfo ( Script::CStruct *p_info )
{
	...
	for (size_t n = m_num_wheels; n--; )
	{
		...
		{
			Crc::ConstCRC("NO_STATE"),
			Crc::ConstCRC("OUT_OF_CONTACT"),
			Crc::ConstCRC("UNDER_GRIPPING"),
			Crc::ConstCRC("GRIPPING"),
			Crc::ConstCRC("SLIPPING"),
			Crc::ConstCRC("SKIDDING"),
			Crc::ConstCRC("HANDBRAKE_THROTTLE"),
			Crc::ConstCRC("HANDBRAKE_LOCKED"),
		};
	...
	if (m_skater_visible)
	{
		p_info->AddVector(Crc::ConstCRC("skater_pos"), m_skater_pos);
		p_info->AddChecksum(Crc::ConstCRC("skater_anim"), m_skater_anim);
	}
...
void CVehicleComponent::ApplyArtificialCollision ( const Mth::Vector& offset )
{
	...
	float impulse_forward = (1.0f + Mth::PlusOrMinus(0.5f)) * Script::GetFloat(Crc::ConstCRC("vehicle_physics_netcoll_forward_impulse"));
	float impulse_sideways = Mth::PlusOrMinus(1.5f) * Script::GetFloat(Crc::ConstCRC("vehicle_physics_netcoll_sideways_impulse"));
	float impulse_upwards = (1.0f + Mth::PlusOrMinus(0.2f)) * Script::GetFloat(Crc::ConstCRC("vehicle_physics_netcoll_upwards_impulse"));
	
	Mth::Vector impulse = impulse_forward * impulse_direction + impulse_sideways * sideways;
	impulse[Y] += impulse_upwards;
	
	float rotate_spin = Mth::PlusOrMinus(1.5f) * Script::GetFloat(Crc::ConstCRC("vehicle_physics_netcoll_spin_impulse"));
	float rotate_flip = (1.0f + Mth::PlusOrMinus(0.8f)) * Script::GetFloat(Crc::ConstCRC("vehicle_physics_netcoll_flip_impulse"));
	...
	m_artificial_collision_timer = Script::GetFloat(Crc::ConstCRC("vehicle_physics_artificial_collision_duration"));
...
void CVehicleComponent::update_wheel_from_structure ( SWheel& wheel, Script::CStruct* p_wheel_struct )
{
	...
	if (p_wheel_struct->ContainsComponentNamed(Crc::ConstCRC( "steering")))
	{
		...
		switch (checksum)
		{
			case Crc::ConstCRC("left"):
				...
				break;
			case Crc::ConstCRC("right"):
				wheel.steering = SWheel::RIGHT;
				break;
			case Crc::ConstCRC("fixed"):
	...
	if (p_wheel_struct->ContainsComponentNamed(Crc::ConstCRC("drive")))
	{
		p_wheel_struct->GetChecksum(Crc::ConstCRC("drive"), &checksum);
		switch (checksum)
		{
			case Crc::ConstCRC("yes"):
				wheel.drive = true;
				break;
			case Crc::ConstCRC("no"):
				wheel.drive = false;
				break;
			default:
	...
	// physics model
	p_wheel_struct->GetFloat(Crc::ConstCRC("spring_rate"), &wheel.spring);
	...
	p_wheel_struct->GetFloat(Crc::ConstCRC("damping_rate"), &wheel.damping);

	// wheel radius (in)
	// larger radius wheels are harder for the engine to rotate and the brake and rolling resistance to stop
	p_wheel_struct->GetFloat(Crc::ConstCRC("radius"), &wheel.radius);

	if (p_wheel_struct->ContainsComponentNamed(Crc::ConstCRC("moment")))
	{
		p_wheel_struct->GetFloat(Crc::ConstCRC("moment"), &wheel.inv_moment);
		wheel.inv_moment = vVP_GRAVITATIONAL_ACCELERATION / wheel.inv_moment;
	}
	
	p_wheel_struct->GetFloat(Crc::ConstCRC("max_draw_y_offset"), &wheel.max_draw_y_offset);
	...
	p_wheel_struct->GetFloat(Crc::ConstCRC("static_friction"), &wheel.static_friction);
	...
	p_wheel_struct->GetFloat(Crc::ConstCRC("dynamic_friction"), &wheel.dynamic_friction);
	...
	p_wheel_struct->GetFloat(Crc::ConstCRC("handbrake_throttle_friction"), &wheel.handbrake_throttle_friction);
	p_wheel_struct->GetFloat(Crc::ConstCRC("handbrake_locked_friction"), &wheel.handbrake_locked_friction);

	// slip speed below which tires undergrip (mph)
	// should be around 1 to 2 mph
	if (p_wheel_struct->ContainsComponentNamed(Crc::ConstCRC("min_static_grip_velocity")))
	{
		p_wheel_struct->GetFloat(Crc::ConstCRC("min_static_grip_velocity"), &wheel.min_static_velocity);
	...
	if (p_wheel_struct->ContainsComponentNamed(Crc::ConstCRC("max_static_grip_velocity")))
	{
		p_wheel_struct->GetFloat(Crc::ConstCRC("max_static_grip_velocity"), &wheel.max_static_velocity);
	...
	if (p_wheel_struct->ContainsComponentNamed(Crc::ConstCRC("min_dynamic_grip_velocity")))
	{
		p_wheel_struct->GetFloat(Crc::ConstCRC("min_dynamic_grip_velocity"), &wheel.min_dynamic_velocity);
		wheel.min_dynamic_velocity = MPH_TO_IPS(wheel.min_dynamic_velocity);
	}

	// torque which the brake exerts on the wheel (ft-lbs)
	if (p_wheel_struct->ContainsComponentNamed(Crc::ConstCRC("brake_torque")))
	{
		p_wheel_struct->GetFloat(Crc::ConstCRC("brake_torque"), &wheel.brake.torque);
	...
	if (p_wheel_struct->ContainsComponentNamed(Crc::ConstCRC("handbrake_torque")))
	{
		p_wheel_struct->GetFloat(Crc::ConstCRC("handbrake_torque"), &wheel.brake.handbrake_torque);
...
void CVehicleComponent::get_input (   )
{
	...
	if (brake_pressed)
	{
		...
		if (exit_request)
		{
			...
			{
				// signal an exit
				GetObj()->BroadcastEvent(Crc::ConstCRC("ExitVehicleRequest"));
			}
		}
	}
	else
...
void CVehicleComponent::draw_shadow (   )
{
	...
	Mth::Vector start_pos = GetObj()->GetPos();
