...
void CStatsManagerComponent::InitFromStructure( Script::CStruct* pParams )
{
    ...
    pProfile->SetPropertyValue( Crc::ConstCRC("air"), 3 );
    pProfile->SetPropertyValue( Crc::ConstCRC("switch"), 3 );
    pProfile->SetPropertyValue( Crc::ConstCRC("spin"), 3 );
    pProfile->SetPropertyValue( Crc::ConstCRC("run"), 3 );
    pProfile->SetPropertyValue( Crc::ConstCRC("speed"), 3 );
    pProfile->SetPropertyValue( Crc::ConstCRC("lip_balance"), 3 );
    pProfile->SetPropertyValue( Crc::ConstCRC("rail_balance"), 3 );
    pProfile->SetPropertyValue( Crc::ConstCRC("manual_balance"), 3 );
    pProfile->SetPropertyValue( Crc::ConstCRC("flip_speed"), 3 );
    pProfile->SetPropertyValue( Crc::ConstCRC("ollie"), 3 );
    */
...
void CStatsManagerComponent::Update()
{
    ...
    if (mpSkaterPhysicsControlComponent->IsSkating())
    {
        ...
        {
            new_event = Crc::ConstCRC("vert_air");
            if ( last_event != Crc::ConstCRC("vert_air") )
            {
                // vert take off
                vert_set = 1;
                vert_height = 0;
        ...
        {
            new_event = Crc::ConstCRC("air");
            if ( last_event != Crc::ConstCRC("air") )
            {
    ...
    {
        // Combo goals
        if ( stat_goals[i].goaltype == Crc::ConstCRC("combo") )
        {
        ...
        // Multiplier goals
        if ( stat_goals[i].goaltype == Crc::ConstCRC("multiplier") )
        {
            if (!stat_goals[i].complete)
            {
                if (!stat_goals[i].shown)
        ...
        // Vert Distance
        if ( stat_goals[i].goaltype == Crc::ConstCRC("vertdist") )
        {
            if (!stat_goals[i].complete)
            {
                if (!stat_goals[i].shown)
        ...
        // Vert Height
        if ( stat_goals[i].goaltype == Crc::ConstCRC("vertheight") )
        {
            if (!stat_goals[i].complete)
            {
                if (!stat_goals[i].shown)
        ...
        // Vert Score
        if ( stat_goals[i].goaltype == Crc::ConstCRC("vertscore") )
        {
            if (!stat_goals[i].complete)
            {
                if (!stat_goals[i].shown)
        ...
        // Vert Spin
        if ( stat_goals[i].goaltype == Crc::ConstCRC("vertspin") )
        {
            if (!stat_goals[i].complete)
            {
                if (!stat_goals[i].shown)
        ...
        // Jump Distance
        if ( stat_goals[i].goaltype == Crc::ConstCRC("olliedist") )
        {
            if (!stat_goals[i].complete)
            {
                if (!stat_goals[i].shown)
        ...
        // Jump Height
        if ( stat_goals[i].goaltype == Crc::ConstCRC("highollie") )
        {
            if (!stat_goals[i].complete)
            {
                if (!stat_goals[i].shown)
        ...
        // Jump Drop
        if ( stat_goals[i].goaltype == Crc::ConstCRC("olliedrop") )
        {
            if (!stat_goals[i].complete)
            {
                if (!stat_goals[i].shown)
        ...
        // Num Fliptricks
        if ( stat_goals[i].goaltype == Crc::ConstCRC("numfliptricks") )
        {
            if (!stat_goals[i].complete)
            {
                if (!stat_goals[i].shown)
        ...
        // Num grabtricks
        if ( stat_goals[i].goaltype == Crc::ConstCRC("numgrabs") )
        {
            if (!stat_goals[i].complete)
            {
                if (!stat_goals[i].shown)
        ...
        // String Counter
        if ( stat_goals[i].goaltype == Crc::ConstCRC("stringcount") )
        {
            if (!stat_goals[i].complete)
            {
                if (!stat_goals[i].shown)
        ...
        // Trick Counter
        if ( stat_goals[i].goaltype == Crc::ConstCRC("trickcount") )
        {
            if (!stat_goals[i].complete)
            {
                if (!stat_goals[i].shown)
...
*/
        ...
        case 0x78e7d855: // StatsManager_GetGoalStatus
        {
            ...
            pScript->GetParams()->AddInteger( Crc::ConstCRC("status"), status );
        }
        break;
            ...
            if (!stat_goals[i].shown)
            {
                ...
                switch ( checksum )
                {
                    ...
                    case 0xef24413b: //"manual"
                        if ( stat_goals[i].goaltype == Crc::ConstCRC("manualtime") )
                        {
                            if ( time >= value )
                            {
                                ShowStatsMessage(i);
                    ...
                    case 0x8d10119d: //"slide"
                        if ( stat_goals[i].goaltype == Crc::ConstCRC("grindtime") )
                        {
                            if ( time >= value )
                            {
                                ShowStatsMessage(i);
                        ...
                        break;
                    case 0xa549b57b: //"lip"
                        if ( stat_goals[i].goaltype == Crc::ConstCRC("liptime") )
                        {
                            if ( time >= value )
                            {
                                ShowStatsMessage(i);
            ...
            if (stat_goals[i].shown)
            {
                ...
                switch ( stat )
                {
                    ...
                    case 0xae798769: //"lip_balance"
                        p_params->AddString( "text", Script::GetString( Crc::ConstCRC("lip_increase_text") ) );
                        break;
                    case 0xf0d90109: //"speed"
                        p_params->AddString( "text", Script::GetString( Crc::ConstCRC("speed_increase_text") ) );
                        break;
                    case 0x9b65d7b8: //"ollie"
                        p_params->AddString( "text", Script::GetString( Crc::ConstCRC("ollie_increase_text") ) );
                        break;
                    case 0x439f4704: //"air"
                        p_params->AddString( "text", Script::GetString( Crc::ConstCRC("air_increase_text") ) );
                        break;
                    case 0x6dcb497c: //"flip_speed"
                        p_params->AddString( "text", Script::GetString( Crc::ConstCRC("flip_increase_text") ) );
                        break;
                    case 0x9016b4e7: //"switch"
                        p_params->AddString( "text", Script::GetString( Crc::ConstCRC("switch_increase_text") ) );
                        break;
                    case 0xedf5db70: //"spin"
                        p_params->AddString( "text", Script::GetString( Crc::ConstCRC("spin_increase_text") ) );
                        break;
                    case 0xaf895b3f: //"run"
                        p_params->AddString( "text", Script::GetString( Crc::ConstCRC("run_increase_text") ) );
                        break;
                    default:
                        break;
                }
