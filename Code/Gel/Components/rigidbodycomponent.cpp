...
void CRigidBodyComponent::InitFromStructure( Script::CStruct* pParams )
{
	...
	pParams->GetVector(Crc::ConstCRC("center_of_mass"), &m_center_of_mass);

	pParams->GetFloat(Crc::ConstCRC("const_acc"), &m_const_acc);

	pParams->GetFloat(Crc::ConstCRC("coeff_restitution"), &m_coeff_restitution);

	pParams->GetFloat(Crc::ConstCRC("coeff_friction"), &m_coeff_friction);
	
	pParams->GetFloat(Crc::ConstCRC("spring_const"), &m_spring_const);

	if (pParams->ContainsComponentNamed(Crc::ConstCRC("linear_velocity_sleep_point")))
	{
		pParams->GetFloat(Crc::ConstCRC("linear_velocity_sleep_point"), &m_linear_velocity_sleep_point_sqr);
		m_linear_velocity_sleep_point_sqr *= m_linear_velocity_sleep_point_sqr;
	}

	if (pParams->ContainsComponentNamed(Crc::ConstCRC("angular_velocity_sleep_point")))
	{
		pParams->GetFloat(Crc::ConstCRC("angular_velocity_sleep_point"), &m_angular_velocity_sleep_point_sqr);
		m_angular_velocity_sleep_point_sqr *= m_angular_velocity_sleep_point_sqr;
	}

	pParams->GetFloat(Crc::ConstCRC("skater_collision_impulse_factor"), &m_skater_collision_impulse_factor);
	
	pParams->GetFloat(Crc::ConstCRC("skater_collision_rotation_factor"), &m_skater_collision_rotation_factor);

	pParams->GetFloat(Crc::ConstCRC("skater_collision_radius"), &m_skater_collision_radius);

	pParams->GetFloat(Crc::ConstCRC("skater_collision_application_radius"), &m_skater_collision_application_radius);

	if (pParams->ContainsComponentNamed(Crc::ConstCRC("skater_collision_assent")))
	{
		float skater_collision_assent;
		pParams->GetFloat(Crc::ConstCRC("skater_collision_assent"), &skater_collision_assent);
	...
	pParams->GetFloat(Crc::ConstCRC("ignore_skater_duration"), &m_ignore_skater_duration);
	
	pParams->GetChecksum(Crc::ConstCRC("CollideScript"), &m_script_names.collide);
	pParams->GetChecksum(Crc::ConstCRC("BounceScript"), &m_script_names.bounce);
	...
	// get sound setup
	if (pParams->ContainsComponentNamed(Crc::ConstCRC("SoundType")))
	{
		uint32 sound_type_id;
		pParams->GetChecksum(Crc::ConstCRC("SoundType"), &sound_type_id, Script::ASSERT);
	...
	m_flags.Set(DIE_UPON_SLEEP, pParams->ContainsFlag(Crc::ConstCRC("DieUponSettling")));
	...
	if (pParams->ContainsFlag(Crc::ConstCRC("box")))
	{
		object_geometry = BOX;
	}
	else if (pParams->ContainsFlag(Crc::ConstCRC("pyramid")))
	{
		object_geometry	= PYRAMID;
	}
	else if (pParams->ContainsFlag(Crc::ConstCRC("cylinder")))
	{
		object_geometry	= CYLINDER;
	}
	else if (pParams->ContainsFlag(Crc::ConstCRC("triangle")))
	{
		object_geometry	= TRIANGLE;
	}

	// setup contacts using an array
	if (pParams->ContainsComponentNamed(Crc::ConstCRC("contacts")))
	{
		Script::CArray* p_array;
		pParams->GetArray(Crc::ConstCRC("contacts"), &p_array);
	...
	switch (object_geometry)
	{
		...
		case CYLINDER:
		{
			...
			pParams->GetFloat(Crc::ConstCRC("radius"), &top_radius);
		...
		case TRIANGLE:
		{
			...
			pParams->GetVector(Crc::ConstCRC("dimensions"), &half_dimensions);
	...
	if (pParams->ContainsComponentNamed(Crc::ConstCRC("directed_friction")))
	{
		Script::CArray* p_array;
		pParams->GetArray(Crc::ConstCRC("directed_friction"), &p_array);
	...
	if (pParams->ContainsComponentNamed(Crc::ConstCRC("rotate_contacts")))
	{
		Mth::Vector angles;
		pParams->GetVector(Crc::ConstCRC("rotate_contacts"), &angles, Script::ASSERT);
		Mth::Matrix matrix;
	...
	if (pParams->ContainsComponentNamed(Crc::ConstCRC("Angles")))
	{
		Mth::Vector angles;
	...
	if (pParams->ContainsComponentNamed(Crc::ConstCRC("center_of_mass")))
	{
		if (pParams->ContainsComponentNamed(Crc::ConstCRC("center_of_mass")))
		{
			pParams->GetVector(Crc::ConstCRC("center_of_mass"), &center_of_mass);
		}
	}
	...
	if (!pParams->GetFloat(Crc::ConstCRC("mass_over_moment"), &m_mass_over_moment))
	{
	...
	s_skater_head_height = GetPhysicsFloat(Crc::ConstCRC("Skater_default_head_height"));
...
CBaseComponent::EMemberFunctionResult CRigidBodyComponent::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	switch ( Checksum )
	{
		...
		case Crc::ConstCRC("RigidBody_Wake"):
			wake();
			break;

		// @script | RigidBody_Sleep | puts the rigidbody to sleep
		case Crc::ConstCRC("RigidBody_Sleep"):
			sleep();
			break;
		...
		case Crc::ConstCRC("RigidBody_Kick"):
		{
			...
			if (pParams->GetVector(Crc::ConstCRC("rotvel"), &v))
			{
				m_rotvel += v;
			}
			if (pParams->GetVector(Crc::ConstCRC("pos"), &v))
			{
		...
		case Crc::ConstCRC("RigidBody_Reset"):
		...
		case Crc::ConstCRC("RigidBody_EnablePlayerCollision"):
		...
		case Crc::ConstCRC("RigidBody_MatchVelocityTo"):
		{
			...
			bool apply_random_adjustment = pParams->ContainsFlag(Crc::ConstCRC("ApplyRandomAdjustment"));
...
void CRigidBodyComponent::GetDebugInfo(Script::CStruct *p_info)
{
	...
	{
		Crc::ConstCRC("ASLEEP"), Crc::ConstCRC("AWAKE")
	};
	...
	if (m_sound_setup.bounce_sound)
	{
		...
		*p_struct += *m_sound_setup.bounce_sound;
		p_info->AddStructurePointer(Crc::ConstCRC("BounceSound"), p_struct);
	}
	if (m_sound_setup.collide_sound)
	{
		Script::CStruct* p_struct = new Script::CStruct;
		*p_struct += *m_sound_setup.collide_sound;
		p_info->AddStructurePointer(Crc::ConstCRC("CollideSound"), p_struct);
	}
...
void CRigidBodyComponent::handle_skater_collisions (   )
{
	for (int n = Mdl::Skate::Instance()->GetNumSkaters(); n--; )
	{
		...
		{
			#ifdef __NOPT_ASSERT__
			if (m_sound_type_id && Script::GetInteger(Crc::ConstCRC("DynamicRigidbodySounds")))
			{
			...
			if (m_sound_setup.collide_sound)
			{
				...
				m_sound_setup.collide_sound->AddFloat(Crc::ConstCRC("Percent"), percent);
...
void CRigidBodyComponent::resolve_collisions (   )
{
	...
	if (max_collision_velocity > m_sound_setup.bounce_velocity_callback_threshold)
	{
		...
		if (m_sound_setup.bounce_sound && NUM_VOICES - Sfx::CSfxManager::Instance()->GetNumSoundsPlaying() > vRP_NUM_UNTOUCHABLE_VOICES)
		{
			...
			m_sound_setup.bounce_sound->AddFloat(Crc::ConstCRC("Percent"), percent);
...
void CRigidBodyComponent::get_sound_setup ( uint32 sound_type_id )
{
	...
	if (p_sound_struct->ContainsComponentNamed(Crc::ConstCRC("CollideSound")))
	{
		p_sound_struct->GetStructure(Crc::ConstCRC("CollideSound"), &p_struct, Script::ASSERT);
		if (m_sound_setup.collide_sound)
		{
			delete m_sound_setup.collide_sound;
		}
	...
	if (p_sound_struct->ContainsComponentNamed(Crc::ConstCRC("BounceSound")))
	{
		p_sound_struct->GetStructure(Crc::ConstCRC("BounceSound"), &p_struct, Script::ASSERT);
		if (m_sound_setup.bounce_sound)
		{
			delete m_sound_setup.bounce_sound;
		}
		...
		*m_sound_setup.bounce_sound += *p_struct;
		...
		if (p_sound_struct->ContainsComponentNamed(Crc::ConstCRC("CollideMuteDelay")))
		{
			int time;
			p_sound_struct->GetInteger(Crc::ConstCRC("CollideMuteDelay"), &time);
			m_sound_setup.collide_mute_delay = static_cast< Tmr::Time >(time);
		}
		if (p_sound_struct->ContainsComponentNamed(Crc::ConstCRC("GlobalCollideMuteDelay")))
		{
			int time;
			p_sound_struct->GetInteger(Crc::ConstCRC("GlobalCollideMuteDelay"), &time);
			m_sound_setup.global_collide_mute_delay = static_cast< Tmr::Time >(time);
		}
		p_sound_struct->GetFloat(Crc::ConstCRC("BounceVelocityCallbackThreshold"), &m_sound_setup.bounce_velocity_callback_threshold);
		p_sound_struct->GetFloat(Crc::ConstCRC("BounceVelocityFullSpeed"), &m_sound_setup.bounce_velocity_full_speed);
	}
}
