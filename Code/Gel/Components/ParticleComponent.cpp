...
void CParticleComponent::InitFromStructure( Script::CStruct* pParams )
{
	...
	{
		pParams->GetChecksum( Crc::ConstCRC("Name"), &part.m_Name );
	}

	pParams->GetChecksum( Crc::ConstCRC("Type"), &part.m_Type );
	part.m_UseMidpoint = pParams->ContainsFlag(Crc::ConstCRC("UseMidPoint"));
	
	pParams->GetFloat( Crc::ConstCRC("scale"), &scale, false );

	pParams->GetFloat( Crc::ConstCRC("StartRadius"), &part.m_Radius[Nx::vBOX_START] );
	pParams->GetFloat( Crc::ConstCRC("MidRadius"), &part.m_Radius[Nx::vBOX_MID] );
	pParams->GetFloat( Crc::ConstCRC("EndRadius"), &part.m_Radius[Nx::vBOX_END] );

	orientation.Ident();
	if( pParams->ContainsFlag( Crc::ConstCRC("orient_to_vel" )))
	{
		Mth::Vector vel;
	...
	pParams->GetInteger( Crc::ConstCRC("MaxStreams"), &part.m_MaxStreams );
	pParams->GetFloat( Crc::ConstCRC("EmitRate"), &part.m_EmitRate );
	pParams->GetFloat( Crc::ConstCRC("Lifetime"), &part.m_Lifetime );
	pParams->GetFloat( Crc::ConstCRC("MidPointPCT"), &part.m_MidpointPct );
	
	if( pParams->ContainsFlag(Crc::ConstCRC("UseStartPosition")))
	{
		pParams->GetVector( Crc::ConstCRC("StartPosition"), &part.m_BoxPos[Nx::vBOX_START] );
	}
	else
	{
	...
	pParams->GetVector( Crc::ConstCRC("BoxDimsStart"), &part.m_BoxDims[Nx::vBOX_START] );
	pParams->GetVector( Crc::ConstCRC("BoxDimsMid"), &part.m_BoxDims[Nx::vBOX_MID] );
	pParams->GetVector( Crc::ConstCRC("BoxDimsEnd"), &part.m_BoxDims[Nx::vBOX_END] );
	...
	part.m_UseMidcolor = pParams->ContainsFlag(Crc::ConstCRC("UseColorMidTime"));

	pParams->GetFloat( Crc::ConstCRC("ColorMidTime"), &part.m_MidpointPct );

	if( pParams->GetArray( Crc::ConstCRC("StartRGB"), &array ))
	{
	...
	if( pParams->GetArray( Crc::ConstCRC("EndRGB"), &array ))
	{
		color = array->GetInt( 0 );
	...
	if( pParams->GetInteger( Crc::ConstCRC("StartAlpha"), &color ))
	{
		part.m_Color[Nx::vBOX_START].a = (uint8) color;
	}
	if( pParams->GetInteger( Crc::ConstCRC("MidAlpha"), &color ))
	{
		part.m_Color[Nx::vBOX_MID].a = (uint8) color;
	}
	if( pParams->GetInteger( Crc::ConstCRC("EndAlpha"), &color ))
	{
		part.m_Color[Nx::vBOX_END].a = (uint8) color;
	}
	
	pParams->GetChecksum( Crc::ConstCRC("BlendMode"), &part.m_BlendMode );
	pParams->GetInteger( Crc::ConstCRC("AlphaCutoff"), &part.m_AlphaCutoff );
	pParams->GetInteger( Crc::ConstCRC("FixedAlpha"), &part.m_FixedAlpha );
	pParams->GetChecksum( Crc::ConstCRC("Texture"), &part.m_Texture );
	
	pParams->GetInteger( Crc::ConstCRC("lod_dist1"), &part.m_LODDistance1 );
	pParams->GetInteger( Crc::ConstCRC("lod_dist2"), &part.m_LODDistance2 );
	pParams->GetInteger( Crc::ConstCRC("SuspendDistance"), &part.m_SuspendDistance );

	part.m_RotMatrix.Ident();	  // Mick: THis was missing before, so not sure how it could have worked

	angles.Set();
	pParams->GetVector( Crc::ConstCRC("Angles"), &angles );
...
void CParticleComponent::GetDebugInfo(Script::CStruct *p_info)
{
	...
	if( mp_particle )
	{
		...
		p_info->AddFloat(Crc::ConstCRC("StartRadius"), part->m_Radius[Nx::vBOX_START] );
		p_info->AddFloat(Crc::ConstCRC("MidRadius"), part->m_Radius[Nx::vBOX_MID] );
		p_info->AddFloat(Crc::ConstCRC("EndRadius"), part->m_Radius[Nx::vBOX_END] );
	
		p_info->AddInteger(Crc::ConstCRC("MaxStreams"), part->m_MaxStreams );
		p_info->AddFloat(Crc::ConstCRC("EmitRate"), part->m_EmitRate );
		p_info->AddFloat(Crc::ConstCRC("Lifetime"), part->m_Lifetime );
		...
		p_info->AddVector(Crc::ConstCRC("BoxDimsMid"), part->m_BoxDims[Nx::vBOX_MID] );
		p_info->AddVector(Crc::ConstCRC("BoxDimsEnd"), part->m_BoxDims[Nx::vBOX_END] );
	
		p_info->AddChecksum(Crc::ConstCRC("UseColorMidTime"), part->m_UseMidcolor );
		
		p_info->AddFloat(Crc::ConstCRC("ColorMidTime"), part->m_ColorMidpointPct );
		...
		p_info->AddArray(Crc::ConstCRC("StartColor"), array );
		delete array;
		...
		p_info->AddArray(Crc::ConstCRC("MidColor"), array );
		delete array;
		...
		p_info->AddArray(Crc::ConstCRC("EndColor"), array );
		delete array;
	
		p_info->AddChecksum(Crc::ConstCRC("BlendMode"), part->m_BlendMode );
		p_info->AddInteger(Crc::ConstCRC("AlphaCutoff"), part->m_AlphaCutoff );
		p_info->AddChecksum(Crc::ConstCRC("Texture"), part->m_Texture );
		
		p_info->AddInteger(Crc::ConstCRC("lod_dist1"), part->m_LODDistance1 );
		p_info->AddInteger(Crc::ConstCRC("lod_dist2"), part->m_LODDistance2 );
		p_info->AddInteger(Crc::ConstCRC("SuspendDistance"), part->m_SuspendDistance );
	}
