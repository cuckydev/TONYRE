...
CSuspendComponent::CSuspendComponent() : CBaseComponent()
{
	...
	m_suspend_distance=Script::GetFloat(Crc::ConstCRC("DefaultMovingObjectSuspendDistance")) * 12.0f;
...
void CSuspendComponent::InitFromStructure( Script::CStruct* pParams )
{
	...
	if (pParams->GetFloat(Crc::ConstCRC("SuspendDistance"),&m_suspend_distance))
	{
		if (m_suspend_distance == 0.0f)
		{
			// if zero suspend dist, then try to get it from lod_dist1
			if (!pParams->GetFloat(Crc::ConstCRC("Lod_dist1"),&m_suspend_distance))
			{
				// else get the default distance
				m_suspend_distance=Script::GetFloat(Crc::ConstCRC("DefaultMovingObjectSuspendDistance"));
			}
		}
	...
	if (pParams->ContainsFlag(Crc::ConstCRC("NeverSuspend")))
	{
		m_never_suspend=true;
	}

	// Get the LOD distances from the node
	if (Script::GetInt(Crc::ConstCRC("NoLOD"),false))					 
	{
	...
	else
	{
		...
		pParams->GetFloat(Crc::ConstCRC("lod_dist1"),&m_lod_dist[0]);
		m_lod_dist[1] = m_lod_dist[0] + 75;						// default for LOD dist 2 is 75 feet beyond 1
		pParams->GetFloat(Crc::ConstCRC("lod_dist2"),&m_lod_dist[1]);
		pParams->GetFloat(Crc::ConstCRC("lod_dist3"),&m_lod_dist[2]);
		pParams->GetFloat(Crc::ConstCRC("lod_dist4"),&m_lod_dist[3]);
		// LOD dist is in feet
		m_lod_dist[0] *= 12.0f;
		m_lod_dist[1] *= 12.0f;
		m_lod_dist[2] *= 12.0f;
...
void CSuspendComponent::GetDebugInfo(Script::CStruct *p_info)
{
	...
	p_info->AddFloat(Crc::ConstCRC("distance_to_camera"),sqrtf(m_camera_distance_squared));
	p_info->AddFloat(Crc::ConstCRC("m_lod_dist_0"),m_lod_dist[0]);
	p_info->AddFloat(Crc::ConstCRC("m_lod_dist_1"),m_lod_dist[1]);
	p_info->AddFloat(Crc::ConstCRC("m_lod_dist_2"),m_lod_dist[2]);
	p_info->AddFloat(Crc::ConstCRC("m_lod_dist_3"),m_lod_dist[3]);
	
	p_info->AddInteger(Crc::ConstCRC("SKIPLOGIC_RETURNS"),SkipLogic());
	p_info->AddInteger(Crc::ConstCRC("SKIPRENDER_RETURNS"),SkipRender());
#endif				 
}
