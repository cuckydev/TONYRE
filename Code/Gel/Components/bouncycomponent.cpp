...
void CBouncyComponent::Update()
{
		...
		if ( m_state != BOUNCYOBJ_STATE_BOUNCING || ( ( !m_destroy_when_done ) && m_bounce_count ) )
		{
			...
			for ( i = 0; i < numSkaters; i++ )
			{
				...
				if ( Mth::DistanceSqr( m_pos, pSkater->m_pos ) < m_skater_collision_radius_squared )
				{
					...
					((CMovingObject*)GetObj())->RunNodeScript(0x58cdc0f0/*CollideScript*/,0x24031741/*CollideScriptParams*/);
					#else
    ...
    switch ( m_state )
	{
		...
		case ( BOUNCYOBJ_STATE_BOUNCING ):
			...
			((CMovingObject*)GetObj())->MovingObj_Update( );    
			#endif
		...
		case ( 1 ):
		{
			...
			if ( feeler.GetCollision())
			{
				...
				if ( fabsf( m_vel[ Y ] ) < ( m_min_bounce_vel + ( 2.0f *( m_gravity * m_time ) ) ) )
				{
					...
					((CMovingObject*)(GetObj()))->HigherLevel( false );
					#endif					
					...
					else
					{
						...
						GetExceptionComponentFromObject( GetObj() )->FlagException( 0x61682835 ); // DoneBouncing
						#endif
					}
					return;
				}
				...
				if ( origVel > m_min_bounce_vel * 2.0f )
				{
					if ( m_bounce_sound )
					{
						...
						((CMovingObject*)(GetObj()))->PlaySound_VolumeAndPan( m_bounce_sound, vol, pitch );
						#endif
					}
...
//	printf ("%d: %s: (%.2f,%.2f,%.2f),(%.2f,%.2f,%.2f)\n",__LINE__,__PRETTY_FUNCTION__,vel[X],vel[Y],vel[Z]); 
	...
	if ( m_hit_sound )
	{
		...
		float pitch = 80.0f + ( float ) Mth::Rnd( 40 );
		((CMovingObject*)(GetObj()))->PlaySound_VolumeAndPan( m_hit_sound, vol, pitch );
		#endif
	}
	...
	((CMovingObject*)(GetObj()))->HigherLevel( true );
	#endif
	SWITCH_STATE( BOUNCYOBJ_STATE_BOUNCING );
	#if 0
	// TODO - exceptions working as a component
	GetExceptionComponentFromObject( GetObj() )->FlagException( 0x7dc30ba6 ); // Bounce
	#endif
}
