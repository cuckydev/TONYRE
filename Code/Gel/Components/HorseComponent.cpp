...
CBaseComponent::EMemberFunctionResult CHorseComponent::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	switch ( Checksum )
	{
		...
		case Crc::ConstCRC("Vehicle_Reset"):
...
void CHorseComponent::calculate_horizontal_speed_and_facing( float &horizontal_speed )
{
	...
	// setup frame's event
	if (desired_speed <= s_get_param(Crc::ConstCRC("max_stand_speed")))
	{
...
//			special_acceleration = true;
	...
	{
		horizontal_speed = Mth::Lerp(horizontal_speed, desired_speed, s_get_param(Crc::ConstCRC("decel_factor")) * m_frame_length);
		...
		if (desired_speed == 0.0f)
		{
			...
			else if (m_last_frame_event == Crc::ConstCRC("Skid") && horizontal_speed > s_get_param(Crc::ConstCRC("stop_skidding_speed")))
			{
				m_frame_event = Crc::ConstCRC("Skid");
			}
		}
...
bool CHorseComponent::adjust_horizontal_vel_for_environment( bool wall_push_active )
{
	...
	for (int n = 0; n < vNUM_FEELERS + 1; n++)
	{
		...
		if (n == vNUM_FEELERS)
		{
			...
			feeler.m_end[Y] += m_vertical_vel * m_frame_length + 0.5f * -s_get_param(Crc::ConstCRC("gravity")) * Mth::Sqr(m_frame_length);
		}
		else
		{
			feeler.m_start = m_pos;
...
//				if (maybe_climb_up_ladder() || maybe_hop_to_hang() || maybe_jump_low_barrier()) return false;
			...
			else if (Tmr::ElapsedTime(m_wall_push_test.test_start_time) > s_get_param(Crc::ConstCRC("barrier_jump_delay")))
			{
...
//		}
	...
	for (int n = 0; n < vNUM_FEELERS + 1; n++)
	{
		...
		if (mp_contacts[n].distance < s_get_param(Crc::ConstCRC("push_feeler_length")) / feeler_length)
		{
...
//		&& ((m_previous_state != WALKING_HANG && m_previous_state != WALKING_LADDER) || Tmr::ElapsedTime(m_state_timestamp) > s_get_param(Crc::ConstCRC("rehang_delay"))))
//	{
	...
	if (control_pad.m_leftX == 0.0f && control_pad.m_leftY == 0.0f)
	{
		...
		else
		{
			...
			if (m_state == WALKING_GROUND)
			{
				...
				Tmr::Time full_run_dpad_delay = static_cast< Tmr::Time >(s_get_param(Crc::ConstCRC("full_run_dpad_delay")));
				Tmr::Time start_run_dpad_delay = static_cast< Tmr::Time >(s_get_param(Crc::ConstCRC("start_run_dpad_delay")));
				...
				else if (elapsed_time < full_run_dpad_delay)
				{
					m_control_magnitude = Mth::SmoothMap(
						s_get_param(Crc::ConstCRC("walk_point")),
						1.0f,
						elapsed_time,
        ...
        if (m_state == WALKING_GROUND && Mth::Abs(angle) < Mth::DegToRad(90.0f + 5.0f) && (forced_run() || m_control_magnitude > s_get_param(Crc::ConstCRC("walk_point"))))
        {
//			if (forced_run() || m_control_magnitude == 1.0f)
//			{
//				m_control_direction += s_get_param(Crc::ConstCRC("dpad_control_damping_factor")) * camera_forward;
//			}
//			else
//			{
//				// smoothly interpolate between damping and no damping
//				m_control_direction += Mth::SmoothMap(0.0f, s_get_param(Crc::ConstCRC("dpad_control_damping_factor")), m_control_magnitude, s_get_param(Crc::ConstCRC("walk_point")), 1.0f)
//					* camera_forward;
//			}
...
void CHorseComponent::adjust_vertical_vel_for_ceiling( void )
{
	...
	feeler.m_start = m_pos;
	feeler.m_end = m_pos;
	feeler.m_end[Y] += s_get_param(Crc::ConstCRC("walker_height"));
	if (!feeler.GetCollision()) return;
