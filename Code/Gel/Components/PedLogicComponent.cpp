...
CBaseComponent::EMemberFunctionResult CPedLogicComponent::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	switch ( Checksum )
	{
		...
		case Crc::ConstCRC("Ped_IsMoving"):
		...
		for ( int i = 0; i < CPathObjectTracker::MAX_OBJECTS_PER_PATH; ++i )
		{
			if ( pp_object_list[i] )
			{
				...
				if ( p_ob != GetObj() )
				{
					...
					float max_y_dist = Script::GetFloat( Crc::ConstCRC("ped_max_y_distance_to_ignore"), Script::ASSERT );
...
bool CPedLogicComponent::UpdateTargetBias()
{
	...
	float target_bias = Script::GetFloat( Crc::ConstCRC("ped_target_node_bias"), Script::ASSERT );
	ResetBias();
...
{
	Dbg_MsgAssert( m_state == Crc::ConstCRC("generic_skater"), ( "CPedLogicComponent::UpdateSkaterSoundTerrain called on non-skater ped" ) );
...
Script::CStruct* CPedLogicComponent::GetJumpParams( Script::CStruct* pNodeData, bool is_vert )
{
	...
	if ( is_spine )
	{
		...
		m_spine_start_angle = spine_start + Script::GetFloat( Crc::ConstCRC("ped_skater_spine_rotation_slop") );
	}
...
void CPedLogicComponent::Bail()
{
	...
	Script::StopScriptsUsingThisObject_Proper( GetObj(), Crc::ConstCRC("ped_skater_grind_bail") );
	Script::StopScriptsUsingThisObject_Proper( GetObj(), Crc::ConstCRC("ped_skater_generic_bail") );
	...
	if ( m_flags & PEDLOGIC_GRINDING )
	{		
		pScript = Script::SpawnScript( Crc::ConstCRC("ped_skater_grind_bail") );
	}
	else 
	{
		pScript = Script::SpawnScript( Crc::ConstCRC("ped_skater_generic_bail") );
	}
	StopSkateActions();
	pScript->mpObject = GetObj();
}
