...
Script::CStruct *CTrickComponent::get_trigger_structure(Script::CStruct *p_struct)
{
	...
	if (Config::GetHardware()==Config::HARDWARE_XBOX)
	{
		...
		{
			p_struct->GetStructure(Crc::ConstCRC("Trigger"),&p_trigger,Script::ASSERT);
			
		}
	}
	else if (Config::GetHardware()==Config::HARDWARE_NGC)
	{
		if (!p_struct->GetStructure(Crc::ConstCRC("NGC_Trigger"),&p_trigger))
		{
			p_struct->GetStructure(Crc::ConstCRC("Trigger"),&p_trigger,Script::ASSERT);
		}
	}
...
void CTrickComponent::UpdateTrickMappings( Obj::CSkaterProfile* pSkaterProfile )
{
	...
	mpTrickMappings->AppendStructure( pSkaterProfile->GetTrickMapping( Crc::ConstCRC("trick_mapping") ) );
}
		...
		case 0x696e5e66: // ReleaseAndTap
		{
			...
			for (int i=0; i<mNumEvents; ++i)
			{
				if (!(mpButtonEvents[index].Used & IgnoreMask))
				{
					...
					if (button != Crc::ConstCRC("up") &&
						button != Crc::ConstCRC("down") &&
						button != Crc::ConstCRC("left") &&
						button != Crc::ConstCRC("right"))
					{
						e[count++]=index;
						if (count==3)
						{
...
bool CTrickComponent::RunTrick(Script::CStruct *pTrick, uint32 optionalFlag, Script::CStruct* pExtraParams)
{
    ...
    if(pTrick->GetStructure(Crc::ConstCRC("Params"),&pParams))
    {
        if (!pParams->ContainsComponentNamed(Crc::ConstCRC("IsExtra")) )
        {
	...
	else
	{
		#ifdef	__NOPT_ASSERT__
		uint32 trickslot=0;
		if (!pTrick->GetChecksum(Crc::ConstCRC("TrickSlot"),&trickslot))
		{
	...
	while (true)
	{
		...
		if (ArrayChecksum)
		{
			...
			if (!RunTrick(pTrickStruct, 0, pExtraParams))
			{
				...
				// Get the checksum of the slot ...
				uint32 TrickSlotChecksum=0;
				pTrickStruct->GetChecksum(Crc::ConstCRC("TrickSlot"),&TrickSlotChecksum);
				...
				if (TrickSlotChecksum)
				{
					...
					if (mpTrickMappings->GetInteger(TrickSlotChecksum,&create_a_trick))
					{
						...
						p_params->AddInteger(Crc::ConstCRC("trick_index"),create_a_trick);
...
void CTrickComponent::TriggerAnyManualTrick(Script::CStruct *pExtraParams)
{
	...
	if (mGotManualTrick)
	{
		...
		if (pArray)
		{
			...
			uint32 TrickSlotChecksum=0;
			pStruct->GetChecksum(Crc::ConstCRC("TrickSlot"),&TrickSlotChecksum);
			...
			uint32 ScriptChecksum=0;
			pStruct->GetChecksum(Crc::ConstCRC("Scr"),&ScriptChecksum);
			if (ScriptChecksum)
			{
				mp_skater_flip_and_rotate_component->DoAnyFlipRotateOrBoardRotateAfters(); // <- See huge comment above definition of this function.
...
void CTrickComponent::SetExtraTricks(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	if (pParams->GetChecksum(Crc::ConstCRC("Tricks"),&ExtraTrickArrayChecksum))
	{
	...
	mSpecialExtraTricksArrayChecksum=0;
	pParams->GetChecksum(Crc::ConstCRC("Special"),&mSpecialExtraTricksArrayChecksum);
			...
			if (!(pScriptArray || p_template_array))
			{
				...
				if (TrickSlotChecksum)
				{
					...
					if (TrickChecksum)
					{
						...
						if (pFoo)
						{
							pFoo->GetArray(Crc::ConstCRC("Scripts"),&pScriptArray);
							pFoo->GetArray(Crc::ConstCRC("Template"),&p_template_array);
							pFoo->GetString(Crc::ConstCRC("Prefix"),&p_prefix);
						}
					}		
				}
			}	
...
{
	uint32 Duration=GetPhysicsInt(Crc::ConstCRC("BashPeriod"));
	...
	float bash_factor=Bashes*GetPhysicsFloat(Crc::ConstCRC("BashSpeedupFactor"));
	float max=GetPhysicsFloat(Crc::ConstCRC("BashMaxPercentSpeedup"))/100.0f;
	if (bash_factor > max)
	{
		bash_factor=max;
	}	
...
*/
		...
		case 0xd8c79bb0: // SetQueueTricks
		{
			...
			if (mNumQueueTricksArrays==1)
			{
				...
				if (!pArray)
				{
					mpQueueTricksArrays[0]=Crc::ConstCRC("DefaultAirTricks");
				}	
			}	
			
			// Set any special tricks array required.
			mSpecialTricksArrayChecksum=0;
			pParams->GetChecksum(Crc::ConstCRC("Special"),&mSpecialTricksArrayChecksum);
			break;
		}
		...
		case 0x1fcf080a: // SetManualTricks
		{
			...
			mSpecialManualTricksArrayChecksum=0;
			pParams->GetChecksum(Crc::ConstCRC("Special"),&mSpecialManualTricksArrayChecksum);
			break;
		}
		...
		case 0x6ba35ab4: // SetExtraGrindTricks
		{
			...
			mSpecialExtraGrindTrickArrayChecksum=0;
			pParams->GetChecksum(Crc::ConstCRC("Special"),&mSpecialExtraGrindTrickArrayChecksum);
			break;
		}
		...
		{
			int duration=200;
			pParams->GetInteger(Crc::ConstCRC("Duration"),&duration);
			
			uint32 used_by=0;
			pParams->GetChecksum(Crc::ConstCRC("UsedBy"),&used_by);
		...
		{
			uint32 Slot=0;
			pParams->GetChecksum(Crc::ConstCRC("Slot"),&Slot);
		...
		{
			int spin = static_cast< int >(Mth::Abs(mTallyAngles) + GetPhysicsFloat(Crc::ConstCRC("spin_count_slop")));
			pScript->GetParams()->AddFloat(Crc::ConstCRC("Spin"), (spin / 180) * 180.0f);
			break;
		}
		...
		case 0x11ca5c42: // ClearPanel_Landed
		{
			...
			if(GetSkater()->IsLocalClient())
			{
				...
				if (mpDeferredTrickName[0])
				{
					...
					{
						// If vert, only count it if the spin is at least 360
						if (Mth::Abs(mTallyAngles)>= 360.0f - GetPhysicsFloat(Crc::ConstCRC("spin_count_slop")) + 0.1f)	// (Mick) adkisted for slop
						{
					...
					else
					{
						...
						if (Mth::Abs(mTallyAngles)>=180.0f - GetPhysicsFloat(Crc::ConstCRC("spin_count_slop")) + 0.1f)  // (Mick) adkisted for slop
						{
							// add the deferred trick to the series
							pScore->Trigger(mpDeferredTrickName, mDeferredTrickScore, mDeferredTrickFlags);
							pScore->SetSpin(mTallyAngles);
				...
				{
					Script::CStruct* p_params = new Script::CStruct;
					p_params->AddChecksum(Crc::ConstCRC("SkaterId"), GetObj()->GetID());
					GetObj()->BroadcastEvent(Crc::ConstCRC("SkaterExitCombo"), p_params);
					delete p_params;
				}
			...
			GetObj()->BroadcastEvent( Crc::ConstCRC("skaterLanded"));
		...
		case 0xb8045433: // ClearPanel_Bailed
		{
			...
			GetObj()->BroadcastEvent( Crc::ConstCRC("skaterBailed"));
...
void CTrickComponent::GetDebugInfo(Script::CStruct *p_info)
{
	...
	if (mNumEvents)
	{
		...
		for (int i=0; i<mNumEvents; ++i)
		{
			...
			p_event->AddChecksum(Crc::ConstCRC("Button"),mpButtonEvents[index].ButtonNameChecksum);
			...
			p_event->AddInteger(Crc::ConstCRC("Time"),mpButtonEvents[index].Time);
			p_event->AddInteger(Crc::ConstCRC("Used"),mpButtonEvents[index].Used);
    ...
    p_info->AddArrayPointer(Crc::ConstCRC("Events"),p_events_array);

	s_add_array_of_names(p_info,mNumQueueTricksArrays,mpQueueTricksArrays,"QueueTrickArrays");
	
	p_info->AddInteger(Crc::ConstCRC("GotManualTrick"),mGotManualTrick);
	s_add_array_of_names(p_info,mNumManualTrickArrays,mpManualTrickArrays,"ManualTrickArrays");
	p_info->AddChecksum(Crc::ConstCRC("SpecialManualTricksArray"),mSpecialManualTricksArrayChecksum);
	...
	p_info->AddInteger(Crc::ConstCRC("ExtraTricksDuration"),mExtraTricksDuration);
	p_info->AddInteger(Crc::ConstCRC("ExtraTricksStartTime"),mExtraTricksStartTime);
	s_add_array_of_names(p_info,mNumExtraTrickArrays,mpExtraTrickArrays,"ExtraTrickArrays");
	p_info->AddChecksum(Crc::ConstCRC("SpecialExtraTricksArray"),mSpecialExtraTricksArrayChecksum);
#endif				 
}
...
void CTrickComponent::IncrementNumTricksInCombo()
{
	...
	if ( mNumTricksInCombo > 3 )
	{
		...
		GetObj()->BroadcastEvent(Crc::ConstCRC("skaterStartingRun"));
	}
}
