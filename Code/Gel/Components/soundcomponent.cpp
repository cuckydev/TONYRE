...
void CSoundComponent::PlayScriptedSound( Script::CScriptStructure *pParams )
{
	...
	if (pParams->ContainsComponentNamed(Crc::ConstCRC("Percent")))
	{
		float minVol = 0.0f;
		float maxVol = 100.0f;
		...
		pParams->GetFloat(Crc::ConstCRC("MaxVol"), &maxVol);
		pParams->GetFloat(Crc::ConstCRC("MinVol"), &minVol);
		pParams->GetFloat(Crc::ConstCRC("MaxPitch"), &maxPitch);
		pParams->GetFloat(Crc::ConstCRC("MinPitch"), &minPitch);
		
		float percent;
		pParams->GetFloat(Crc::ConstCRC("Percent"), &percent, Script::ASSERT);
		percent *= (1.0f / 100.0f);
