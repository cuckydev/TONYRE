...
bool CWalkComponent::maybe_climb_up_ladder ( bool user_requested )
{
	...
	m_frame_event = Crc::ConstCRC("LadderOntoBottom");
...
bool CWalkComponent::maybe_climb_down_ladder (   )
{
    ...
    if (!Mdl::Skate::Instance()->GetRailManager()->CheckForLadderRail(
        m_pos,
        s_get_param(Crc::ConstCRC("max_horiz_snap_distance")),
        s_get_param(Crc::ConstCRC("max_vert_snap_distance")),
		false,
        this,
        rail_data,
	...
	m_frame_event = Crc::ConstCRC("LadderOntoTop");
...
bool CWalkComponent::maybe_grab_to_ladder ( const Mth::Vector& start_pos, const Mth::Vector& end_pos )
{
	...
	m_frame_event = Crc::ConstCRC("Ladder");
...
void CWalkComponent::move_to_ladder_bottom_complete (   )
{
	...
	m_along_rail_factor = s_get_param(Crc::ConstCRC("ladder_bottom_offset_up")) / rail_length;
...
void CWalkComponent::move_to_ladder_top_complete (   )
{
	...
	// calculate initial positoin along rail
	m_along_rail_factor = (rail_length - s_get_param(Crc::ConstCRC("ladder_top_offset_up"))) / rail_length;
...
bool CWalkComponent::FilterLadderUpRail ( const Mth::Vector& bottom_pos, const Mth::Vector& top_pos, const Mth::Matrix& orientation, SLadderRailData& rail_data )
{
	...
	if (Mth::DotProduct(m_facing, orientation[Z]) < cosf(Mth::DegToRad(s_get_param(Crc::ConstCRC("max_onto_ladder_angle"))))) return false;
    
    // calculate the climb position
    rail_data.climb_point = bottom_pos + ladder_direction * s_get_param(Crc::ConstCRC("ladder_bottom_offset_up"));
	rail_data.climb_point -= s_get_param(Crc::ConstCRC("ladder_climb_offset")) * orientation[Z];
...
bool CWalkComponent::FilterLadderDownRail ( const Mth::Vector& bottom_pos, const Mth::Vector& top_pos, const Mth::Matrix& orientation, SLadderRailData& rail_data )
{
    ...
    // calculate the climb position
    rail_data.climb_point = top_pos - ladder_direction * s_get_param(Crc::ConstCRC("ladder_top_offset_up"));
	rail_data.climb_point -= s_get_param(Crc::ConstCRC("ladder_climb_offset")) * orientation[Z];
...
bool CWalkComponent::FilterLadderAirGrabRail ( const Mth::Vector& bottom_pos, const Mth::Vector& top_pos, const Mth::Matrix& orientation, const Mth::Vector& preliminary_climb_point, float along_rail_factor, SLadderRailData& rail_data )
{
    ...
    float off_bottom_height = bottom_pos[Y] + ladder_direction[Y] * s_get_param(Crc::ConstCRC("ladder_bottom_offset_up"));
    ...
    float off_top_height = top_pos[Y] - ladder_direction[Y] * s_get_param(Crc::ConstCRC("ladder_top_offset_up"));
	if (preliminary_climb_point[Y] > off_top_height) return false;
	
	// move the climb point a touch back off the rail
	rail_data.climb_point = preliminary_climb_point - s_get_param(Crc::ConstCRC("ladder_climb_offset")) * orientation[Z];
...
{
	if (Mth::Abs((Mth::PI / 2.0f) - Mth::Abs(mp_input_component->GetControlPad().m_leftAngle)) > cosf(Mth::DegToRad(s_get_param(Crc::ConstCRC("ladder_control_tolerance")))))
	{
		ladder_movement();
		return;
	}
...
void CWalkComponent::ladder_movement (   )
{
	...
	// the offset from the ladder bottom to rail top
	Mth::Vector rail = mp_rail_manager->GetPos(p_top_rail_node) - mp_rail_manager->GetPos(mp_rail_start);
	Dbg_MsgAssert(rail.Length() > s_get_param(Crc::ConstCRC("ladder_top_offset_up")), ("Ladder too short for ladder animations"));
	...
	float bottom_along_rail_factor = s_get_param(Crc::ConstCRC("ladder_bottom_offset_up")) / rail_length;
	...
	float top_along_rail_factor = (rail_length - s_get_param(Crc::ConstCRC("ladder_top_offset_up"))) / rail_length;
	...
	{
		m_frame_event = up ? Crc::ConstCRC("LadderMoveUp") : Crc::ConstCRC("LadderMoveDown");
	}
	
	// calculate our new position
	m_pos = mp_rail_manager->GetPos(mp_rail_start) + m_along_rail_factor * rail - s_get_param(Crc::ConstCRC("ladder_climb_offset")) * m_facing;
	DUMP_WPOSITION
...
{
	Mth::Vector stand_pos = m_pos - s_get_param(Crc::ConstCRC("ladder_bottom_offset_forward")) * m_facing;
	stand_pos[Y] -= s_get_param(Crc::ConstCRC("ladder_bottom_offset_up"));
	...
	m_frame_event = Crc::ConstCRC("LadderOffBottom");
}
...
void CWalkComponent::off_ladder_top (   )
{
	...
	Mth::Vector stand_pos = m_pos + s_get_param(Crc::ConstCRC("ladder_top_offset_forward")) * m_facing; 
	stand_pos[Y] += s_get_param(Crc::ConstCRC("ladder_top_offset_up"));
	...
	m_frame_event = Crc::ConstCRC("LadderOffTop");
}
