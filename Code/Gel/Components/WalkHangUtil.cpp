...
bool CWalkComponent::maybe_hop_to_hang (   )
{
	...
	search_start_pos[Y] += s_get_param(Crc::ConstCRC("lowest_hang_height"));
	...
	float hop_to_hang_reach = s_get_param(Crc::ConstCRC("max_hop_to_hang_height")) - s_get_param(Crc::ConstCRC("lowest_hang_height"));
	float max_hop_to_hang_forward_reach = s_get_param(Crc::ConstCRC("max_hop_to_hang_forward_reach"));
	float max_hop_to_hang_backward_reach = s_get_param(Crc::ConstCRC("max_hop_to_hang_backward_reach"));
	...
	float hop_target_vert_vel = s_get_param(Crc::ConstCRC("hop_target_vert_vel"));
	float gravity = s_get_param(Crc::ConstCRC("gravity"));
	...
	m_frame_event = Crc::ConstCRC("Hop");
...
bool CWalkComponent::maybe_drop_to_hang (   )
{
	...
	calculate_anim_wait_facing_drift_parameters(rail_data.hang_facing, s_get_param(Crc::ConstCRC("drop_to_hang_rotate_factor")));
	
	m_anim_wait_camera_mode = AWC_GOAL;
	
	m_critical_point_offset = s_get_param(Crc::ConstCRC("hang_critical_point_horiz_offset")) * rail_data.hang_facing;
	...
	m_frame_event = Crc::ConstCRC("DropToHang");
...
{
	m_critical_point_offset = s_get_param(Crc::ConstCRC("hang_critical_point_horiz_offset")) * m_facing;
...
bool CWalkComponent::maybe_grab_to_hang ( Mth::Vector start_pos, Mth::Vector end_pos )
{
	...
	start_pos[Y] += s_get_param(Crc::ConstCRC("hang_vert_origin_offset"));
	end_pos[Y] += s_get_param(Crc::ConstCRC("hang_vert_origin_offset"));
	...
	m_frame_event = Crc::ConstCRC("Hang");
...
bool CWalkComponent::FilterHangRail ( const Mth::Vector& pos_a, const Mth::Vector& pos_b, const Mth::Vector& preliminary_hang_point, const Mth::Vector& grab_from_point, float along_rail_factor, SHangRailData& rail_data, bool drop_to_hang )
{
	...
	float hang_vert_origin_offset = s_get_param(Crc::ConstCRC("hang_vert_origin_offset")); 
	...
	if (Mth::Abs(rail_direction[Y]) > sinf(Mth::DegToRad(s_get_param(Crc::ConstCRC("hang_max_rail_ascent"))))) return false;
	...
	feeler.m_start = rail_data.hang_point + s_get_param(Crc::ConstCRC("hang_critical_point_horiz_offset")) * rail_data.hang_facing;
	feeler.m_start[Y] += s_get_param(Crc::ConstCRC("hang_init_anim_feeler_height"));
	feeler.m_end = feeler.m_start;
	feeler.m_end += s_get_param(Crc::ConstCRC("hang_init_anim_feeler_length")) * rail_data.hang_facing;
	if (feeler.GetCollision())
	{
		...
		#endif
		rail_data.initial_animation = Crc::ConstCRC("WALL");
	}
	else
	{
		#ifdef __USER_DAN__
		...
		#endif
		rail_data.initial_animation = Crc::ConstCRC("SWING");
	}
...
bool CWalkComponent::investigate_hang_rail_vicinity ( CFeeler& feeler, const Mth::Vector& preliminary_hang_point, SHangRailData& rail_data )
{
	...
	feeler.m_start = rail_data.hang_point - s_get_param(Crc::ConstCRC("ledge_front_feeler_back")) * rail_data.hang_facing;
	feeler.m_end = rail_data.hang_point + s_get_param(Crc::ConstCRC("ledge_front_feeler_forward")) * rail_data.hang_facing;
	...
	*/
	...
	rail_data.hang_point[Y] -= s_get_param(Crc::ConstCRC("hang_vert_origin_offset"));
	rail_data.hang_point += s_get_param(Crc::ConstCRC("hang_horiz_origin_offset")) * rail_data.hang_facing;
	...
	Mth::Vector critical_point = rail_data.hang_point + s_get_param(Crc::ConstCRC("hang_critical_point_horiz_offset")) * rail_data.hang_facing;
	...
	if (feeler.GetCollision())
	{
		if (m_state == WALKING_GROUND)
		{
			...
			feeler.m_end = m_pos;
			feeler.m_end[Y] += s_get_param(Crc::ConstCRC("hop_obstruction_feeler_up"));
			#ifdef __USER_DAN__
			if (Script::GetInteger(Crc::ConstCRC("walking_debug_lines")))
			{
				feeler.DebugLine(255, 0, 255, 0);
...
{
	feeler.m_start = rail_data.hang_point + s_get_param(Crc::ConstCRC("hang_critical_point_horiz_offset")) * rail_data.hang_facing
		+ s_get_param(Crc::ConstCRC("hang_obstruction_feeler_side")) * check_offset_direction;
	feeler.m_end = feeler.m_start;
	feeler.m_start[Y] += -s_get_param(Crc::ConstCRC("hang_vert_origin_offset")) + get_hang_critical_point_vert_offset();
	feeler.m_end[Y] += s_get_param(Crc::ConstCRC("hang_obstruction_feeler_up"));
	if (feeler.GetCollision())
	{
		#ifdef __USER_DAN__
		if (Script::GetInteger(Crc::ConstCRC("walking_debug_lines")))
...
void CWalkComponent::go_hop_state (   )
{
	...
	if (m_vertical_vel > -s_get_param(Crc::ConstCRC("hop_target_vert_vel")))
	{
		m_pos += m_horizontal_vel * m_frame_length;
		m_pos[Y] += m_vertical_vel * m_frame_length + 0.5f * -s_get_param(Crc::ConstCRC("gravity")) * Mth::Sqr(m_frame_length);
		
		m_vertical_vel += -s_get_param(Crc::ConstCRC("gravity")) * m_frame_length;
...
void CWalkComponent::go_hang_state (   )
{
	...
	if (m_control_pegged)
	{
		...
		if (mp_input_component->GetControlPad().IsLeftAnalogUpPressed())
		{
			...
			Mth::Vector stand_pos = m_pos + s_get_param(Crc::ConstCRC("pull_up_offset_forward")) * m_facing;
			stand_pos[Y] += s_get_param(Crc::ConstCRC("pull_up_offset_up"));
			if (maybe_pull_up_from_hang(stand_pos)) return;
			
			// try standing a little closer
			stand_pos = m_pos + 0.5f * s_get_param(Crc::ConstCRC("pull_up_offset_forward")) * m_facing;
			stand_pos[Y] += s_get_param(Crc::ConstCRC("pull_up_offset_up"));
			if (maybe_pull_up_from_hang(stand_pos)) return;
		}
	...
	// left or right
	float control_vel;
	if (Mth::Abs((Mth::PI / 2.0f) - Mth::Abs(mp_input_component->GetControlPad().m_leftAngle)) < cosf(Mth::DegToRad(s_get_param(Crc::ConstCRC("hang_vert_control_tolerance")))))
	{
		control_vel = s_get_param(Crc::ConstCRC("hang_move_speed")) * m_control_magnitude
	...
	m_hang_move_vel = Mth::Lerp(m_hang_move_vel, control_vel, s_get_param(Crc::ConstCRC("hang_move_lerp_rate")) * m_frame_length);
	
	if (Mth::Abs(m_hang_move_vel) > s_get_param(Crc::ConstCRC("hang_move_cutoff")))
	{
	...
	CFeeler feeler;
	feeler.m_start = m_frame_start_pos + s_get_param(Crc::ConstCRC("hang_critical_point_horiz_offset")) * m_facing;
	feeler.m_end = m_pos + s_get_param(Crc::ConstCRC("hang_critical_point_horiz_offset")) * m_facing;
	if (feeler.GetCollision())
	{
...
void CWalkComponent::hang_movement ( float movement )
{
	...
	if (m_along_rail_factor < 0.0f || m_along_rail_factor > 1.0f)
	{
		...
		else
		{
			...
			m_frame_event = left ? Crc::ConstCRC("HangMoveLeft") : Crc::ConstCRC("HangMoveRight");
		}
	}
	else
	{
...
bool CWalkComponent::check_environment_for_hang_movement_collisions ( const Mth::Vector& facing, float along_rail_factor, const Mth::Vector& pos, bool left, const Mth::Vector& perp_facing, Mth::Vector rail, const CRailNode* p_rail_start, const CRailNode* p_rail_end )
{
	...
	if (distance_to_end_of_rail < s_get_param(Crc::ConstCRC("hang_move_collision_side_length")) + 1.0f)
	{
	...
	CFeeler feeler;
	feeler.m_start = pos;
	feeler.m_start[Y] += s_get_param(Crc::ConstCRC("hang_move_collision_up"));
	feeler.m_start -= s_get_param(Crc::ConstCRC("hang_move_collision_back")) * facing;
	
	// feel to the side
	feeler.m_end = feeler.m_start;
	feeler.m_end += s_get_param(Crc::ConstCRC("hang_move_collision_side_length")) * check_direction;
	if (feeler.GetCollision())
	{
		#ifdef __USER_DAN__
		if (Script::GetInteger(Crc::ConstCRC("walking_debug_lines")))
	...
	feeler.m_end = feeler.m_start;
	feeler.m_end[Y] += s_get_param(Crc::ConstCRC("hang_move_collision_side_height"));
	feeler.m_end += s_get_param(Crc::ConstCRC("hang_move_collision_side_length")) * check_direction;
	if (feeler.GetCollision())
	{
		#ifdef __USER_DAN__
	...
	feeler.m_end = feeler.m_start;
	feeler.m_end[Y] += 0.5f * s_get_param(Crc::ConstCRC("hang_move_collision_side_height"));
	feeler.m_end += s_get_param(Crc::ConstCRC("hang_move_collision_side_length")) * check_direction;
	if (feeler.GetCollision())
	{
		#ifdef __USER_DAN__
	...
	feeler.m_end = feeler.m_start;
	feeler.m_end[Y] += -s_get_param(Crc::ConstCRC("hang_move_collision_up")) + get_hang_critical_point_vert_offset() - 1.0f;
	feeler.m_end += s_get_param(Crc::ConstCRC("hang_critical_point_horiz_offset")) * check_direction_facing;
	feeler.m_end += s_get_param(Crc::ConstCRC("hang_move_collision_side_length")) * check_direction;
...
bool CWalkComponent::maybe_pull_up_from_hang ( Mth::Vector& proposed_stand_pos )
{
	...
	CFeeler feeler;
	float push_feeler_length = 0.5f * s_get_param(Crc::ConstCRC("push_feeler_length"));
	...
	feeler.m_start[Y] += s_get_param(Crc::ConstCRC("hang_vert_origin_offset")) + s_get_param(Crc::ConstCRC("pull_up_obstruction_height"));
	...
	m_critical_point_offset = -(6.0f + s_get_param(Crc::ConstCRC("pull_up_offset_forward"))) * m_facing;
	...
	m_frame_event = Crc::ConstCRC("PullUpFromHang");
...
float CWalkComponent::get_hang_display_offset (   )
{
	...
	{
		return (1.0f - mp_model_component->GetModel()->GetScale()[Y]) * s_get_param(Crc::ConstCRC("hang_vert_origin_offset"));
	}
	else
	{
		return 0.0f;
...
{
	return s_get_param(Crc::ConstCRC("hang_critical_point_vert_offset"))
		- s_get_param(Crc::ConstCRC("max_cas_scaling")) * s_get_param(Crc::ConstCRC("hang_vert_origin_offset"));
}
