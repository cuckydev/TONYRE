...
uint32 get_desc_id_from_structure( Script::CStruct* pStructure )
{
	...
	uint32 descId;
	pStructure->GetChecksum( Crc::ConstCRC("desc_id"), &descId, true );
	return descId;
}
	...
	*pReturnNumStructs = 0;
	...
	for ( int i = 0; i < part_array_size; i++ )
	{
		...
		if ( allowed_to_pick )
		{
			...
			pStruct->GetChecksum( Crc::ConstCRC("random_set"), &this_random_set, Script::NO_ASSERT );
...
Script::CStruct* GetRandomOptionStructure( uint32 partChecksum, uint32 random_set )
{
	...
	for ( int i = 0; i < part_array_size; i++ )
	{
		...
		if ( random_set )
		{
			...
			// (used for matching skin tones)
			uint32 this_random_set = 0;
			if ( pStruct->GetChecksum( Crc::ConstCRC("random_set"), &this_random_set, Script::NO_ASSERT ) )
			{
				if ( this_random_set != random_set )
				{
					allow_random = false;
		...
		if ( allow_random )
		{
			...
			pStruct->GetInteger( Crc::ConstCRC("already_selected"), &already_selected, true );
	...
	pReturnStruct->AddInteger( Crc::ConstCRC("already_selected"), 1 );
...
Script::CStruct* GetRandomOptionStructureByIndex( uint32 partChecksum, int desired_index, uint32 random_set )
{
	...
	if ( !random_set )
	{
		for ( int i = 0; i < part_array_size; i++ )
		{
			...
			int random_index = -1;
			if ( pStruct->GetInteger( Crc::ConstCRC("random_index"), &random_index, Script::ASSERT ) )
			{
