...
void CBaseMenu::SetProperties(Script::CStruct *pProps)
{
	...
	if (resolve_just(pProps, Crc::ConstCRC("internal_just"), just, just+1))
		SetInternalJust(just[0], just[1]);
	
	int regular_space_val = 0;
	if (pProps->GetInteger(Crc::ConstCRC("regular_space_amount"), &regular_space_val))
		m_regular_space_val = (float) regular_space_val;
	
	pProps->GetFloat(Crc::ConstCRC("padding_scale"), &m_padding_scale);
	
	int spacing_between = 0;
	if (pProps->GetInteger(Crc::ConstCRC("spacing_between"), &spacing_between))
		m_spacing_between = (float) spacing_between;
		
	if (pProps->ContainsFlag(Crc::ConstCRC("enable_pad_handling")))
		m_pad_handling_enabled = true;
	else if (pProps->ContainsFlag(Crc::ConstCRC("disable_pad_handling")))
		m_pad_handling_enabled = false;

	if (pProps->ContainsFlag(Crc::ConstCRC("allow_wrap")))
		m_allow_wrap = true;
	else if (pProps->ContainsFlag(Crc::ConstCRC("dont_allow_wrap")))
		m_allow_wrap = false;
...
bool CBaseMenu::PassTargetedEvent(Obj::CEvent *pEvent, bool broadcast)
{
    ...
    if (pEvent->GetType() == Obj::CEvent::TYPE_NOTIFY_CHILD_LOCK)
    {
        ...
        if (need_new_focus_event && m_in_focus) 
        {
			...
			{
				Script::CStruct data;
				data.AddInteger(Crc::ConstCRC("controller"), m_focus_controller);
	...
	if (pEvent->GetType() == Obj::CEvent::TYPE_FOCUS && !m_in_focus)
	{
		...
		if (pEvent->GetData() && pEvent->GetData()->GetInteger(Crc::ConstCRC("grid_index"), &desired_grid_index))
		{
			...
			while(p_child)
			{
				...
				if (p_child->GetIntegerTag(Crc::ConstCRC("tag_grid_x"), &found_grid_index))
				{
		...
		if ( pEvent->GetData() && pEvent->GetData()->GetChecksum( Crc::ConstCRC("child_id"), &desired_child_id ) )
		{
...
void CBaseMenu::find_focusable_item(int dir, bool include_current, bool updateGridIndex)
{
	...
	while(count)
	{
		...
		if (updateGridIndex)
		{
			...
			m_current_grid_index = -1;
			p_new_child->GetIntegerTag(Crc::ConstCRC("tag_grid_x"), &m_current_grid_index);
		}

		if ( !p_new_child->ContainsFlagTag(Crc::ConstCRC("tag_not_focusable")) && 
...
void CBaseMenu::change_selection(int dir)
{
	...
	if (m_selected_id) 
	{
		...
		if (p_child)
			p_child->GetIntegerTag(Crc::ConstCRC("tag_selected_childs_grid_index"), &grid_index_within_child);
...
{
	SetChecksumTag(Crc::ConstCRC("tag_selected_id"), m_selected_id);
	SetIntegerTag(Crc::ConstCRC("tag_selected_index"), m_selected_index);
	//printf("setup_tags(), %s\n", Script::FindChecksumName(m_id));

	SetIntegerTag(Crc::ConstCRC("tag_selected_childs_grid_index"), m_current_grid_index);
}
