...
void CElement3d::SetProperties(Script::CStruct *p_props)
{
	...
	else if (p_props->GetString( Crc::ConstCRC("model"), &p_model_name ))
	{
		UnloadModels();
	...
	else if (p_props->GetArray(Crc::ConstCRC("models"),&p_model_array))
	{
		UnloadModels();
	...
	if (p_props->GetChecksum( Crc::ConstCRC("sector"), &sector_name ))
	{
		UnloadModels();
	...
	else if (p_props->GetArray(Crc::ConstCRC("sectors"), &p_sector_array))
	{
		UnloadModels();
	...
	p_props->GetFloat(Crc::ConstCRC("AngVelY"),&m_angvel_y);
	p_props->GetFloat(Crc::ConstCRC("AngVelZ"),&m_angvel_z);

	if (p_props->ContainsFlag(Crc::ConstCRC("AngleOrderXYZ"))) m_angle_order = ANGLE_ORDER_XYZ;
	if (p_props->ContainsFlag(Crc::ConstCRC("AngleOrderXZY"))) m_angle_order = ANGLE_ORDER_XZY;
	if (p_props->ContainsFlag(Crc::ConstCRC("AngleOrderYXZ"))) m_angle_order = ANGLE_ORDER_YXZ;
	if (p_props->ContainsFlag(Crc::ConstCRC("AngleOrderYZX"))) m_angle_order = ANGLE_ORDER_YZX;
	if (p_props->ContainsFlag(Crc::ConstCRC("AngleOrderZXY"))) m_angle_order = ANGLE_ORDER_ZXY;
	if (p_props->ContainsFlag(Crc::ConstCRC("AngleOrderZYX"))) m_angle_order = ANGLE_ORDER_ZYX;

	if (p_props->ContainsFlag(Crc::ConstCRC("DisablePointing")))
	{
		m_point_type=POINT_TYPE_NONE;
	}
		
	uint32 node_checksum=0;
	if (p_props->GetChecksum(Crc::ConstCRC("NodeToPointTo"),&node_checksum))
	{
	...
	m_id_of_object_to_point_to=0;
	if (p_props->GetChecksum(Crc::ConstCRC("ObjectToPointTo"),&m_id_of_object_to_point_to))
	{
		m_point_type=POINT_TYPE_OBJECT;
	}	
	
	m_tilt=0.0f;
	p_props->GetFloat(Crc::ConstCRC("Tilt"),&m_tilt);
	...
	if (p_props->GetStructure(CRCD(0xca1c41,"HoverParams"),&p_hover_params))
	{
		...
		float f=1.0f;
		p_hover_params->GetFloat(Crc::ConstCRC("Freq"),&f);
	...
	if (p_props->GetStructure(Crc::ConstCRC("ParentParams"),&p_parent_params))
	{
		...
		if (p_parent_params->GetInteger(Crc::ConstCRC("Skater"),&m_skater_number))
		{
		...
		p_parent_params->GetChecksum(Crc::ConstCRC("Node"),&m_parent_node_name);
	...
	if (p_props->ContainsFlag(Crc::ConstCRC("scale_to_screen_dims")))
	{
		AutoComputeScale();
	}
	...
	m_scale_with_distance=p_props->ContainsFlag(Crc::ConstCRC("ScaleWithDistance"));
	m_scale_multiplier=0.002f;
	p_props->GetFloat(Crc::ConstCRC("ScaleMultiplier"),&m_scale_multiplier);
	m_max_scale=3.0f;
	p_props->GetFloat(Crc::ConstCRC("MaxScale"),&m_max_scale);


	int viewport;
	if (p_props->GetInteger(Crc::ConstCRC("Active_Viewport"), &viewport))
	{
