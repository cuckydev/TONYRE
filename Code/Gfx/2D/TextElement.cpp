...
void CTextElement::SetProperties(Script::CStruct *pProps)
{
	...
	uint32 font_crc;
	if (pProps->GetChecksum(Crc::ConstCRC("font"), &font_crc))
		SetFont(font_crc);
	Dbg_MsgAssert(m_font_checksum, ("no font loaded"));
	
	const char *p_text;
	if (pProps->GetText(Crc::ConstCRC("text"), &p_text))
		SetText(p_text);

	Dbg_MsgAssert(m_id != Obj::CBaseManager::vNO_OBJECT_ID, ("what, no ID"));

	if (pProps->ContainsFlag(Crc::ConstCRC("blur_effect")))
		AttachBlurEffect();
	if (pProps->ContainsFlag(Crc::ConstCRC("no_blur_effect")))
		DetachBlurEffect();

	Image::RGBA blur_rgba;
	if (resolve_rgba(pProps, Crc::ConstCRC("blur_rgba"), &blur_rgba) && mp_blur_effect)
		mp_blur_effect->SetRGBA(blur_rgba);

	Script::CPair shadow_offs;
	if (pProps->GetPair(Crc::ConstCRC("shadow_offs"), &shadow_offs))
	{
		SetShadowOff(shadow_offs.mX, shadow_offs.mY);
	}
	
	if (pProps->ContainsFlag(Crc::ConstCRC("shadow")))
		SetShadowState(true);
	if (pProps->ContainsFlag(Crc::ConstCRC("no_shadow")))
	...
	if (pProps->ContainsFlag(Crc::ConstCRC("override_encoded_rgba")))
		m_override_encoded_rgba = true;
	if (pProps->ContainsFlag(Crc::ConstCRC("dont_override_encoded_rgba")))
		m_override_encoded_rgba = false;
}
...
void CTextBlockElement::SetProperties(Script::CStruct *pProps)
{
	...
	if (pProps->GetFloat(Crc::ConstCRC("internal_scale"), &m_internal_scale))
		m_object_flags |= CScreenElement::vCHANGED_STATIC_PROPS;

    pProps->GetFloat(Crc::ConstCRC("line_spacing"), &m_line_spacing_scale);
	...
	if (resolve_just(pProps, Crc::ConstCRC("internal_just"), &m_internal_just_x, &m_internal_just_y))
		m_object_flags |= CScreenElement::vCHANGED_STATIC_PROPS;
	...
	if (pProps->ContainsFlag( Crc::ConstCRC("allow_expansion") ))
		m_allow_expansion = true;
	
	if (pProps->GetText(Crc::ConstCRC("text"), pp_line_tab))
	{
