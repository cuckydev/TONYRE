	...
	if ( !skate_mod->IsMultiplayerGame() )
	{
		if ( skate_mod->m_requested_level != CRCD( 0x9f2bafb7, "load_skateshop" ) )
		{
			...
			if ( !p_element )
			{
				...
				if ( pEventData && pEventData->GetInteger( Crc::ConstCRC("device_num"), &device_num, Script::NO_ASSERT ) )
				{
...
bool ScriptCreateScreenElement(Script::CScriptStructure *pParams, Script::CScript *pScript)
{
    ...
    pScript->GetParams()->AddChecksum(Crc::ConstCRC("id"), id);
...
void SetScoreTHPS4(char* score_text, int skater_num)
{
	...
	if (p_element)
	{
		Dbg_MsgAssert((uint32)p_element->GetType() == Crc::ConstCRC("TextElement"), ("type is 0x%x", p_element->GetType()));
...
{
	Front::CScreenElementManager* p_screen_elem_man = Front::CScreenElementManager::Instance();
	CScreenElementPtr p_element = p_screen_elem_man->GetElement(Crc::ConstCRC("the_time"));
	if (p_element)
	{
		Dbg_MsgAssert((uint32) p_element->GetType() == Crc::ConstCRC("TextElement"), ("type is 0x%x", p_element->GetType()));
