...
void CScreenElement::SetProperties(Script::CStruct *pProps)
{
	...
	Script::CPair pos;
	if (pProps->GetPair(Crc::ConstCRC("pos"), &pos))
	{
	...
	Mth::Vector pos3D;
	if (pProps->GetVector(Crc::ConstCRC("pos3D"), &pos3D))
	{
	...
	Script::CPair dims;
	if (pProps->GetPair(Crc::ConstCRC("dims"), &dims))
	{
		SetDims(dims.mX, dims.mY, FORCE_DIMS);
	}

	uint32 parent_id = pManager->ResolveComplexID(pProps, Crc::ConstCRC("parent"));
	if (parent_id)
	{
	...
	if (resolve_just(pProps, Crc::ConstCRC("just"), just, just+1))
		SetJust(just[0], just[1]);
	
	
	Image::RGBA rgba;
	if (resolve_rgba(pProps, Crc::ConstCRC("rgba"), &rgba))
	{
		SetRGBA(rgba);
	}
		
	float z_priority;
	if (pProps->GetFloat(Crc::ConstCRC("z_priority"), &z_priority))
	{
		SetZPriority(z_priority);
	}
	
	if (pProps->ContainsFlag(Crc::ConstCRC("not_focusable")))
	{
		// sets a flag
		SetChecksumTag(NONAME, Crc::ConstCRC("tag_not_focusable"));
	}
	else if (pProps->ContainsFlag(Crc::ConstCRC("focusable")))
	{
		RemoveFlagTag(Crc::ConstCRC("tag_not_focusable"));
	}
	
	uint32 local_id;
	if (pProps->GetChecksum(Crc::ConstCRC("local_id"), &local_id))
	{
		SetChecksumTag(Crc::ConstCRC("tag_local_id"), local_id); 
	}

	if (pProps->ContainsFlag(Crc::ConstCRC("unpause")))
	{
		SetMorphPausedState(false);
	}

	if ( pProps->ContainsFlag( Crc::ConstCRC("block_events") ) )
	{
		m_object_flags |= vEVENTS_BLOCKED;
	}
	else if ( pProps->ContainsFlag( Crc::ConstCRC("unblock_events") ) )
	{
		m_object_flags &= ~vEVENTS_BLOCKED;
	}

	if ( pProps->ContainsFlag( Crc::ConstCRC("hide") ) )
	{
		m_object_flags |= vHIDDEN;
	}
	else if ( pProps->ContainsFlag( Crc::ConstCRC("unhide") ) )
	{
		m_object_flags &= ~vHIDDEN;
		m_object_flags |= vCHANGED_STATIC_PROPS;
	}
...
{
	// alpha value
	pStruct->AddFloat( Crc::ConstCRC("alpha"), m_local_props.alpha );
	...
	pStruct->AddArray( Crc::ConstCRC("rgba"), p_rgba );
	...
	if ( scaleX == scaleY )
		pStruct->AddFloat( Crc::ConstCRC("scale"), scaleX );
	else
		pStruct->AddPair( Crc::ConstCRC("scale"), scaleX, scaleY );
		
	// position
	pStruct->AddPair( Crc::ConstCRC("pos"), m_local_props.GetScreenPosX(), m_local_props.GetScreenPosY() );

	// are the events blocked?
	if ( EventsBlocked() )
		pStruct->AddInteger( Crc::ConstCRC("events_blocked"), 1 );
	else
		pStruct->AddInteger( Crc::ConstCRC("events_blocked"), 0 );
}
...
{	
	...
	if ( pProps->ContainsFlag( Crc::ConstCRC("restore_alpha") ) )
		SetAlpha( m_originalAlpha );
	
	float alpha;
	if ( pProps->GetFloat(Crc::ConstCRC("alpha"), &alpha) )
	{
		// TODO: temp hack for remembering alpha values		
		if ( pProps->ContainsFlag( Crc::ConstCRC("remember_alpha") ) )
	...
	if ( pProps->ContainsFlag( Crc::ConstCRC("relative_scale") ) )
		relative_scale = 1;
	if (pProps->GetFloat(Crc::ConstCRC("scale"), &scale))
	{
		SetScale(scale, scale, relative_scale );
	}
	Script::CPair scale_pair;
	if (pProps->GetPair(Crc::ConstCRC("scale"), &scale_pair))
	{
	...
	float desired_time;
	Tmr::Time anim_time;
	if (pProps->GetFloat(Crc::ConstCRC("time"), &desired_time))
	{
	...
	if (pProps->GetChecksum(Crc::ConstCRC("anim"), &anim_type))
	{
		if (anim_type == Crc::ConstCRC("fast_out"))
			m_object_flags |= vANIM_FAST_OUT << vANIM_FIRST_BIT;
		else if (anim_type == Crc::ConstCRC("fast_in"))
			m_object_flags |= vANIM_FAST_IN << vANIM_FIRST_BIT;
		else if (anim_type == Crc::ConstCRC("gentle"))
...
{
	if (Checksum == Crc::ConstCRC("SetProps"))
	{
		SetProperties(pParams);
	}
    else if (Checksum == Crc::ConstCRC("DoMorph"))
	{
		float time;
		if (pParams->GetFloat(Crc::ConstCRC("time"), &time))
		{
	...
	else if (Checksum == Crc::ConstCRC("Die"))
	{
	...
	else if ( Checksum == Crc::ConstCRC("GetProps" ) )
	{
...
bool CScreenElement::resolve_pos(Script::CStruct *pProps, float *pX, float *pY)
{
	...
	if (pProps->GetStructure(Crc::ConstCRC("pos"), &p_pos_struct))
	{
	...
	else if (pProps->GetPair(Crc::ConstCRC("pos"), &pos_pair))
	{
		*pX = pos_pair.mX;
		*pY = pos_pair.mY;
		return true;
		...
		switch(type)
		{
			...
			case ESYMBOLTYPE_NAME:
			{
				...
				for (int i = 0; i < 2; i++)
				{
					...
					if (crc == Crc::ConstCRC("left") || crc == Crc::ConstCRC("top"))
						just[i] = -1.0f;
					else if (crc == Crc::ConstCRC("right") || crc == Crc::ConstCRC("bottom"))
						just[i] = 1.0f;
					else
						just[i] = 0;
				}
...
bool CContainerElement::PassTargetedEvent(Obj::CEvent *pEvent, bool broadcast)
{
	...
	if (pEvent->GetType() == Obj::CEvent::TYPE_FOCUS || pEvent->GetType() == Obj::CEvent::TYPE_UNFOCUS)
	{
		...
		if (m_focusable_child) 
		{
			...
			Script::CStruct data;
			data.AddInteger(Crc::ConstCRC("controller"), controller);
