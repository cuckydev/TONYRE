...
void CBaseScrollingMenu::SetProperties(Script::CStruct *pProps)
{
	...
	int scroll_speed;
	if (pProps->GetInteger(Crc::ConstCRC("scroll_speed"), &scroll_speed))
		m_scroll_speed = (float) scroll_speed;
	
	pProps->GetInteger(Crc::ConstCRC("num_items_to_show"), &m_num_items_to_display);

	if (pProps->ContainsFlag(Crc::ConstCRC("reset_window_top")))
	{
		m_top_or_left_line = 0;
		m_needs_update = true;
	}
	else if (pProps->ContainsFlag(Crc::ConstCRC("reset_window_bottom")))
	{
	...
	else if ( pProps->ContainsFlag( Crc::ConstCRC("reset_window") ) )
	{
		m_needs_update = true;
	}
