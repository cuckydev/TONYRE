...
uint32 get_desc_id_from_structure( Script::CStruct* pStructure )
{
	...
	uint32 descId = 0;
	pStructure->GetChecksum( Crc::ConstCRC("desc_id"), &descId, Script::NO_ASSERT );
	return descId;
}
...
void CModelAppearance::resolve_randomized_desc_ids()
{
	...
	while ( p_comp )
	{
		...
		if ( p_comp->mType == ESYMBOLTYPE_STRUCTURE )
		{
			...
			if ( p_comp->mpStructure->ContainsFlag( Crc::ConstCRC("randomized_desc_id") ) )
			{
				...
				pActualStruct->GetChecksum( Crc::ConstCRC("random_set"), &random_set, Script::NO_ASSERT );
				
				p_comp->mpStructure->Clear();
				p_comp->mpStructure->AddChecksum( Crc::ConstCRC("desc_id"), get_desc_id_from_structure(pActualStruct) );
			}
			else if ( p_comp->mpStructure->GetInteger( Crc::ConstCRC("random_index"), &randomIndex, Script::NO_ASSERT ) )
			{
...
void CModelAppearance::set_part(uint32 partChecksum, uint32 descID, Script::CStruct* pParams)
{
	...
	pParams->GetInteger( Crc::ConstCRC("use_default_hsv"), &use_default_hsv, Script::NO_ASSERT );
	
	if ( !use_default_hsv )
	{
		int h, s, v;
		pParams->GetInteger( Crc::ConstCRC("h"), &h, Script::ASSERT );
		pParams->GetInteger( Crc::ConstCRC("s"), &s, Script::ASSERT );
		pParams->GetInteger( Crc::ConstCRC("v"), &v, Script::ASSERT );
		
		pStruct->AddInteger( Crc::ConstCRC("use_default_hsv"), 0 );
		pStruct->AddInteger( Crc::ConstCRC("h"), h );
		pStruct->AddInteger( Crc::ConstCRC("s"), s );
		pStruct->AddInteger( Crc::ConstCRC("v"), v );
	}

	pStruct->AddChecksum( Crc::ConstCRC("desc_id"), descID );
}
...
bool CModelAppearance::Init()
{
	...
	Script::CStruct* pResetStructure = Script::GetStructure( Crc::ConstCRC("appearance_init_structure"), Script::NO_ASSERT );
	if ( pResetStructure )
	{
...
void compress_model_appearance( Script::CStruct* pStruct )
{
	...
	// from the structure.
	pStruct->RemoveComponent( Crc::ConstCRC("deck_layer1") );
	pStruct->RemoveComponent( Crc::ConstCRC("deck_layer2") );
	pStruct->RemoveComponent( Crc::ConstCRC("deck_layer3") );
	pStruct->RemoveComponent( Crc::ConstCRC("deck_layer4") );
	pStruct->RemoveComponent( Crc::ConstCRC("deck_layer5") );

	pStruct->RemoveComponent( Crc::ConstCRC("head_tattoo") );
	pStruct->RemoveComponent( Crc::ConstCRC("left_bicep_tattoo") );
	pStruct->RemoveComponent( Crc::ConstCRC("left_forearm_tattoo") );
	pStruct->RemoveComponent( Crc::ConstCRC("right_bicep_tattoo") );
	pStruct->RemoveComponent( Crc::ConstCRC("right_forearm_tattoo") );
	pStruct->RemoveComponent( Crc::ConstCRC("chest_tattoo") );
	pStruct->RemoveComponent( Crc::ConstCRC("back_tattoo") );
	pStruct->RemoveComponent( Crc::ConstCRC("left_leg_tattoo") );
	pStruct->RemoveComponent( Crc::ConstCRC("right_leg_tattoo") );
}
#endif
...
bool CModelAppearance::CallMemberFunction( uint32 checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	switch ( checksum )
	{
		...
		case 0xd27427ff:		// SetChecksum
		{
			...
			pParams->GetChecksum( Crc::ConstCRC("field"), &fieldChecksum, Script::ASSERT );
		...
		case 0xe961bafa:		// PartGotFlag
		{
			...
			pParams->GetChecksum( Crc::ConstCRC("part"), &partChecksum, Script::ASSERT );
			uint32 flagChecksum;
			pParams->GetChecksum( Crc::ConstCRC("flag"), &flagChecksum, Script::ASSERT );
