...
void	CEngine::s_plat_get_metrics(Script::CStruct * p_info)
{
	...
	if (p_ps2_scene->GetEngineScene())
	{
		...
		p_ps2->AddInteger(Crc::ConstCRC("NULLEngineScene"),0);
	}	
	else
	{
		// K: Added this so that the script refresh_poly_count in lighttool.q
		// can display "N/A" for the values.
		p_ps2->AddInteger(Crc::ConstCRC("NULLEngineScene"),1);
	}

	p_ps2->AddInteger(Crc::ConstCRC("total"), p_metrics->m_total);        
	p_ps2->AddInteger(Crc::ConstCRC("leaf"),  p_metrics->m_leaf);  
	p_ps2->AddInteger(Crc::ConstCRC("objects"),  p_metrics->m_object);  
	p_ps2->AddInteger(Crc::ConstCRC("vert"),  p_metrics->m_verts);  
	p_ps2->AddInteger(Crc::ConstCRC("poly"),  p_metrics->m_polys);  
	p_ps2->AddFloat(Crc::ConstCRC("verts_per_poly"),  (float)p_metrics->m_verts/(float)p_metrics->m_polys);  
	p_ps2->AddFloat(Crc::ConstCRC("polys_per_object"),  (float)p_metrics->m_polys/(float)p_metrics->m_object);  
	p_ps2->AddFloat(Crc::ConstCRC("polys_per_mesh"),  (float)p_metrics->m_polys/(float)p_metrics->m_leaf);  
		   
	p_info->AddStructurePointer(Crc::ConstCRC("scene"),p_ps2);
