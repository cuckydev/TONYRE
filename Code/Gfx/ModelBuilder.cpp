...
bool CModelBuilder::CallMemberFunction( uint32 checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	switch ( checksum )
	{
		...
		case 0x18924da1:		// GeomSetUVOffset
		{
			...
			uint32 material;
			pParams->GetChecksum( Crc::ConstCRC("material"), &material, Script::ASSERT );

			int pass;
			pParams->GetInteger( Crc::ConstCRC("pass"), &pass, Script::ASSERT );
		...
		{
			if ( pParams->ContainsComponentNamed( Crc::ConstCRC("bone_scaling") ) )
			{
				uint32 partChecksum;
				pParams->GetChecksum( vCHECKSUM_PART, &partChecksum, Script::ASSERT );
		...
		case 0xc0bc8271:		// ModelApplyBodyShape
		{
			...
			if ( pParams->GetStructure( vCHECKSUM_BODY_SHAPE, &pBodyShapeStructure, false ) )
			{
				...
				Script::CStruct* pActualStruct = mp_appearance->GetActualDescStructure( Crc::ConstCRC("skater_m_head") );
				if ( pActualStruct && pParams->ContainsFlag( Crc::ConstCRC("is_bighead_cheat") ) ) 
				{
				   pActualStruct->GetStructure( Crc::ConstCRC("bighead_scale_info"), &pBodyShapeStructure );
				}
...
bool CModelBuilder::model_add_geom( uint32 partChecksum )
{
	...
	if ( pActualStruct ) 
	{
		...
		pActualStruct->GetInteger( Crc::ConstCRC("multicolor"), &supports_multiple_colors, Script::NO_ASSERT );
		...
		if ( ( pFaceTexture && pFaceTexture->IsValid() ) || ( mp_appearance->WillEventuallyHaveFaceTexture() ) )
		{
			...
			if ( pActualStruct->GetString( Crc::ConstCRC("mesh_if_facemapped"), &pMeshName, Script::NO_ASSERT ) )
			{
...
bool CModelBuilder::model_apply_object_scale()
{
	...
	Script::CStruct* pVirtualStruct = mp_appearance->GetVirtualDescStructure( Crc::ConstCRC("object_scaling") );
	if ( pVirtualStruct ) 
	{
		if ( !pVirtualStruct->ContainsComponentNamed( vCHECKSUM_USE_DEFAULT_SCALE ) )
		{
...
bool CModelBuilder::geom_modulate_color( Script::CStruct* pParams )
{
	...
	if ( pVirtualStruct ) 
	{
		...
		if ( use_default_hsv )
		{
			...
			int h, s, v;
			if ( pActualStruct
				 && pActualStruct->GetInteger( Crc::ConstCRC("default_h"), &h, Script::NO_ASSERT )
				 && pActualStruct->GetInteger( Crc::ConstCRC("default_s"), &s, Script::NO_ASSERT )
...
// debugging
	...
	if ( pVirtualStruct ) 
	{
		if ( !pVirtualStruct->ContainsComponentNamed( Crc::ConstCRC("use_default_uv") ) )
		{
			return false;
		}
		
		int use_default_uv;
		pVirtualStruct->GetInteger( Crc::ConstCRC("use_default_uv"), &use_default_uv );
...
//			(assumes that we're starting from an unmodified model)
		...
		float u_offset;
        pVirtualStruct->GetFloat(Crc::ConstCRC("uv_u"), &u_offset, Script::ASSERT);
		
		float v_offset;
		pVirtualStruct->GetFloat(Crc::ConstCRC("uv_v"), &v_offset, Script::ASSERT);
		
		float uv_scale;
		pVirtualStruct->GetFloat(Crc::ConstCRC("uv_scale"), &uv_scale, Script::ASSERT);
		
		float uv_rot;
		pVirtualStruct->GetFloat(Crc::ConstCRC("uv_rot"), &uv_rot, Script::ASSERT);
