...
EAnimFunctionResult CBonedAnimController::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	switch ( Checksum )
	{
		...
		case 0xaf2fae19:	// playsequence
		{
			...
			pParams->GetChecksum( Crc::ConstCRC("animname"), &anim_name, Script::ASSERT );
			pParams->GetFloat( Crc::ConstCRC("starttime"), &start_time, Script::ASSERT );
			pParams->GetFloat( Crc::ConstCRC("endtime"), &end_time, Script::ASSERT );
			pParams->GetInteger( Crc::ConstCRC("looptype"), &loop_type, Script::ASSERT );
			pParams->GetFloat( Crc::ConstCRC("blendperiod"), &blend_period, Script::ASSERT );
			pParams->GetFloat( Crc::ConstCRC("speed"), &speed, Script::ASSERT );
...
void CWobbleController::GetDebugInfo( Script::CStruct* p_info )
{
	...
	p_info->AddFloat( Crc::ConstCRC("m_wobbleTargetTime"), m_wobbleTargetTime );

	p_info->AddFloat( Crc::ConstCRC("wobbleAmpA"), m_wobbleDetails.wobbleAmpA );
	p_info->AddFloat( Crc::ConstCRC("wobbleAmpB"), m_wobbleDetails.wobbleAmpB );
	p_info->AddFloat( Crc::ConstCRC("wobbleK1"), m_wobbleDetails.wobbleK1 );
	p_info->AddFloat( Crc::ConstCRC("wobbleK2"), m_wobbleDetails.wobbleK2 );
	p_info->AddFloat( Crc::ConstCRC("spazFactor"), m_wobbleDetails.spazFactor );
#endif				 
}
...
EAnimFunctionResult CWobbleController::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	switch ( Checksum )
	{
		...
		case 0xd0209498:	// setwobbletarget
		{
			...
			if ( pParams->GetFloat( Crc::ConstCRC("wobbletargetalpha"), &wobbleTargetAlpha, Script::NO_ASSERT ) )
			{
				// check bounds
		...
		case 0xea6d0efd:	// setwobbledetails
		{
			...
			if ( pParams->GetFloat( Crc::ConstCRC("wobbleAmpA"), &wobbleAmpA, Script::NO_ASSERT )
				 && pParams->GetFloat( Crc::ConstCRC("wobbleAmpB"), &wobbleAmpB, Script::NO_ASSERT )
				 && pParams->GetFloat( Crc::ConstCRC("wobbleK1"), &wobbleK1, Script::NO_ASSERT )
				 && pParams->GetFloat( Crc::ConstCRC("wobbleK2"), &wobbleK2, Script::NO_ASSERT )
				 && pParams->GetFloat( Crc::ConstCRC("spazFactor"), &spazFactor, Script::NO_ASSERT ) )
			{
...
void CFlipRotateController::InitFromStructure( Script::CStruct* pParams )
{
	...
	int flipped;
	if ( pParams->GetInteger( Crc::ConstCRC("flipped"), &flipped, Script::NO_ASSERT ) )
	{
		m_flipped = flipped;
	}

	int rotated;
	if ( pParams->GetInteger( Crc::ConstCRC("rotated"), &rotated, Script::NO_ASSERT ) )
	{
		m_rotated = rotated;
	}
}
...
//	if ( m_rotated )
	{
		...
		idx = pSkeleton->GetBoneIndexById( Crc::ConstCRC("bone_board_root") );
		if ( idx != -1 )
		{	   
		...
		case 0x02565bd1: //	SetBoneTransMin
			{
				...
				uint32 boneName;
				pParams->GetFloat( Crc::ConstCRC("x"), &vec[X], false );
				pParams->GetFloat( Crc::ConstCRC("y"), &vec[Y], false );
				pParams->GetFloat( Crc::ConstCRC("z"), &vec[Z], false );
				pParams->GetChecksum( Crc::ConstCRC("bone"), &boneName, true );
...
{   
	...
	float speed = 1.0f;
	pParams->GetFloat( Crc::ConstCRC("speed"), &speed, Script::NO_ASSERT );
	
	pParams->GetFloat(Crc::ConstCRC("Current"), &Current);
...
EAnimFunctionResult CPartialAnimController::CallMemberFunction( uint32 Checksum, Script::CStruct* pParams, Script::CScript* pScript )
{
	switch ( Checksum )
	{
		...
		{
			Dbg_Assert( pScript );
			pScript->GetParams()->AddFloat( Crc::ConstCRC("partialAnimTime"), mp_animChannel->GetCurrentAnimTime() );
			pScript->GetParams()->AddChecksum( Crc::ConstCRC("partialAnim"), mp_animChannel->GetCurrentAnim() );
			pScript->GetParams()->AddInteger( Crc::ConstCRC("partialAnimComplete"), mp_animChannel->IsAnimComplete() );
		...
		{
			float incVal;
			pParams->GetFloat( Crc::ConstCRC("incVal"), &incVal, Script::ASSERT );			
