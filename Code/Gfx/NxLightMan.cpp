...
void CLightManager::sUpdateVCLights()
{
	...
	for (; s_next_fake_light_node_to_process<required_num_nodes_processed; ++s_next_fake_light_node_to_process)
	{
		...
		uint32 id=0;
		p_node->GetChecksum(Crc::ConstCRC("Name"),&id);
...
bool ScriptGetLightCurrentColor(Script::CScriptStructure *pParams, Script::CScript *pScript)
{
    ...
    pScript->GetParams()->AddInteger(Crc::ConstCRC("ambient_red"), rgb.r);
    pScript->GetParams()->AddInteger(Crc::ConstCRC("ambient_green"), rgb.g);
    pScript->GetParams()->AddInteger(Crc::ConstCRC("ambient_blue"), rgb.b);

    rgb = CLightManager::sGetLightDiffuseColor(0);

    pScript->GetParams()->AddInteger(Crc::ConstCRC("red_0"), rgb.r);
    pScript->GetParams()->AddInteger(Crc::ConstCRC("green_0"), rgb.g);
    pScript->GetParams()->AddInteger(Crc::ConstCRC("blue_0"), rgb.b);

    rgb = CLightManager::sGetLightDiffuseColor(1);

    pScript->GetParams()->AddInteger(Crc::ConstCRC("red_1"), rgb.r);
    pScript->GetParams()->AddInteger(Crc::ConstCRC("green_1"), rgb.g);
    pScript->GetParams()->AddInteger(Crc::ConstCRC("blue_1"), rgb.b);
...
bool ScriptDrawDirectionalLightLines(Script::CScriptStructure *pParams, Script::CScript *pScript)
{
    ...
    float pitch;
	pParams->GetFloat( Crc::ConstCRC("pitch"), &pitch, Script::ASSERT );
...
#	if 0
	...
	for (uint32 i=0; i<p_nodearray->GetSize(); ++i)
	{
		...
		if (class_checksum == Crc::ConstCRC("LevelLight"))
		{
			...
			p_node_struct->GetFloat(Crc::ConstCRC("Brightness"),&sp_vc_lights[s_num_vc_lights].m_value);	
			p_node_struct->GetFloat(Crc::ConstCRC("Radius"),&sp_vc_lights[s_num_vc_lights].m_radius);	
	...
	if( s_num_vc_lights == 0 )
	{
		...
		for (uint32 i=0; i<p_nodearray->GetSize(); ++i)
		{
			...
			uint32 class_checksum = 0;		
			p_node_struct->GetChecksum( Crc::ConstCRC("Class"), &class_checksum );
			if (class_checksum == Crc::ConstCRC("Pedestrian"))
			{
				SkateScript::GetPosition(p_node_struct,&sp_vc_lights[s_num_vc_lights].m_pos);
...
void CLightManager::sFakeLight(uint32 id, Script::CStruct *pParams)
{
	...
	if( p_light )
	{
		...
		if( pParams->GetFloat( Crc::ConstCRC("percent"), &amount ))
		{
