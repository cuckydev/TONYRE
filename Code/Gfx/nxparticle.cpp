...
bool CParticle::CallMemberFunction( uint32 Checksum, Script::CStruct *pParams, Script::CScript *pScript )
{
	...
	switch(Checksum)
	{
		...
		case 0x99cd17b:		// SetPos
			{
				...
				float y = 0.0f;
				float z = 0.0f;
				if ( pParams->GetVector(Crc::ConstCRC("pos"),&pos) )
				{
					x = pos[X];
					y = pos[Y];
					z = pos[Z];
		...
		case 0x63274395:		// SetSpeedRange
			{
				float min = 0.0f;
				pParams->GetFloat(Crc::ConstCRC("min"),&min);
				float max = min;
				pParams->GetFloat(Crc::ConstCRC("max"),&max);
		...
		case 0x7911855b:		// SetAngleSpread
			{
				float spread = 0.0f;
				pParams->GetFloat(Crc::ConstCRC("spread"),&spread);
