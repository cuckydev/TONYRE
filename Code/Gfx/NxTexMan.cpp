...
bool ScriptLoadTexture(Script::CScriptStructure *pParams, Script::CScript *pScript)
{
	...
	if (pParams->ContainsFlag(Crc::ConstCRC("no_vram_alloc")))
	{
		alloc_vram = false;
	}
...
bool ScriptUnloadTexture(Script::CScriptStructure *pParams, Script::CScript *pScript)
{
	...
	{
		if ( !pParams->ContainsFlag( Crc::ConstCRC("dont_assert") ) )
		{
...
bool ScriptRemoveTextureFromVram(Script::CScriptStructure *pParams, Script::CScript *pScript)
{
	...
	} else if (!pParams->ContainsFlag(Crc::ConstCRC("no_assert"))) {
...
bool ScriptGenerate32BitImage(Script::CScriptStructure *pParams, Script::CScript *pScript)
{
	...
	if (p_texture)
	{
		...
		if (pParams->GetInteger(Crc::ConstCRC("renderable"), &value) && (value == 1))
		{
			renderable = true;
		}
		if (pParams->GetInteger(Crc::ConstCRC("store_original"), &value) && (value == 1))
		{
			store_original = true;
		}
...
bool ScriptOffsetTexture(Script::CScriptStructure *pParams, Script::CScript *pScript)
{
	...
	if (p_texture)
	{
		...
		pParams->GetInteger(Crc::ConstCRC("x_offset"), &x_offset);
		pParams->GetInteger(Crc::ConstCRC("y_offset"), &y_offset);

		use_fill_color |= pParams->GetInteger(Crc::ConstCRC("fill_r"), &r);
		use_fill_color |= pParams->GetInteger(Crc::ConstCRC("fill_g"), &g);
...
bool ScriptPullTextureToEdge(Script::CScriptStructure *pParams, Script::CScript *pScript)
{
	...
	if (p_texture)
	{
		...
		pParams->GetInteger(Crc::ConstCRC("point"), &point);
...
bool ScriptAdjustTextureBrightness(Script::CScriptStructure *pParams, Script::CScript *pScript)
{
	...
	if (p_texture)
	{
		...
		pParams->GetFloat(Crc::ConstCRC("brightness"), &brightness, true);
		pParams->GetInteger(Crc::ConstCRC("adjust_current"), &adjust_current);
		if (adjust_current == 1)
		{
...
bool ScriptAdjustTextureHSV(Script::CScriptStructure *pParams, Script::CScript *pScript)
{
	...
	if (p_texture)
	{
		...
		bool got_params;
		got_params = pParams->GetFloat(Crc::ConstCRC("h"), &h);
		got_params = pParams->GetFloat(Crc::ConstCRC("s"), &s) && got_params;
		got_params = pParams->GetFloat(Crc::ConstCRC("v"), &v) && got_params;
...
bool ScriptCopyTexture(Script::CScriptStructure *pParams, Script::CScript *pScript)
{
	...
	{
		uint32 new_checksum;
		if (pParams->GetChecksum(Crc::ConstCRC("new"), &new_checksum))
		{
...
bool ScriptCombineTextures(Script::CScriptStructure *pParams, Script::CScript *pScript)
{
	...
	if (!pParams->GetChecksum(Crc::ConstCRC("src"), &tex1_checksum))
	{
		Dbg_MsgAssert(0, ("no original source texture specified"));
	}
	if (!pParams->GetChecksum(Crc::ConstCRC("top"), &tex2_checksum))
	{
		Dbg_MsgAssert(0, ("no top source texture specified"));
	}
	pParams->GetChecksum(Crc::ConstCRC("new"), &new_checksum);
...
//	Dbg_MsgAssert(*p_name,("%s\n Empty name string for Particle Texture\n",pScript->GetScriptInfo()));
	...
	bool	perm = pParams->ContainsFlag(Crc::ConstCRC("perm"));
